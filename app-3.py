...
# ── STARTUP ──────────────────────────────────────────────────────
# FIX: With gunicorn.conf.py enforcing workers=1, there is only ONE process.
# We no longer rely on app_context() startup (which fired in every worker).
# Instead, use os.environ as a cross-fork flag so _ensure_started() is
# guaranteed to run exactly once — in the gunicorn worker, not the master.
...
# Gunicorn calls this module in the worker process — start scanner here.
# The os.environ flag prevents a second call if something else imports the module.
if os.environ.get("_SCANNER_STARTED") != "1":
