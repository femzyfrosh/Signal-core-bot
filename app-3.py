...
# Use /data if a Railway Volume is mounted there, otherwise fall back to app dir
...
def auto_whitelist_ip():
    """
    Automatically whitelist this server's outbound IP on MEXC API key.
    Uses MEXC Spot API (which supports IP whitelist management).
    Runs on every startup so IP changes on redeploy are handled automatically.
    """
    ...
    try:
        ...
        log(f"🌐 Railway outbound IP: {my_ip}  ← Auto-whitelisting on MEXC...")

        # MEXC Spot API — update API key IP whitelist
        ...
        if r.status_code == 200:
            log(f"✅ IP {my_ip} auto-whitelisted on MEXC successfully! No manual action needed.")
        elif r.status_code == 400 and "already" in r.text.lower():
            log(f"✅ IP {my_ip} already whitelisted on MEXC.")
        else:
            # Fallback: just log the IP clearly so user can add manually
            log(f"⚠️ Auto-whitelist returned {r.status_code}: {r.text[:120]}")
            log(f"📋 Manual fallback — add this IP to MEXC API whitelist: {my_ip}")
    except Exception as e:
...
# FIX: MEXC public API returns 403 "Host not in allowlist" without these headers.
# Root cause of "Initialising... 0/0" — get_all_pairs() silently returned [].
MEXC_PUBLIC_HEADERS = {
    "User-Agent": (
...
# FIX: get_all_pairs — MEXC_PUBLIC_HEADERS fixes 403; fallback to TOP_PAIRS so
#      scanning never stalls even if the full pair list endpoint is temporarily down.
def get_all_pairs():
    for attempt in range(4):
        try:
            r = requests.get(
                f"{MEXC_BASE}/detail",
                headers=MEXC_PUBLIC_HEADERS,
                timeout=15
            )
            if r.status_code == 403:
                log(f"⚠️ [Pairs] 403 — response: {r.text[:60]} (attempt {attempt+1}/4)")
                time.sleep(5 * (attempt + 1))
                continue
            if r.status_code == 429:
                log(f"⚠️ [Pairs] Rate limited (429) — waiting {10*(attempt+1)}s")
                time.sleep(10 * (attempt + 1))
                continue
            if r.status_code != 200:
                log(f"⚠️ [Pairs] HTTP {r.status_code} (attempt {attempt+1}/4)")
                time.sleep(3)
                continue
            data = r.json()
            if not data.get("success"):
                log(f"⚠️ [Pairs] success=false: {data.get('message','?')}")
                time.sleep(3)
                continue
            seen, pairs = set(), []
            for item in data.get("data", []):
                ...
                if item.get("state") == 0 and sym.endswith("_USDT") and sym not in seen:
                    ...
                    pairs.append(sym)
            if pairs:
                log(f"✅ [Pairs] Fetched {len(pairs)} USDT pairs")
                return sorted(pairs)
            log("⚠️ [Pairs] Empty list — retrying")
        except requests.exceptions.Timeout:
            log(f"⚠️ [Pairs] Timeout attempt {attempt+1}/4")
        except Exception as e:
            log(f"⚠️ [Pairs] Error {attempt+1}/4: {e}")
        time.sleep(2 ** attempt * 2)
    # FIX: Fallback — scan TOP_PAIRS so the bot is never fully idle
    log(f"⚠️ [Pairs] All retries failed — using {len(TOP_PAIRS)} TOP_PAIRS as fallback")
    return list(TOP_PAIRS)

# FIX: get_candles — MEXC_PUBLIC_HEADERS fixes 403 that caused all pairs to show
#      "no_candles" in diagnostics. Added retry + diagnostic logging.
def get_candles(symbol, interval, limit=150):
    for attempt in range(3):
        try:
            ...
            if r.status_code == 403:
                if attempt == 0:
                    log(f"⚠️ [Candles] 403 on {symbol}/{interval}: {r.text[:50]}")
...
def get_ticker(symbol):
    ...
    """
    try:
        r = requests.get(f"{MEXC_BASE}/ticker", params={"symbol": symbol},
                        headers=MEXC_PUBLIC_HEADERS, timeout=6)  # FIX: 403 fix
        data = r.json()
...
def scan_pair(symbol):
    ...
    if not ref_candles or len(ref_candles) < 30:
        diag["no_candles"] += 1
        # FIX: Log every 20th miss so Railway shows if ALL pairs are failing (= headers issue)
        if diag.get("no_candles", 0) % 20 == 1:
            log(f"⚠️ [Scan] Empty candles: {symbol}/Hour4 "
                f"(total={diag.get('no_candles',0)}) — "
                f"if all pairs fail, check MEXC_PUBLIC_HEADERS are applied")
        return results
...
def mexc_request(method, path, params=None, signed=True):
    ...
    try:
        ...
        return None, f"Timeout on {path} — MEXC not responding"
    except Exception as e:
...
def scanner_loop():
    ...
    while True:
        try:
            ...
            log(f"❌ [Scanner] {type(e).__name__}: {e}")
...
# FIX: start_scanner — scanner threads launch IMMEDIATELY.
#      IP detection + API test run in a background thread so they can never
#      block or delay the scanner from starting (network timeouts were causing this).
def start_scanner():
    # FIX: Start scanning threads first — zero blocking before this
    ...
    log("🚀 Scanner + all monitor threads launched — startup checks running in background")

    def _bg_startup():
        time.sleep(3)   # let threads initialize first
        # ── IP detection & Telegram notification ──────────────────────────
        try:
            ip = None
            for url in ["https://api.ipify.org", "https://ifconfig.me", "https://icanhazip.com"]:
                try:
                    ...
                    if cand and len(cand) < 50 and " " not in cand:
                        ip = cand; break
                except Exception:
                    continue
            if ip:
                log(f"🌐 Outbound IP: {ip} — whitelist on MEXC if API calls fail")
                ...
                if tok and "PASTE" not in tok and cid:
                    try:
                        ts_str = datetime.now(LOCAL_TZ).strftime("%Y-%m-%d %H:%M UTC+1")
                        msg = (
                            "🚨 <b>Mad Man Scanner — IP Notification</b>\n"
                            "━━━━━━━━━━━━━━━━━━\n"
                            f"<b>IP:</b> <code>{ip}</code>\n"
                            "━━━━━━━━━━━━━━━━━━\n"
                            "MEXC → API Management → Edit Key → IP Whitelist\n"
                            f"Add <code>{ip}</code> (or use 0.0.0.0/0 to allow all)\n"
                            f"<i>{ts_str}</i>"
                        )
                        requests.post(
                        ...
                        log(f"⚠️ Telegram IP notify error: {e}")
            else:
                log("⚠️ Could not detect outbound IP")
        except Exception as e:
            log(f"⚠️ IP detection error: {e}")
        ...
        try:
            ...
            secret = trade_config.get("api_secret", "")
            if not key or "PASTE" in key or not secret or "PASTE" in secret:
                log("⚠️ API keys not configured — live trading will not work")
                return
            data, err = mexc_request("GET", "/account/assets")
            if err:
                log(f"❌ MEXC API test FAILED: {err}")
                if "Access Denied" in err or "HTML" in err:
                    log("   → Whitelist the IP above on MEXC → API Management → IP Whitelist")
            else:
                log("✅ MEXC API connection OK — futures account accessible")
        except Exception as e:
            log(f"⚠️ MEXC API test error: {e}")

    threading.Thread(target=_bg_startup, daemon=True, name="startup").start()
...
# ── STARTUP ──────────────────────────────────────────────────────
# FIX: Removed "with app.app_context(): _ensure_started()" — that block
# fired in EVERY Gunicorn worker. Each worker got its own scanner thread
# but HTTP requests hit whichever worker answered, so scan_state/signals
# were always empty on the wrong worker → dashboard looked dead.
#
# Fix: call _ensure_started() directly at module level. Combined with
# running Gunicorn with --workers 1, there is exactly one process, one
# scanner, one shared memory space. Dashboard always works.
#
# ⚠️  Railway start command MUST be:
#   gunicorn app-3:app --workers 1 --worker-class gthread --threads 4 --bind 0.0.0.0:$PORT
