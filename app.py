...
const HCORNER = 16;  // corner/endpoint hit radius px
...
function initDrawCanvas(){
  ...
  // Hover cursor (desktop only) — separate listener so it never blocks pDown logic
  ...
  // ── Pass wheel (scroll-to-zoom) through to chart ALWAYS ──────
  canvas.addEventListener('wheel',function(e){
    // Re-dispatch on the chart's underlying canvas so LightweightCharts
    // handles zoom normally regardless of draw canvas being on top
    const chartEl=canvas.parentElement?.querySelector('canvas:not(#draw-canvas)');
    if(chartEl){
      chartEl.dispatchEvent(new WheelEvent('wheel',{
  ...
  function onDown(e){
    ...
    if(hi<0){
      // No shape hit → deselect and re-dispatch to chart for pan/zoom
      selectedIdx=-1;
      redrawCanvas();
      // Re-fire the event on the chart's canvas so pan works
      const chartEl=canvas.parentElement?.querySelector('canvas:not(#draw-canvas)');
      if(chartEl){
        chartEl.dispatchEvent(new MouseEvent('mousedown',{
          bubbles:false,cancelable:true,
          clientX:e.clientX,clientY:e.clientY,
          button:e.button,buttons:e.buttons,
          ctrlKey:e.ctrlKey,shiftKey:e.shiftKey,altKey:e.altKey,metaKey:e.metaKey
        }));
        // Track that chart owns this drag so onMove/onUp also forward
        _chartOwnsPointer=true;
      }
      return;
    }
    _chartOwnsPointer=false;
    ...
    if(hitType==='body'){
      ...
      // Shift+drag on trendline → parallel clone
  ...
  function onMove(e){
    ...
    // If chart owns this drag (started on empty area), forward and bail
    if(_chartOwnsPointer){
      const chartEl=canvas.parentElement?.querySelector('canvas:not(#draw-canvas)');
      if(chartEl){
    ...
    if(_mode==='moving'&&_snapShape){
      const dt=cp.time-ds.time;
      const dp=cp.price-ds.price;
      sh.t1=_snapShape.t1+dt; sh.p1=_snapShape.p1+dp;
      sh.t2=_snapShape.t2+dt; sh.p2=_snapShape.p2+dp;
      // Parallel clone stays at origin; moved line is the new one
      redrawCanvas();
      return;
    }
    ...
    if(_mode==='resizing'&&_snapShape&&_resizeHandle){
      ...
      if(sh.type==='rect'){
        // Each corner drags freely; opposite corner is the anchor
        const snap=_snapShape;
        const x1s=_t2x(snap.t1)||0, y1s=_p2y(snap.p1)||0;
        const x2s=_t2x(snap.t2)||0, y2s=_p2y(snap.p2)||0;
        const lxs=Math.min(x1s,x2s),rxs=Math.max(x1s,x2s);
        const tys=Math.min(y1s,y2s),bys=Math.max(y1s,y2s);
        // anchor = opposite corner pixel coords
        const anchors={
  ...
  function onUp(e){
    ...
    // If chart owned this drag, forward mouseup and reset
    if(_chartOwnsPointer){
      _chartOwnsPointer=false;
      const chartEl=canvas.parentElement?.querySelector('canvas:not(#draw-canvas)');
      if(chartEl){
        chartEl.dispatchEvent(new MouseEvent('mouseup',{
          bubbles:false,cancelable:true,
          clientX:e.clientX,clientY:e.clientY,
          button:e.button,buttons:e.buttons,
          ctrlKey:e.ctrlKey,shiftKey:e.shiftKey,altKey:e.altKey,metaKey:e.metaKey
    ...
    if(_mode==='drawing'&&previewShape&&moved){
      ...
      // Exit draw tool after placing shape — click again selects/moves it
    ...
    // Single tap = select; double tap = open settings
    if(!moved){
      const{x,y}=_canvasXY(canvas,cx,cy);
      const hi=_findShape(x,y);
      const now=Date.now();
