...
scan_settings = {
    ...
    "scan_interval":         1,
    "cycle_rest":            5,
    "market_refresh_interval": 10,   # seconds between live price updates on market page
    "tg_signals":        True,
    "tg_trades":         True,
...
def _run_startup_diagnostics():
    ...
    msg_lines = [
        ...
        f"<b>Trading Mode:</b>    Cross Margin (openType=2)",
    ...
    log(f"   Trading mode : Cross Margin (openType=2)")
...
def mexc_request(method, path, params=None, signed=True, _retry=0):
    ...
    try:
        ...
        if r.status_code == 403:
            ...
            log(f"     1. IP {_get_cached_ip()} not whitelisted on MEXC → check API key IP restrictions")
...
def place_order(sig):
    ...
    log(f"   leverage={leverage}x openType={open_type} (Cross Margin)")
...
<div class="tab-wrap">
  <div class="tabs">
    ...
    <button class="tab" onclick="sw('market',this)">📈 Market</button>
    ...
    <button class="tab" onclick="sw('trade-cfg',this)">🤖 Live Auto-Trade</button>
  ...
  <div id="tab-market" style="display:none">
    ...
    <div id="mkt-wrap" style="display:flex;flex-direction:column;height:82vh;border:1.5px solid rgba(79,110,247,.18);border-radius:16px;overflow:hidden;background:#0d0f1e">
      <!-- WATCHLIST VIEW -->
      <div id="mkt-wl-view" style="display:flex;flex-direction:column;height:100%">
        <div style="padding:12px 14px 8px;flex-shrink:0;border-bottom:1px solid rgba(79,110,247,.12)">
          <div style="font-size:.7rem;font-weight:800;letter-spacing:.12em;color:#6b7299;text-transform:uppercase;margin-bottom:8px">📡 Watchlist · 30 Pairs</div>
          <div style="position:relative">
            <span style="position:absolute;left:10px;top:50%;transform:translateY(-50%);color:#6b7299;font-size:.8rem;pointer-events:none">🔍</span>
            <input id="mkt-search" type="text" placeholder="Search symbol..." oninput="mktFilter(this.value)" style="width:100%;background:#12152a;border:1px solid rgba(79,110,247,.18);border-radius:10px;padding:8px 12px 8px 32px;color:#e8eaf6;font-size:.82rem;outline:none"/>
          </div>
        </div>
        <div id="mkt-list" style="flex:1;overflow-y:auto;padding-bottom:8px;scrollbar-width:thin;scrollbar-color:#2a2f52 transparent"></div>
      </div>
      <!-- CHART VIEW -->
      <div id="mkt-chart-view" style="display:none;flex-direction:column;height:100%">
        <div style="padding:10px 14px 6px;flex-shrink:0;border-bottom:1px solid rgba(79,110,247,.12)">
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px">
            <button onclick="mktBackToList()" style="background:#12152a;border:1px solid rgba(79,110,247,.18);border-radius:8px;color:#6b7299;padding:4px 10px;font-size:.72rem;font-weight:600;cursor:pointer">← Back</button>
            <span id="mkt-ch-sym" style="font-size:1rem;font-weight:800;letter-spacing:.03em">-</span>
            <span style="font-size:.58rem;padding:2px 7px;border-radius:5px;background:rgba(79,110,247,.12);border:1px solid rgba(79,110,247,.2);color:#4f6ef7;letter-spacing:.05em">PERP</span>
            <span id="mkt-ch-chg" style="margin-left:auto;font-size:.7rem;font-weight:700;padding:2px 8px;border-radius:6px">-</span>
          </div>
          <div style="display:flex;align-items:baseline;gap:10px;margin-bottom:4px">
            <span id="mkt-ch-price" style="font-size:1.3rem;font-weight:700;font-family:JetBrains Mono,monospace">-</span>
          </div>
          <div style="display:flex;gap:14px">
            <div><div style="font-size:.52rem;color:#6b7299;text-transform:uppercase;letter-spacing:.07em">High 24h</div><div id="mkt-ch-high" style="font-size:.68rem;font-weight:700;font-family:JetBrains Mono,monospace">-</div></div>
            <div><div style="font-size:.52rem;color:#6b7299;text-transform:uppercase;letter-spacing:.07em">Low 24h</div><div id="mkt-ch-low" style="font-size:.68rem;font-weight:700;font-family:JetBrains Mono,monospace">-</div></div>
          </div>
        </div>
        <div id="mkt-tf-bar" style="display:flex;gap:4px;padding:7px 12px;flex-shrink:0;border-bottom:1px solid rgba(79,110,247,.12);overflow-x:auto;scrollbar-width:none">
          <button class="mkt-tf" data-tf="Min1" onclick="mktSwitchTF(this,'Min1')">1m</button>
          <button class="mkt-tf" data-tf="Min2" onclick="mktSwitchTF(this,'Min2')">2m</button>
          <button class="mkt-tf" data-tf="Min3" onclick="mktSwitchTF(this,'Min3')">3m</button>
          <button class="mkt-tf" data-tf="Min4" onclick="mktSwitchTF(this,'Min4')">4m</button>
          <button class="mkt-tf mkt-tf-active" data-tf="Min5" onclick="mktSwitchTF(this,'Min5')">5m</button>
          <button class="mkt-tf" data-tf="Min15" onclick="mktSwitchTF(this,'Min15')">15m</button>
          <button class="mkt-tf" data-tf="Min30" onclick="mktSwitchTF(this,'Min30')">30m</button>
          <button class="mkt-tf" data-tf="Min45" onclick="mktSwitchTF(this,'Min45')">45m</button>
          <button class="mkt-tf" data-tf="Min60" onclick="mktSwitchTF(this,'Min60')">1h</button>
          <button class="mkt-tf" data-tf="Hour2" onclick="mktSwitchTF(this,'Hour2')">2h</button>
          <button class="mkt-tf" data-tf="Hour3" onclick="mktSwitchTF(this,'Hour3')">3h</button>
          <button class="mkt-tf" data-tf="Hour4" onclick="mktSwitchTF(this,'Hour4')">4h</button>
          <button class="mkt-tf" data-tf="Day1" onclick="mktSwitchTF(this,'Day1')">1D</button>
        </div>
        <div style="flex:1;position:relative;min-height:0">
          <div id="mkt-chart-container" style="width:100%;height:100%"></div>
          <div id="mkt-chart-loading" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:#0d0f1e;z-index:5;flex-direction:column;gap:10px">
            <div style="width:30px;height:30px;border:3px solid #2a2f52;border-top-color:#4f6ef7;border-radius:50%;animation:spin .7s linear infinite"></div>
            <div style="font-size:.62rem;color:#6b7299;letter-spacing:.05em">Loading chart...</div>
          </div>
        </div>
      </div>
    </div>
    <style>
    .mkt-tf{padding:4px 10px;border-radius:7px;border:1px solid rgba(79,110,247,.18);background:transparent;color:#6b7299;font-size:.63rem;font-weight:700;cursor:pointer;white-space:nowrap;flex-shrink:0;transition:all .15s}
    .mkt-tf-active{background:#4f6ef7!important;border-color:#4f6ef7!important;color:#fff!important;box-shadow:0 0 10px rgba(79,110,247,.35)}
    .mkt-row{display:flex;align-items:center;padding:10px 14px;border-bottom:1px solid rgba(79,110,247,.06);cursor:pointer;transition:background .12s;gap:10px}
    .mkt-row:active{background:rgba(79,110,247,.06)}
    .mkt-icon{width:36px;height:36px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:.85rem;flex-shrink:0;font-weight:700;border:1px solid;position:relative}
    .mkt-sig-badge{position:absolute;top:-3px;right:-3px;width:9px;height:9px;border-radius:50%;background:#ffc93c;border:2px solid #0d0f1e;animation:blink 1.5s infinite}
    .mkt-price-load{display:inline-block;animation:mkt-pulse 1.1s ease-in-out infinite}
    @keyframes mkt-pulse{0%,100%{opacity:.3}50%{opacity:1}}
    </style>
    <script>
    (function(){
    var MKT_PAIRS=["PENGU_USDT","GME_USDT","MEME_USDT","RIVER_USDT","DRIFT_USDT","FARTCOIN_USDT","FLOKI_USDT","BONK_USDT","WIF_USDT","PEPE_USDT","AVAX_USDT","POPCAT_USDT","ONDO_USDT","ARB_USDT","RENDER_USDT","FET_USDT","OPG_USDT","APT_USDT","LINK_USDT","TAO_USDT","INJ_USDT","SEI_USDT","HBAR_USDT","KAS_USDT","NEAR_USDT","SUI_USDT","HYPE_USDT","MAT_USDT","XRP_USDT","BAN_USDT"];
    var MKT_COLORS=[["#4f6ef7","rgba(79,110,247,.15)"],["#7c3aed","rgba(124,58,237,.15)"],["#00d4aa","rgba(0,212,170,.15)"],["#ff4d6a","rgba(255,77,106,.15)"],["#ffc93c","rgba(255,201,60,.15)"],["#06b6d4","rgba(6,182,212,.15)"],["#f97316","rgba(249,115,22,.15)"],["#a78bfa","rgba(167,139,250,.15)"],["#10b981","rgba(16,185,129,.15)"],["#ec4899","rgba(236,72,153,.15)"]];
    var MKT_EMOJI={BTC:"₿",ETH:"Ξ",SOL:"◎",BNB:"⬡",AVAX:"🏔",LINK:"🔗",INJ:"💉",SUI:"💧",NEAR:"🔮",KAS:"💎",XRP:"✦",ARB:"🔵",HBAR:"ℏ",SEI:"🌊",APT:"⚡",TAO:"τ",RENDER:"🎨",FET:"🤖",PENGU:"🐧",GME:"🎮",MEME:"😂",FLOKI:"🐕",BONK:"🐶",WIF:"🎩",PEPE:"🐸",POPCAT:"🐱",ONDO:"🔷",DRIFT:"🌊",RIVER:"🏞",FARTCOIN:"💨",HYPE:"🔥",BAN:"🍌"};
    var mktPrices={},mktSigsMap={},mktFilterStr="",mktCurPair=null,mktCurTF="Min5";
    var mktChart=null,mktSeries=null,mktChartTimer=null,mktPriceTimer=null;
    function mktPairColor(sym){var h=0;for(var i=0;i<sym.length;i++){h=(h*31+sym.charCodeAt(i))&0xffff;}return MKT_COLORS[h%MKT_COLORS.length];}
    function mktEmoji(sym){var b=sym.replace("_USDT","");return MKT_EMOJI[b]||b.slice(0,2).toUpperCase();}
    function mktFmtPrice(v){if(!v&&v!==0)return"-";var n=Number(v);if(n>=1000)return"$"+n.toLocaleString("en",{maximumFractionDigits:2});if(n>=100)return"$"+n.toFixed(2);if(n>=1)return"$"+n.toFixed(4);if(n>=0.01)return"$"+n.toFixed(5);if(n>=0.0001)return"$"+n.toFixed(6);return"$"+n.toFixed(8);}
    window.mktFilter=function(v){mktFilterStr=v.trim().toUpperCase();mktRenderList();};
    function mktRenderList(){
      var list=document.getElementById("mkt-list");
      if(!list)return;
      var fil=MKT_PAIRS.filter(function(s){return!mktFilterStr||s.includes(mktFilterStr)||s.replace("_USDT","").includes(mktFilterStr);});
      if(!fil.length){list.innerHTML='<div style="text-align:center;color:#6b7299;padding:40px 16px;font-size:.8rem">No pairs match</div>';return;}
      list.innerHTML=fil.map(function(sym){
        var d=mktPrices[sym]||{};
        var sig=mktSigsMap[sym];
        var cols=mktPairColor(sym);var clr=cols[0],bg=cols[1];
        var base=sym.replace("_USDT","");
        var hp=d.price!=null&&d.price!==undefined;
        var chg=Number(d.change||0);
        var up=chg>0,dn=chg<0;
        var chgCls=up?"color:#00d4aa":dn?"color:#ff4d6a":"color:#6b7299";
        var chgBg=up?"background:rgba(0,212,170,.1)":dn?"background:rgba(255,77,106,.1)":"background:rgba(107,114,153,.1)";
        var chgTxt=hp?(up?"+":"")+chg.toFixed(2)+"%":"···";
        var priceColor=hp?(up?"#00d4aa":dn?"#ff4d6a":"#e8eaf6"):"#6b7299";
        var priceHtml=hp?'<span style="color:'+priceColor+';font-family:monospace;font-size:.85rem;font-weight:700">'+mktFmtPrice(d.price)+'</span>'
          :'<span class="mkt-price-load" style="color:#6b7299;font-family:monospace;font-size:.85rem">···</span>';
        return '<div class="mkt-row" onclick="mktOpenPair(\''+ sym +'\')">'+
          '<div class="mkt-icon" style="background:'+bg+';border-color:'+clr+'40;color:'+clr+'">'+mktEmoji(sym)+(sig?'<span class="mkt-sig-badge"></span>':"")+'</div>'+
          '<div style="flex:1;min-width:0">'+
          '<div style="font-weight:700;font-size:.86rem">'+base+'<span style="color:#6b7299;font-weight:400;font-size:.72rem">/USDT</span></div>'+
          '<div style="font-size:.62rem;color:#6b7299;margin-top:1px">MEXC · PERP'+(sig?' · 🎯 '+sig.direction+' '+sig.grade:'')+"</div>"+
          '</div>'+
          '<div style="text-align:right;flex-shrink:0">'+priceHtml+
          '<div style="margin-top:3px"><span style="'+chgCls+';'+chgBg+';font-size:.62rem;font-weight:700;padding:2px 6px;border-radius:5px;display:inline-block">'+chgTxt+'</span></div></div>'+
          '<span style="color:#2a2f52;font-size:.65rem;margin-left:4px">›</span></div>';
      }).join("");
    }
    async function mktFetchPrices(){
      try{
        var ctrl=new AbortController();setTimeout(function(){ctrl.abort();},7000);
        var r=await fetch("/api/prices",{signal:ctrl.signal});
        var data=await r.json();
        if(data&&Object.keys(data).length>0){Object.assign(mktPrices,data);mktRenderList();if(mktCurPair)mktUpdateHeader();}
      }catch(e){}
    }
    async function mktFetchMexcDirect(){
      try{
        var r=await fetch("https://contract.mexc.com/api/v1/contract/ticker");
        var data=await r.json();
        if(!data.success||!data.data)return;
        var tickers=Array.isArray(data.data)?data.data:[data.data];
        tickers.forEach(function(d){
          var sym=d.symbol;
          if(!MKT_PAIRS.includes(sym))return;
          var price=parseFloat(d.lastPrice||d.last||0);
          var high=parseFloat(d.high24h||d.high||0);
          var low=parseFloat(d.low24h||d.low||0);
          var chg=parseFloat(d.priceChangePercent||d.changeRate||d.riseFallRate||0);
          if(chg!==0&&Math.abs(chg)<1.5)chg*=100;
          if(chg===0&&price>0){var o=parseFloat(d.open24h||d.openPrice||0);if(o>0)chg=parseFloat(((price-o)/o*100).toFixed(2));}
          mktPrices[sym]={price:parseFloat(price.toFixed(8)),change:parseFloat(chg.toFixed(2)),high:parseFloat(high.toFixed(8)),low:parseFloat(low.toFixed(8))};
        });
        mktRenderList();if(mktCurPair)mktUpdateHeader();
      }catch(e){}
    }
    async function mktFetchSignals(){
      try{
        var r=await fetch("/api/signals?limit=200");var arr=await r.json();
        mktSigsMap={};arr.forEach(function(s){if(!mktSigsMap[s.symbol])mktSigsMap[s.symbol]=s;});
        mktRenderList();
      }catch(e){}
    }
    window.mktOpenPair=function(sym){
      mktCurPair=sym;
      document.getElementById("mkt-wl-view").style.display="none";
      var cv=document.getElementById("mkt-chart-view");
      cv.style.cssText="display:flex;flex-direction:column;height:100%";
      document.getElementById("mkt-ch-sym").textContent=sym.replace("_USDT","")+"/USDT";
      mktUpdateHeader();mktLoadChart(sym,mktCurTF);
    };
    window.mktBackToList=function(){
      mktCurPair=null;
      if(mktChart){try{mktChart.remove();}catch(e){}mktChart=null;mktSeries=null;}
      if(mktChartTimer){clearInterval(mktChartTimer);mktChartTimer=null;}
      document.getElementById("mkt-chart-view").style.display="none";
      document.getElementById("mkt-wl-view").style.cssText="display:flex;flex-direction:column;height:100%";
    };
    window.mktSwitchTF=function(btn,tf){
      document.querySelectorAll(".mkt-tf").forEach(function(b){b.classList.remove("mkt-tf-active");});
      btn.classList.add("mkt-tf-active");mktCurTF=tf;if(mktCurPair)mktLoadChart(mktCurPair,tf);
    };
    async function mktFetchCandles(sym,tf){
      try{
        var ctrl=new AbortController();setTimeout(function(){ctrl.abort();},6000);
        var r=await fetch("/api/candles/"+sym+"?interval="+tf+"&limit=300",{signal:ctrl.signal});
        var data=await r.json();
        if(!Array.isArray(data)||!data.length)return[];
        return data.sort(function(a,b){return a.time-b.time;});
      }catch(e){return[];}
    }
    async function mktLoadChart(sym,tf){
      var loading=document.getElementById("mkt-chart-loading");
      var container=document.getElementById("mkt-chart-container");
      if(loading)loading.style.display="flex";
      if(mktChart){try{mktChart.remove();}catch(e){}mktChart=null;mktSeries=null;}
      if(mktChartTimer){clearInterval(mktChartTimer);mktChartTimer=null;}
      container.innerHTML="";
      var candles=await mktFetchCandles(sym,tf);
      if(loading)loading.style.display="none";
      var lc=window.LightweightCharts;
      if(!candles.length||!lc){
        container.innerHTML='<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#6b7299;font-size:.8rem;flex-direction:column;gap:8px"><div style="font-size:2rem">📊</div><div>No data for '+sym.replace("_USDT","")+" · "+tf+"</div></div>";
        return;
      }
      mktChart=lc.createChart(container,{width:container.clientWidth,height:container.clientHeight,layout:{background:{type:"solid",color:"#0d0f1e"},textColor:"#6b7299"},grid:{vertLines:{color:"rgba(79,110,247,.07)"},horzLines:{color:"rgba(79,110,247,.07)"}},crosshair:{mode:lc.CrosshairMode.Normal,vertLine:{color:"rgba(79,110,247,.4)"},horzLine:{color:"rgba(79,110,247,.4)"}},rightPriceScale:{borderColor:"rgba(79,110,247,.12)"},timeScale:{borderColor:"rgba(79,110,247,.12)",timeVisible:true,secondsVisible:false,fixLeftEdge:true},handleScroll:{mouseWheel:true,pressedMouseMove:true,horzTouchDrag:true},handleScale:{axisPressedMouseMove:true,mouseWheel:true,pinch:true}});
      mktSeries=mktChart.addCandlestickSeries({upColor:"#00d4aa",downColor:"#ff4d6a",borderUpColor:"#00d4aa",borderDownColor:"#ff4d6a",wickUpColor:"#00d4aa",wickDownColor:"#ff4d6a"});
      mktSeries.setData(candles);
      var sig=mktSigsMap[sym];
      if(sig&&sig.entry&&sig.sl&&sig.tp){
        var entry=parseFloat(sig.entry),sl=parseFloat(sig.sl),tp=parseFloat(sig.tp);
        if(!isNaN(entry))mktChart.addPriceLine({price:entry,color:"rgba(255,201,60,.8)",lineWidth:1,lineStyle:lc.LineStyle.Dashed,axisLabelVisible:true,title:"Entry"});
        if(!isNaN(sl))mktChart.addPriceLine({price:sl,color:"rgba(255,77,106,.7)",lineWidth:1,lineStyle:lc.LineStyle.Dotted,axisLabelVisible:true,title:"SL"});
        if(!isNaN(tp))mktChart.addPriceLine({price:tp,color:"rgba(0,212,170,.7)",lineWidth:1,lineStyle:lc.LineStyle.Dotted,axisLabelVisible:true,title:"TP"});
      }
      mktChart.timeScale().fitContent();
      var ro=new ResizeObserver(function(){if(mktChart&&container.clientWidth&&container.clientHeight){mktChart.applyOptions({width:container.clientWidth,height:container.clientHeight});}});
      ro.observe(container);
      mktChartTimer=setInterval(async function(){
        if(!mktSeries||mktCurPair!==sym)return;
        var fresh=await mktFetchCandles(sym,tf);
        if(fresh.length>0){fresh.slice(-2).forEach(function(c){mktSeries.update(c);});}
        mktUpdateHeader();
      },8000);
    }
    function mktUpdateHeader(){
      if(!mktCurPair)return;
      var d=mktPrices[mktCurPair]||{};
      var chg=Number(d.change||0);var up=chg>=0;
      var pe=document.getElementById("mkt-ch-price");var ce=document.getElementById("mkt-ch-chg");
      if(pe){pe.textContent=mktFmtPrice(d.price);pe.style.color=up?"#00d4aa":"#ff4d6a";}
      if(ce){ce.textContent=(up?"+":"")+chg.toFixed(2)+"%";ce.style.background=up?"rgba(0,212,170,.1)":"rgba(255,77,106,.1)";ce.style.color=up?"#00d4aa":"#ff4d6a";}
      var he=document.getElementById("mkt-ch-high"),le=document.getElementById("mkt-ch-low");
      if(he)he.textContent=mktFmtPrice(d.high);if(le)le.textContent=mktFmtPrice(d.low);
    }
    window.mktInit=function(){
      mktRenderList();
      mktFetchPrices();mktFetchMexcDirect();mktFetchSignals();
      if(!mktPriceTimer){mktPriceTimer=setInterval(function(){mktFetchPrices();mktFetchMexcDirect();},15000);}
    };
    })();
    <\/script>
  </div>
    ...
    <div class="panel">
      ...
      <div class="trade-form">
        ...
        <div class="tf-group">
          ...
          <input class="tf-inp" type="number" id="s-market-refresh" min="3" max="300" step="1" value="10"/>
...
async function fetchMonitor(){if(activeTab!=="monitor")return;try{
...
async function loadTradeConfig(){try{const r=await fetch("/api/trade-config");const d=await r.json();$("t-risk").value=d.risk_pct||1;if($("t-leverage"))$("t-leverage").value=d.leverage||0;if($("t-max"))$("t-max").value=d.max_trades||3;updateTradeBadge(d.enabled);}catch{}}
...
window.sw=function(tab,btn){activeTab=tab;document.querySelectorAll(".tab").forEach(b=>b.classList.remove("active"));btn.classList.add("active");["signals","market","trades","monitor","history","trade-cfg","paper","log","settings"].forEach(t=>{const el=$("tab-"+t);if(el)el.style.display=t===tab?"block":"none";});if(tab==="market"){if(typeof mktInit==="function")mktInit();}if(tab==="log"){fetchLog();fetchDiag();}if(tab==="trades")window.fetchPnl();if(tab==="monitor")fetchMonitor();if(tab==="history")fetchHistory();if(tab==="trade-cfg"){loadTradeConfig();syncModelToggles();window.fetchPnl();}if(tab==="paper"){loadPaperConfig();fetchPaperData();}if(tab==="settings")loadSettings();};
...
async function loadSettings(){
  try{
    ...
    if(g("s-market-refresh"))g("s-market-refresh").value=d.market_refresh_interval||10;
...
async function poll(){tick++;const ps=[fetchSigs(),fetchStats(),fetchState()];if(tick%(priceRefreshSecs)===0)ps.push(fetchPrices());if(tick%3===0)ps.push(fetchPnlTicker());if(activeTab==="log")ps.push(fetchLog());if(activeTab==="log"&&tick%3===0)ps.push(fetchDiag());if(activeTab==="monitor"&&tick%2===0)ps.push(fetchMonitor());if(activeTab==="trades"&&tick%3===0)ps.push(window.fetchPnl());if(activeTab==="trade-cfg"&&tick%3===0)ps.push(window.fetchPnl());if(activeTab==="history"&&tick%5===0)ps.push(fetchHistory());if(activeTab==="paper"&&tick%2===0)ps.push(fetchPaperData());await Promise.all(ps);setTimeout(poll,1000);}
...
def api_prices():
    ...
    def fetch_one(sym):
        ...
        if t:
            with lock:
                out[sym]=t
    threads=[threading.Thread(target=fetch_one,args=(sym,),daemon=True) for sym in TOP_PAIRS]
...
def start_scanner():
    ...
    log("🚀 Scanner + M1 + M2a/M2b + M3 + paper monitor threads launched.")
...
/* ─── WATCHLIST PAGE ─── */
.wl-header{padding:14px 16px 10px;flex-shrink:0}
.wl-title{font-family:var(--display);font-size:.8rem;font-weight:800;letter-spacing:.12em;color:var(--dim);text-transform:uppercase;margin-bottom:8px}
.wl-search{position:relative}
.wl-search input{width:100%;background:var(--s2);border:1px solid var(--border);border-radius:10px;padding:9px 12px 9px 34px;color:var(--text);font-family:var(--font);font-size:.82rem;outline:none;transition:border-color .2s}
.wl-search input:focus{border-color:var(--accent)}
.wl-search input::placeholder{color:var(--dim)}
.search-ico{position:absolute;left:10px;top:50%;transform:translateY(-50%);color:var(--dim);font-size:.8rem;pointer-events:none}
...
.wl-row{display:flex;align-items:center;padding:11px 16px;border-bottom:1px solid rgba(79,110,247,.06);cursor:pointer;transition:background .15s;position:relative;gap:12px}
.wl-row:active{background:rgba(79,110,247,.06)}
.wl-row::after{content:'';position:absolute;left:0;top:0;bottom:0;width:3px;background:transparent;transition:background .2s}
.wl-row.has-signal::after{background:var(--yellow)}
.sym-icon{width:38px;height:38px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:1rem;flex-shrink:0;font-weight:700;font-family:var(--mono);border:1px solid;position:relative}
.sym-icon .signal-badge{position:absolute;top:-4px;right:-4px;width:10px;height:10px;border-radius:50%;background:var(--yellow);border:2px solid var(--bg);animation:blink 1.5s infinite}
.wl-info{flex:1;min-width:0}
.wl-sym{font-weight:700;font-size:.88rem;letter-spacing:.02em}
.wl-base{font-size:.65rem;color:var(--dim);font-family:var(--mono);margin-top:1px}
.wl-right{text-align:right;flex-shrink:0}
.wl-price{font-family:var(--mono);font-size:.88rem;font-weight:700}
.wl-chg{font-family:var(--mono);font-size:.65rem;font-weight:700;padding:2px 6px;border-radius:5px;margin-top:3px;display:inline-block}
.wl-chg.up{background:rgba(0,212,170,.1);color:var(--green)}
.wl-chg.dn{background:rgba(255,77,106,.1);color:var(--red)}
.wl-chg.flat{background:rgba(107,114,153,.1);color:var(--dim)}
.wl-arrow{color:var(--muted);font-size:.65rem;margin-left:2px;flex-shrink:0}
/* ─── CHART PAGE ─── */
...
/* ─── PRICE LOADING ─── */
.price-loading{display:inline-block;color:var(--dim);animation:pulse 1.2s ease-in-out infinite}
...
<div class="app">
  ...
  <div class="topbar">
    <div class="topbar-left">
      <button class="back-btn" id="back-btn" onclick="goToWatchlist()">← Back</button>
      <div class="logo" id="logo-txt">SignalCore</div>
      <div class="scanner-dot"></div>
      <span class="pair-label" id="pair-label"></span>
    </div>
  ...
  <div class="main">
    ...
    <div class="page hidden-right" id="page-chart">
      <div class="chart-header">
        ...
        <div class="ch-stats">
          <div class="ch-stat"><span class="ch-stat-lbl">High 24h</span><span class="ch-stat-val" id="ch-high">–</span></div>
          <div class="ch-stat"><span class="ch-stat-lbl">Low 24h</span><span class="ch-stat-val" id="ch-low">–</span></div>
  ...
  <nav class="bottom-nav">
    ...
    <a class="nav-btn" href="/dashboard" style="text-decoration:none">
      <span class="nav-ico">📡</span>
      <span class="nav-lbl">Signals</span>
    </a>
  </nav>
...
function goToWatchlist(){
  ...
  document.getElementById("pair-label").classList.remove("show");
  document.getElementById("logo-txt").style.display="";
  stopChartUpdate();
}
window.goToWatchlist=goToWatchlist;

function goToChart(sym,tf){
  const wp=document.getElementById("page-watchlist");
  const cp=document.getElementById("page-chart");
  ...
  document.getElementById("pair-label").classList.add("show");
  document.getElementById("pair-label").textContent=(sym||"").replace("_USDT","")+"/USDT";
  loadChart(sym,tf||currentTF);
...
async function fetchPrices(){
  try{
    ...
    const r=await fetch("/api/prices",{signal:ctrl.signal});
    clearTimeout(tid);
    ...
    // Fallback: fetch directly from MEXC for visible pairs
    fetchMexcDirect();
  }
}

async function fetchMexcDirect(){
  // Fetch tickers directly from MEXC public API as fallback
  try{
    const r=await fetch("https://contract.mexc.com/api/v1/contract/ticker",{method:"GET"});
    const data=await r.json();
    if(!data.success||!data.data)return;
    ...
    tickers.forEach(d=>{
      ...
      const high=parseFloat(d.high24h||d.high||0);
      const low=parseFloat(d.low24h||d.low||0);
      let change=parseFloat(d.priceChangePercent||d.changeRate||d.riseFallRate||0);
      if(change!==0&&Math.abs(change)<1.5)change=change*100;
      if(change===0&&price>0){
        const open24=parseFloat(d.open24h||d.openPrice||0);
        if(open24>0)change=parseFloat(((price-open24)/open24*100).toFixed(2));
      }
      prices[sym]={price:parseFloat(price.toFixed(8)),change:parseFloat(change.toFixed(2)),high:parseFloat(high.toFixed(8)),low:parseFloat(low.toFixed(8))};
    });
    renderWatchlist();
    updateChartHeader();
  }catch{}
...
function renderWatchlist(){
  ...
  if(!filtered.length){
    list.innerHTML='<div style="text-align:center;color:var(--dim);padding:40px 16px;font-size:.8rem">No pairs match your search</div>';
    return;
  }
  list.innerHTML=filtered.map(sym=>{
    ...
    const chgTxt=hasPrice?(up?"+":"")+chg.toFixed(2)+"%":"...";
    const priceDisplay=hasPrice?fmtPrice(d.price):'<span class="price-loading">···</span>';
    return`<div class="wl-row${sig?" has-signal":""}" onclick="openPair('${sym}')">
      <div class="sym-icon" style="background:${bg};border-color:${clr}40;color:${clr}">
        ${pairEmoji(sym)}${sig?'<span class="signal-badge"></span>':""}
      </div>
      <div class="wl-info">
        <div class="wl-sym">${base}<span style="color:var(--dim);font-weight:400;font-size:.75rem">/USDT</span></div>
        <div class="wl-base">MEXC · PERP${sig?` · 🎯 ${sig.direction} ${sig.grade}`:""}</div>
      </div>
      <div class="wl-right">
        <div class="wl-price" style="color:${hasPrice?(up?"var(--green)":dn?"var(--red)":"var(--text)"):"var(--dim)"}">${priceDisplay}</div>
        <span class="wl-chg ${hasPrice?chgCls:"flat"}">${chgTxt}</span>
      </div>
...
window.switchTF=function(btn,tf){
  ...
  currentTF=tf;
  if(currentPair){loadChart(currentPair,tf);saveLastPair(currentPair,tf);}
}
...
async function loadChart(sym,tf){
  ...
  if(sig&&sig.entry&&sig.sl&&sig.tp){
    ...
    if(!isNaN(entry)){
      const entryLine=chart.addPriceLine({price:entry,color:"rgba(255,201,60,.8)",lineWidth:1,lineStyle:LightweightCharts.LineStyle.Dashed,axisLabelVisible:true,title:"Entry"});
    }
    if(!isNaN(sl)){
      chart.addPriceLine({price:sl,color:"rgba(255,77,106,.7)",lineWidth:1,lineStyle:LightweightCharts.LineStyle.Dotted,axisLabelVisible:true,title:"SL"});
    }
    if(!isNaN(tp)){
      chart.addPriceLine({price:tp,color:"rgba(0,212,170,.7)",lineWidth:1,lineStyle:LightweightCharts.LineStyle.Dotted,axisLabelVisible:true,title:"TP"});
    }
  }
...
function updateChartHeader(){
  ...
  document.getElementById("ch-high").textContent=fmtPrice(d.high);
  document.getElementById("ch-low").textContent=fmtPrice(d.low);
}
...
function startChartUpdate(sym,tf){
  stopChartUpdate();
  chartTimer=setInterval(()=>updateLatestCandle(sym,tf),8000);
}
...
// ── Init ────────────────────────────────────────────────────────────
async function init(){
  // Render pairs immediately (with loading placeholders) so list is never blank
  renderWatchlist();

  // Start fetching prices and signals concurrently
  fetchPrices();
  fetchMexcDirect(); // also try direct as fast path
  fetchSignals();

  // Fetch refresh interval from settings (default 10s)
  let refreshSecs=10;
  try{
    const r=await fetch("/api/settings");
    const s=await r.json();
    if(s.market_refresh_interval&&s.market_refresh_interval>0){
      refreshSecs=parseInt(s.market_refresh_interval);
    }
  }catch{}

  // Price refresh every N seconds (configurable in Settings)
  priceTimer=setInterval(fetchPrices,refreshSecs*1000);
