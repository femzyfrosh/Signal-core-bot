...
def api_ticker(symbol):
    """
    Live ticker for a single symbol.
    Always fetches direct from MEXC — never reads from cache.
    Returns {ok, price, change, high, low} with high/low guaranteed non-zero
    (falls back to Day1 candle high/low if ticker fields are missing).
    """
    t = get_ticker(symbol)
    if t and t.get("price", 0) > 0:
        # Update the batch cache so watchlist stays in sync
        with price_cache_lock:
            price_cache[symbol] = t
        return jsonify({"ok": True, **t})
    # get_ticker returned None — try once more with a fresh batch fetch for this symbol
    try:
        r = requests.get(f"{MEXC_BASE}/ticker", params={"symbol": symbol}, timeout=8)
        data = r.json()
        if data.get("success") and data.get("data"):
            d = data["data"]
            if isinstance(d, list): d = d[0]
            t2 = _parse_mexc_ticker(d)
            if t2["price"] > 0:
                # Still fill high/low from Day1 if zero
                if t2["high"] <= 0 or t2["low"] <= 0:
                    try:
                        dc = get_candles(symbol, "Day1", limit=2)
                        if dc:
                            if t2["high"] <= 0: t2["high"] = round(float(dc[-1].get("high",0)),8)
                            if t2["low"]  <= 0: t2["low"]  = round(float(dc[-1].get("low", 0)),8)
                    except: pass
                with price_cache_lock:
                    price_cache[symbol] = t2
                return jsonify({"ok": True, **t2})
    except: pass
