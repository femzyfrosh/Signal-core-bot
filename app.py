...
def place_order(sig):
    ...
    return True, f"Order placed: {sig['symbol']} {sig['direction']} size={size}"
...
<div class="prog"><div class="prog-in">
  <span class="prog-lbl" id="cpair">🚀 Scanner active — fetching pairs...</span>
...
<div class="sec">
  ...
  <div class="prices-grid" id="pgrid"><div class="pc"><div class="pc-sym">BTC/USDT</div><div class="pc-price" style="color:var(--dim)">Loading...</div></div><div class="pc"><div class="pc-sym">ETH/USDT</div><div class="pc-price" style="color:var(--dim)">Loading...</div></div><div class="pc"><div class="pc-sym">SOL/USDT</div><div class="pc-price" style="color:var(--dim)">Loading...</div></div><div class="pc"><div class="pc-sym">BNB/USDT</div><div class="pc-price" style="color:var(--dim)">Loading...</div></div><div class="pc"><div class="pc-sym">XRP/USDT</div><div class="pc-price" style="color:var(--dim)">Loading...</div></div><div class="pc"><div class="pc-sym">DOGE/USDT</div><div class="pc-price" style="color:var(--dim)">Loading...</div></div></div>
</div>
...
async function fetchPrices(){try{const r=await fetch("/api/prices");const data=await r.json();$("pupd").textContent="Updated "+new Date().toLocaleTimeString();const keys=Object.keys(data);if(!keys.length){$("pgrid").innerHTML=TOP.map(sym=>`<div class="pc"><div class="pc-sym">${sym.replace("_USDT","")}/USDT</div><div class="pc-price" style="color:var(--dim)">Loading...</div></div>`).join("");return;}$("pgrid").innerHTML=TOP.map(sym=>{const d=data[sym],name=sym.replace("_USDT","");if(!d)return`<div class="pc"><div class="pc-sym">${name}/USDT</div><div class="pc-price" style="color:var(--dim)">–</div></div>`;const up=d.change>=0;return`<div class="pc ${up?"up":"dn"}"><div class="pc-sym">${name}/USDT</div><div class="pc-price ${up?"up":"dn"}">${fmtP(d.price)}</div><span class="pc-chg ${up?"up":"dn"}">${up?"▲":"▼"} ${Math.abs(d.change).toFixed(2)}%</span></div>`;}).join("");}catch{$("pgrid").innerHTML=TOP.map(sym=>`<div class="pc"><div class="pc-sym">${sym.replace("_USDT","")}/USDT</div><div class="pc-price" style="color:var(--dim)">–</div></div>`).join("");}}
...
async function poll(){tick++;const ps=[fetchSigs(),fetchStats(),fetchState(),fetchPrices()];if(activeTab==="log")ps.push(fetchLog());if(activeTab==="log"&&tick%3===0)ps.push(fetchDiag());if(activeTab==="monitor"&&tick%2===0)ps.push(fetchMonitor());if(activeTab==="trades"&&tick%3===0)ps.push(window.fetchPnl());if(activeTab==="history"&&tick%5===0)ps.push(fetchHistory());if(activeTab==="trade-cfg"&&tick%3===0)ps.push(window.fetchTradeCfgData());if(activeTab==="paper"&&tick%2===0)ps.push(fetchPaperData());await Promise.all(ps);setTimeout(poll,2000);}
