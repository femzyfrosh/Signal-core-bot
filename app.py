...
<div class="app">
  ...
  <div class="topbar">
    ...
    <div class="topbar-right">
      <a class="dash-link" href="/dashboard">Dashboard</a>
    </div>
  </div>
  ...
  <div class="main">
    ...
    <div class="page hidden-right" id="page-chart">
      ...
      <div class="chart-wrap">
        ...
        <div class="empty-chart" id="empty-chart" style="display:none">
...
async function fetchCandles(sym,tf){
  try{
    const r=await fetch(`${MEXC_BASE}/kline/${sym}?interval=${tf}&limit=300`);
    const data=await r.json();
    if(!data.success||!data.data)return[];
    const raw=data.data;
    const times=raw.time||[],opens=raw.open||[],highs=raw.high||[],lows=raw.low||[],closes=raw.close||[];
    const out=[];
    for(let i=0;i<times.length;i++){
      try{
        const t=parseInt(times[i]);
        out.push({time:t>1e12?Math.floor(t/1000):t,open:parseFloat(opens[i]),high:parseFloat(highs[i]),low:parseFloat(lows[i]),close:parseFloat(closes[i])});
      }catch{}
    }
...
function init(){
  ...
  // Price refresh every 7s
  priceTimer=setInterval(fetchPrices,7000);
