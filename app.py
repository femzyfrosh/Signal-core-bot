...
// ── MEXC-style card helpers ─────────────────────────────────────────


// ── MEXC Real-time Price WebSocket Engine ────────────────────────────
var _wsConn   = null;
var _wsSyms   = [];
var _wsTrades = {};
var _wsPing   = null;
...
function _startPriceWS(trades) {
  ...
  // Skip if already connected to same symbols
  ...
  try {
    ...
    ws.onopen = function() {
      syms.forEach(function(sym) {
        ws.send(JSON.stringify({ method: "sub.ticker", param: { symbol: sym.replace("_","") } }));
      });
      // Keepalive ping every 15s
      _wsPing = setInterval(function() {
        if (_wsConn && _wsConn.readyState === WebSocket.OPEN)
          _wsConn.send(JSON.stringify({ method: "ping" }));
        else clearInterval(_wsPing);
      }, 15000);
    };
    ws.onmessage = function(evt) {
      try {
        ...
        if (!price) return;
        // Map MEXC symbol (BTCUSDT) back to our key (BTC_USDT)
        var raw   = d.symbol || "";
        var ourSym = _wsTrades[raw] ? raw : raw.replace(/USDT$/, "_USDT");
    ...
    ws.onclose = function() {
      ...
      // Reconnect after 3s if still have trades
      if (Object.keys(_wsTrades).length)
        setTimeout(function(){ if(Object.keys(_wsTrades).length) _startPriceWS(trades); }, 3000);
    };
    ws.onerror = function() { clearInterval(_wsPing); };
  } catch(e) {}
}

function _stopPriceWS() {
  clearInterval(_wsPing); _wsPing = null;
  if (_wsConn) { try { _wsConn.close(); } catch(e) {} _wsConn = null; }
  _wsTrades = {}; _wsSyms = [];
}

// One immediate REST fetch so prices show before WS handshake
async function _patchPricesREST(trades) {
  await Promise.all(trades.map(async function(t) {
    try {
      ...
      if (!price) return;
      var td = _wsTrades[t.symbol]; if (!td) return;
      var pnl = td.isLong ? (price-td.entry)/td.entry*td.risk*td.lev : (td.entry-price)/td.entry*td.risk*td.lev;
      var roi = td.risk > 0 ? pnl/td.risk*100 : 0;
...
function _oCol(label, value, color, dataSym, cls) {
  return "<div style=\"background:rgba(0,0,0,.2);border-radius:8px;padding:7px 8px\">"
    ...
    + "<div " + (cls ? "class=\"" + cls + "\" " : "") + (dataSym ? "data-sym=\"" + dataSym + "\" " : "") + "style=\"font-size:.72rem;font-weight:700;color:" + color + ";font-family:'JetBrains Mono',monospace\">" + value + "</div>"
  + "</div>";
}

// ── Patch live PnL in-place (no DOM rebuild) ────────────────────────
function _updateOpenCards(pnlMap) {
  document.querySelectorAll(".oc-pnl[data-sym]").forEach(function(el) {
    var sym = el.getAttribute("data-sym");
    var live = pnlMap[sym]; if (!live) return;
    var pv = live.pnl || 0, roi = live.roi_pct || 0;
    el.style.color = pv >= 0 ? "#10b981" : "#ef4444";
    el.innerHTML = (pv>=0?"+":"") + pv.toFixed(3) + " <span style=\"font-size:.78rem\">[" + (roi>=0?"+":"") + roi.toFixed(2) + "%]</span>";
  });
  document.querySelectorAll(".oc-cur[data-sym]").forEach(function(el) {
    var live = pnlMap[el.getAttribute("data-sym")]; if (!live||!live.current) return;
    el.textContent = fmt(live.current);
  });
}

// ── Patch paper PnL in-place ────────────────────────────────────────
function _updatePaperCards(trades) {
  trades.forEach(function(t) {
    var pv = t.pnl||0, roi = t.pnl_pct||0;
    var pnlEl = document.querySelector(".oc-pnl[data-sym=\"" + t.symbol + "\"]");
    if (pnlEl) {
      pnlEl.style.color = pv>=0?"#10b981":"#ef4444";
      pnlEl.innerHTML = (pv>=0?"+":"") + pv.toFixed(3) + " <span style=\"font-size:.78rem\">[" + (roi>=0?"+":"") + roi.toFixed(2) + "%]</span>";
    }
    var curEl = document.querySelector(".oc-cur[data-sym=\"" + t.symbol + "\"]");
...
function _buildOpenCards(trades, wrapId, countId, pnlMap, isLive) {
  ...
  trades.forEach(function(t, i) {
    ...
    var marginRat = live.margin_ratio ? live.margin_ratio.toFixed(2)+"%" : "–";
    ...
    html += "<div style=\"background:var(--s1);border:1.5px solid var(--border);border-radius:14px;padding:14px 15px;margin-bottom:10px\">"
      ...
      + "<div style=\"display:flex;align-items:center;justify-content:space-between;margin-bottom:6px\">"
        + "<span style=\"font-family:'Fredoka One',sans-serif;font-size:1rem;color:var(--text)\">"
          + t.symbol.replace("_USDT","") + "USDT <span style=\"font-size:.65rem;color:var(--dim);font-family:'Nunito',sans-serif;font-weight:400\">Perpetual</span>"
        + "</span>"
      ...
      + "<div style=\"margin-bottom:10px\">"
        ...
        + "</span>"
        + (t.grade?"<span style=\"float:right;font-size:.65rem;font-weight:700;color:#a78bfa;font-family:'Fredoka One',sans-serif\">"+t.grade+"</span>":"")
      + "</div>"
      // Big PnL — has data-sym for live patching
      + "<div style=\"display:flex;align-items:baseline;justify-content:space-between;margin-bottom:10px\">"
        + "<span style=\"font-size:.62rem;color:var(--dim);font-family:'JetBrains Mono',monospace\">Unrealized PnL (USDT)</span>"
        + "<span class=\"oc-pnl\" data-sym=\""+t.symbol+"\" style=\"font-size:1.05rem;font-weight:900;font-family:'Fredoka One',sans-serif;color:"+pnlColor+"\">"
          + (pv>=0?"+":"")+pv.toFixed(3)+" <span style=\"font-size:.78rem\">["+(roi>=0?"+":"")+roi.toFixed(2)+"%]</span>"
        + "</span>"
      + "</div>"
      ...
      + "<div style=\"display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-bottom:6px\">"
        ...
        + _oCol("Margin Ratio", marginRat, "#f59e0b")
      + "</div>"
      // Row 2: Entry Price | Market Price | Liq Price  — market price has data-sym
      + "<div style=\"display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-bottom:10px\">"
        + _oCol("Entry Price", fmt(t.entry), "var(--text)")
        + _oCol("Market Price", cur?fmt(cur):"–", "var(--text)", t.symbol, "oc-cur")
  ...
  // Start WebSocket for real-time PnL updates (both live + paper)
  if (trades.length) { _startPriceWS(trades); _patchPricesREST(trades); }
}
...
function _buildMexcCards(data, wrapId) {
  ...
  data.forEach(function(t, i) {
    ...
    var pvSign    = pv>=0?"+":"";
    var ppSign    = pp>=0?"+":"";
    var statusCol = isWin?"#10b981":"#ef4444";

    html += "<div style=\"background:var(--s1);border:1.5px solid var(--border);border-radius:14px;padding:14px 15px;margin-bottom:10px\">"
      // Header
      + "<div style=\"display:flex;align-items:center;justify-content:space-between;margin-bottom:10px\">"
        + "<span style=\"font-family:'Fredoka One',sans-serif;font-size:.95rem;color:var(--text)\">"
          + t.symbol.replace("_USDT","") + " <span style=\"font-size:.7rem;color:var(--dim)\">Perpetual</span>"
        + "</span>"
      ...
      + "<div style=\"display:flex;gap:6px;margin-bottom:12px\">"
        + "<span style=\"background:"+(isLong?"rgba(16,185,129,.15)":"rgba(239,68,68,.15)")+";border:1px solid "+(isLong?"rgba(16,185,129,.4)":"rgba(239,68,68,.4)")+";color:"+dirColor+";font-size:.65rem;font-weight:800;padding:3px 10px;border-radius:5px;font-family:'JetBrains Mono',monospace\">"+(isLong?"Long":"Short")+"</span>"
        ...
        + "<span style=\"margin-left:auto;background:"+(isWin?"rgba(16,185,129,.1)":"rgba(239,68,68,.1)")+";border:1px solid "+(isWin?"rgba(16,185,129,.3)":"rgba(239,68,68,.3)")+";color:"+statusCol+";font-size:.6rem;font-weight:700;padding:3px 10px;border-radius:5px;font-family:'JetBrains Mono',monospace\">"+(t.status||"CLOSED")+"</span>"
      + "</div>"
      // Data rows
      + _mRow("Entry Price",       fmt(t.entry),                      "var(--text)")
      + _mRow("Close Price",       fmt(t.close_price||t.exit_price||0), "var(--yellow)")
      + _mRow("Realized PnL (USDT)", pvSign+pv.toFixed(4),           pnlColor)
      + _mRow("PnL Rate",          ppSign+pp.toFixed(2)+"%",          pnlColor)
      + _mRow("Stop Loss",         fmt(t.sl),                         "#ef4444")
      + _mRow("Take Profit",       fmt(t.tp),                         "#10b981")
...
window.fetchPnl=async function(){
  var onTrades=activeTab==="trades",onCfg=activeTab==="trade-cfg",onOpen=activeTab==="trade-open";
  if(!onTrades&&!onCfg&&!onOpen)return;
  try{
    var res=await Promise.all([fetch("/api/trades").then(function(r){return r.json();}),fetch("/api/pnl").then(function(r){return r.json();}),fetch("/api/recent-trades").then(function(r){return r.json();})]);
    var tr=res[0],pnl=res[1],hist=res[2];
    var pnlMap={};
    (pnl.positions||[]).forEach(function(p){pnlMap[p.symbol]=p;});
    // If cards already exist, just patch numbers in-place
    var liveWrap=$("live-trades-wrap");
    if(liveWrap&&liveWrap.querySelector(".oc-pnl")&&!onCfg){
      _updateOpenCards(pnlMap);
    } else {
      _buildOpenCards(tr,"live-trades-wrap","trades-count",pnlMap,true);
      _buildOpenCards(tr,"live-trades-wrap2","live-pos-count",pnlMap,true);
    }
    _buildMexcCards(hist,"history-wrap2");
  }catch(e){}
};
async function fetchHistory(){
  if(activeTab!=="history")return;
  try{
    var r=await fetch("/api/recent-trades");
    var data=await r.json();
    _buildMexcCards(data,"history-wrap");
  }catch(e){}
}
...
// Open positions
const ptWrap=$("paper-trades-wrap");if(ptWrap){if(!trades.length){ptWrap.innerHTML='<div class="empty" style="padding:40px"><div class="empty-ico">📝</div><div class="empty-t">No open paper trades</div><div class="empty-s">Enable paper trading and turn on auto-trade to place trades from signals automatically</div></div>';}else{if(ptWrap.querySelector(".oc-pnl")){_updatePaperCards(trades);}else{_buildOpenCards(trades,"paper-trades-wrap",null,null,false);}}}
// History
const phWrap=$("paper-history-wrap");if(phWrap){if(!hist.length){phWrap.innerHTML='<div class="empty" style="padding:30px"><div class="empty-ico">📭</div><div class="empty-t">No paper trades yet</div></div>';}else{_buildMexcCards(hist,"paper-history-wrap");}}
...
// ── News Center ──────────────────────────────────────────────────────────
// All fetching is client-side — Railway blocks server-side outbound HTTP.
// Uses sessionStorage cache so articles appear instantly on tab switch.
let _newsData=[];
let _newsSrc="all";
let _newsLoaded=false;
const _NEWS_CACHE_KEY="sc_news_v4";
const _NEWS_CACHE_TTL=5*60*1000;
...
function _parseRSS(text,srcKey,srcLabel){
  try{
    var doc=new DOMParser().parseFromString(text,"application/xml");
    if(doc.querySelector("parsererror"))return[];
    return Array.from(doc.querySelectorAll("item")).slice(0,12).map(function(el){
      var g=function(t){return(el.querySelector(t)?el.querySelector(t).textContent:"").trim();};
      var link=g("link")||(el.querySelector("guid")?el.querySelector("guid").textContent:"")||"";
      var raw=g("description")||g("summary");
      var desc=raw.replace(/<[^>]*>/g,"").replace(/&[a-z#0-9]+;/gi," ").trim().slice(0,180);
      return{title:g("title"),url:link,source:srcLabel,srcKey:srcKey,published:g("pubDate")||g("date")||"",description:desc};
...
async function _fetchOneFeed(feed){
  for(var i=0;i<_PROXIES.length;i++){
    try{
      var r=await fetch(_PROXIES[i](feed.rss),{signal:AbortSignal.timeout(9000)});
      if(!r.ok)continue;
      var ct=r.headers.get("content-type")||"";
      var xml="";
      if(ct.indexOf("json")>=0){var j=await r.json();xml=j.contents||j.body||"";}
      else{xml=await r.text();}
      if(!xml||xml.length<200)continue;
      var items=_parseRSS(xml,feed.key,feed.label);
...
async function _fetchCryptoCompare(){
  try{
    var r=await fetch("https://min-api.cryptocompare.com/data/v2/news/?lang=EN&sortOrder=latest&limit=30",{
    ...
    return(d.Data||[]).map(function(a){
      return{
        ...
        description:(a.body||"").slice(0,180),
...
async function _fetchCoinGecko(){
  try{
    var r=await fetch("https://api.coingecko.com/api/v3/news?per_page=20",{signal:AbortSignal.timeout(10000)});
    if(!r.ok)return[];
    var raw=await r.json();
    var items=Array.isArray(raw)?raw:(raw.data||[]);
...
function _saveCache(items){try{sessionStorage.setItem(_NEWS_CACHE_KEY,JSON.stringify({t:Date.now(),items:items}));}catch(e){}}
function _loadCache(){
  try{
    var raw=sessionStorage.getItem(_NEWS_CACHE_KEY);if(!raw)return null;
    var o=JSON.parse(raw);
    return(Date.now()-o.t<_NEWS_CACHE_TTL)?o.items:null;
  }catch(e){return null;}
}

function setNewsSrc(src,btn){
  _newsSrc=src;
  document.querySelectorAll(".news-src-btn").forEach(function(b){b.classList.remove("active");});
  if(btn)btn.classList.add("active");
  renderNews();
}

async function loadNews(force){
  var list=$("news-list");var status=$("news-status");
  if(!list)return;
  if(!force){
    var cached=_loadCache();
    if(cached&&cached.length>0){
      _newsData=cached;_newsLoaded=true;
      if(status){status.style.display="block";status.style.color="";
        status.textContent=cached.length+" articles · cached · "+new Date().toLocaleTimeString();}
      renderNews();return;
    }
  }
  list.innerHTML="<div class=\"empty\" style=\"padding:50px 20px\"><div class=\"empty-ico\">📡</div><div class=\"empty-t\">Fetching news…</div><div class=\"empty-s\">Connecting to sources…</div></div>";
  var btn=$("news-refresh-btn");if(btn)btn.style.opacity=".5";

  var results=await Promise.allSettled([
    _fetchCryptoCompare(),
    _fetchCoinGecko(),
    _fetchOneFeed(_NEWS_FEEDS[0]),
    _fetchOneFeed(_NEWS_FEEDS[1]),
    _fetchOneFeed(_NEWS_FEEDS[2]),
    _fetchOneFeed(_NEWS_FEEDS[3]),
    _fetchOneFeed(_NEWS_FEEDS[4]),
  ]);

  var all=[];var seen={};
  results.forEach(function(r){
    if(r.status==="fulfilled")(r.value||[]).forEach(function(a){
      var k=a.title.toLowerCase().slice(0,55);
      if(!seen[k]){seen[k]=1;all.push(a);}
    });
  });
  all.sort(function(a,b){try{return new Date(b.published)-new Date(a.published);}catch(e){return 0;}});
  if(btn)btn.style.opacity="1";
  var srcOk=results.filter(function(r){return r.status==="fulfilled"&&r.value&&r.value.length>0;}).length;
  if(status){
    status.style.display="block";
    if(all.length===0){status.textContent="⚠️ All sources failed — tap Refresh";status.style.color="#ef4444";}
    else{status.style.color="";status.textContent=all.length+" articles · "+srcOk+" sources · "+new Date().toLocaleTimeString();}
  }
  if(all.length>0){_newsData=all;_newsLoaded=true;_saveCache(all);}
  else{_newsLoaded=true;}
  renderNews();
}

function renderNews(){
  var list=$("news-list");if(!list)return;
  var items=_newsData.slice();
  if(_newsSrc&&_newsSrc!=="all")items=items.filter(function(n){return n.srcKey===_newsSrc;});
  if(!items.length){
  ...
  items.forEach(function(n,i){
    var pub=n.published?_nTimeSince(n.published):"";
    var desc=n.description?"<div style=\"font-size:.74rem;color:#64748b;line-height:1.55;margin-top:5px;font-family:Nunito,sans-serif;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden\">"+_nEsc(n.description.trim())+"</div>":"";
    var cats=n.categories?"<span style=\"color:rgba(124,58,237,.6);font-family:JetBrains Mono,monospace;font-size:.55rem\">"+_nEsc(n.categories)+"</span>":"";
    var isNew=n.published&&(Date.now()-new Date(n.published))<30*60*1000;
    var badge=(i===0||isNew)?"<span style=\"background:rgba(56,189,248,.12);border:1px solid rgba(56,189,248,.35);color:#38bdf8;font-size:.55rem;font-weight:700;padding:2px 7px;border-radius:5px;letter-spacing:.05em;margin-bottom:5px;display:inline-block\">BREAKING</span><br>":"";
    var hash=0;for(var ci=0;ci<n.title.length;ci++)hash+=n.title.charCodeAt(ci);
    var up=hash%3!==0;
    var ac=up?"#10b981":"#ef4444";
    var arrow=up
      ?"<svg width=\"14\" height=\"14\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\""+ac+"\" stroke-width=\"2.5\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><polyline points=\"23 6 13.5 15.5 8.5 10.5 1 18\"/><polyline points=\"17 6 23 6 23 12\"/></svg>"
      :"<svg width=\"14\" height=\"14\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\""+ac+"\" stroke-width=\"2.5\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><polyline points=\"23 18 13.5 8.5 8.5 13.5 1 6\"/><polyline points=\"17 18 23 18 23 12\"/></svg>";
    var bc=i===0?"#38bdf8":up?"rgba(16,185,129,.6)":"rgba(239,68,68,.4)";
    html+="<a href=\""+(n.url||"#")+"\" target=\"_blank\" rel=\"noopener noreferrer\""
      +" style=\"display:block;background:var(--s1);border:1.5px solid var(--border);border-left:3px solid "+bc+";border-radius:14px;padding:13px 14px;text-decoration:none;transition:background .18s\""
      ...
      +"<div style=\"display:flex;align-items:flex-start;justify-content:space-between;gap:8px\">"
        +"<div style=\"flex:1;min-width:0\">"+badge
          +"<div style=\"font-family:Nunito,sans-serif;font-size:.87rem;font-weight:800;color:var(--text);line-height:1.42\">"+_nEsc(n.title)+"</div>"+desc
        +"</div>"
        +"<div style=\"flex-shrink:0;padding-top:2px\">"+arrow+"</div>"
      +"</div>"
      +"<div style=\"display:flex;align-items:center;gap:7px;flex-wrap:wrap;margin-top:8px\">"
        +"<span style=\"background:rgba(124,58,237,.1);border:1px solid rgba(124,58,237,.2);border-radius:20px;padding:2px 9px;font-size:.58rem;font-family:JetBrains Mono,monospace;color:#a78bfa;display:inline-flex;align-items:center;gap:4px\">"
          +"<span style=\"width:5px;height:5px;border-radius:50%;background:#a78bfa;display:inline-block\"></span>"+_nEsc(n.source)
        +"</span>"+cats+(pub?"<span style=\"font-family:JetBrains Mono,monospace;font-size:.58rem;color:var(--dim);margin-left:auto\">"+pub+"</span>":"")
      +"</div></a>";
  });
  list.innerHTML=html;
}

function _nTimeSince(d){
  try{
