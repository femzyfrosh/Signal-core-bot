...
MEXC_SYMBOL_MAP = {
    ...
    "BONK_USDT":     "BONK_USDT",    # BONK -- try as-is first
    "FARTCOIN_USDT": "FARTCOIN_USDT",
    "RIVER_USDT":    "RIVER_USDT",
}
...
def _parse_mexc_ticker(d):
    ...
    Uses explicit None-safe extraction so legitimate 0.0 values are never
    discarded by Python truthiness -- fixes missing price/high/low/change on
    GME, BONK, MAT and any pair where a field happens to be exactly 0.
    """
    def _first(*keys):
        """Return first non-None, non-empty value across given dict keys, as float."""
        for k in keys:
            v = d.get(k)
    ...
    price  = _first("lastPrice", "last", "fairPrice", "indexPrice", "price")
    high   = _first("high24h",   "highPrice",  "highest",  "high")
    low    = _first("low24h",    "lowPrice",   "lowest",   "low")

    # riseFallRate is a decimal fraction on MEXC (e.g. -0.0026 = -0.26%).
    # IMPORTANT: do NOT skip when == 0 -- flat market IS legitimately 0% change.
    rfr = d.get("riseFallRate")
    if rfr is not None and rfr != "":
        try:
            change = float(rfr) * 100
        except (TypeError, ValueError):
            change = 0.0
    else:
        raw = _first("priceChangePercent", "changeRate", "rate", "change24h", "riseFallValue")
        change = raw if abs(raw) > 2 else raw * 100

    # Last resort: derive % change from 24h open price
    if change == 0.0 and price > 0:
        open24 = _first("open24h", "openPrice", "open")
        if open24 > 0:
            change = round((price - open24) / open24 * 100, 4)
...
def get_ticker(symbol):
    """Fetch a single live ticker from MEXC futures. Used by the scanner."""
    mexc_sym = MEXC_SYMBOL_MAP.get(symbol, symbol)
    for attempt in ([mexc_sym] if mexc_sym == symbol else [mexc_sym, symbol]):
        try:
            r = requests.get(f"{MEXC_BASE}/ticker", params={"symbol": attempt}, timeout=6)
            data = r.json()
...
def _fetch_all_tickers_batch():
    ...
    try:
        ...
        new_entries = {}
        for d in items:
            mexc_sym = d.get("symbol", "")
            # Match our internal name — direct or via reverse alias map
            our_sym = mexc_sym if mexc_sym in top_set else MEXC_SYMBOL_REVERSE.get(mexc_sym)
            if not our_sym:
                continue
            try:
                t = _parse_mexc_ticker(d)
                if t["price"] > 0:
                    new_entries[our_sym] = t
            except:
                pass
        if new_entries:
            with price_cache_lock:
                price_cache.update(new_entries)
        # Individual fallback: fetch any pairs still missing after batch
        missing = [s for s in top_set if s not in new_entries]
        for sym in missing:
            # Build ordered list of MEXC names to try: alts, mapped name, raw name
            attempts = list(MEXC_SYMBOL_ALTS.get(sym, []))
            mexc_name = MEXC_SYMBOL_MAP.get(sym, sym)
            for cand in [mexc_name, sym]:
            ...
            for attempt in attempts:
                if got:
                    break
                try:
                    r2 = requests.get(f"{MEXC_BASE}/ticker", params={"symbol": attempt}, timeout=6)
                    d2 = r2.json()
...
.tabs{display:flex;flex-direction:column;gap:6px;background:var(--s1);border:2px solid var(--border);border-radius:16px;padding:6px;margin-bottom:18px}
.tabs-row{display:grid;gap:5px}
.tabs-row.row1{grid-template-columns:repeat(5,1fr)}
.tabs-row.row2{grid-template-columns:repeat(4,1fr)}
.tab{flex:none;width:100%;padding:9px 4px;border:none;border-radius:10px;font-family:'Nunito',sans-serif;font-size:.7rem;font-weight:800;cursor:pointer;transition:all .2s;color:var(--dim);background:transparent;white-space:nowrap;text-align:center;overflow:hidden;text-overflow:ellipsis}
...
.toast{position:fixed;bottom:90px;left:50%;transform:translateX(-50%) translateY(100px);background:var(--s2);border:2px solid var(--border2);border-radius:16px;padding:12px 22px;font-family:'Nunito',sans-serif;font-size:.85rem;font-weight:800;box-shadow:0 18px 50px rgba(0,0,0,.6);opacity:0;transition:all .4s cubic-bezier(.34,1.56,.64,1);pointer-events:none;z-index:9999;white-space:nowrap}
...
/* ── BOTTOM NAV ── */
.bnav{position:fixed;bottom:0;left:0;right:0;height:72px;background:rgba(12,11,24,.97);border-top:1.5px solid var(--border);backdrop-filter:blur(24px);display:flex;align-items:center;justify-content:center;gap:0;z-index:500;padding:0 16px}
.bnav-inner{display:flex;align-items:center;gap:6px;background:rgba(255,255,255,.03);border:1.5px solid rgba(255,255,255,.06);border-radius:20px;padding:6px 8px}
.bnav-btn{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;padding:8px 20px;border:none;border-radius:14px;background:transparent;cursor:pointer;transition:all .22s;min-width:68px;color:var(--dim);position:relative}
.bnav-btn:hover{color:var(--text);background:rgba(255,255,255,.04)}
.bnav-btn.active{background:linear-gradient(135deg,rgba(124,58,237,.25),rgba(219,39,119,.15));color:#a78bfa;border:1.5px solid rgba(124,58,237,.3)}
.bnav-btn .bico{font-size:1.15rem;line-height:1;transition:transform .22s}
.bnav-btn:hover .bico,.bnav-btn.active .bico{transform:translateY(-2px)}
.bnav-btn .blbl{font-family:'Nunito',sans-serif;font-size:.6rem;font-weight:800;letter-spacing:.04em;line-height:1}
/* ── PAGES ── */
.page{display:none}.page.active{display:block}
/* ── TRADE PAGE 2-COL ── */
.trade-split{display:grid;grid-template-columns:1fr 1fr;gap:14px}
@media(max-width:900px){.trade-split{grid-template-columns:1fr}}
...
<div id="tab-signals">
  ...
  <div id="page-trade" class="page">
    <div class="trade-split">
      <div>
        <div class="sec-hdr" style="margin-bottom:10px"><span class="sec-ttl">🤖 Auto Trade</span><div class="sec-line"></div></div>
<div id="tab-trade-cfg-inner">
    <div class="panel">
      ...
      <div class="trade-actions">
        ...
        <button class="trade-btn tb-off" id="t-disable-btn" onclick="enableTrade(false)">⏹ Disable</button>
      </div>
    ...
    <div class="panel">
      ...
      <div>
        <div class="sec-hdr" style="margin-bottom:10px"><span class="sec-ttl">📝 Paper Trade</span><div class="sec-line"></div></div>
<div id="tab-paper-inner">
    <div class="panel">
      ...
      <div class="trade-actions">
        ...
        <button class="trade-btn tb-off" id="p-disable-btn" onclick="enablePaper(false)">⏹ Disable Paper</button>
    ...
    <!-- History at bottom of Trade page -->
    <div style="margin-top:14px">
      <div class="sec-hdr" style="margin-bottom:10px"><span class="sec-ttl">📜 Trade History</span><div class="sec-line"></div></div>
<div id="tab-history-inner">
    <div class="panel">
...
<div id="tab-settings-inner">
    ...
    <!-- Logs section inside Settings -->
    <div style="margin-top:14px">
      <div class="sec-hdr" style="margin-bottom:10px"><span class="sec-ttl">🖥️ Logs</span><div class="sec-line"></div></div>
<div id="tab-log-inner">
  ...
  <div id="page-market" class="page">
    <div style="border-radius:16px;overflow:hidden;height:82vh;border:2px solid var(--border)">
      <iframe id="market-iframe" src="" style="width:100%;height:100%;border:none;background:var(--s1)" loading="lazy"></iframe>
    </div>
  </div><!-- /page-market -->
  ...
  <div id="page-signals" class="page">
    <div class="sec-hdr" style="margin-bottom:10px"><span class="sec-ttl">🎯 Signals</span><div class="sec-line"></div><button class="action-btn" onclick="discardAllSigs()" style="font-size:.65rem;padding:5px 12px;background:rgba(239,68,68,.08);border-color:rgba(239,68,68,.25);color:var(--red)">🗑 Discard All</button></div>
    <div id="sig-list-page" class="sig-list"></div>
    <!-- Monitor panels -->
    <div style="margin-top:20px">
      <div class="sec-hdr" style="margin-bottom:10px"><span class="sec-ttl">👁 Monitor</span><div class="sec-line"></div></div>
<div id="tab-monitor-inner">
    <div class="panel">
...
<nav class="bnav">
  <div class="bnav-inner">
    <button class="bnav-btn active" data-page="home" onclick="sw('home')">
      <span class="bico">🏠</span><span class="blbl">Home</span>
    </button>
    <button class="bnav-btn" data-page="trade" onclick="sw('trade')">
      <span class="bico">💹</span><span class="blbl">Trade</span>
    </button>
    <button class="bnav-btn" data-page="settings" onclick="sw('settings')">
      <span class="bico">⚙️</span><span class="blbl">Settings</span>
    </button>
    <button class="bnav-btn" data-page="market" onclick="sw('market')">
      <span class="bico">📈</span><span class="blbl">Market</span>
    </button>
    <button class="bnav-btn" data-page="signals" onclick="sw('signals')">
      <span class="bico">🎯</span><span class="blbl">Signals</span>
    </button>
  </div>
</nav>
...
(function(){
'use strict';
let allSigs=[],toastT,activePage='home',activeTab='home',tick=0,lastCount=0,paperAutoOn=false;
...
// ── DISCARD SIGNALS ──────────────────────────────────────────────────
let discardedSigs=new Set();
window.discardSig=function(id){discardedSigs.add(id);renderSigs();renderSigsPage();};
window.discardAllSigs=function(){allSigs.forEach(s=>discardedSigs.add(s.id||s.timestamp));renderSigs();renderSigsPage();toast('🗑 All signals cleared','sell');};
function renderSigsPage(){
  const list=$('sig-list-page');if(!list)return;
  const f=allSigs.filter(s=>!discardedSigs.has(s.id||s.timestamp));
  if(!f.length){list.innerHTML='<div class="empty"><div class="empty-ico">🔭</div><div class="empty-t">No signals yet</div><div class="empty-s">Signals appear here when the scanner finds setups.</div></div>';return;}
  list.innerHTML=f.slice(0,100).map((s,i)=>{
    const id=s.id||s.timestamp||i;
    const card=buildCard(s,1000+i);
    return card.replace('</div>','<button class="action-btn close-btn" style="position:absolute;top:10px;right:12px;padding:3px 9px;font-size:.65rem;z-index:2" onclick="discardSig(\''+id+'\')">🗑 Discard</button></div>');
  }).join('');
}const b=$("det-"+i);if(!b)return;b.classList.toggle("open");const t=b.previousElementSibling;if(t)t.textContent=b.classList.contains("open")?"▼ Score Breakdown":"▶ Score Breakdown";};
...
window.sw=function(page){
  ...
  const nb=document.querySelector(`.bnav-btn[data-page="${page}"]`);
  if(nb)nb.classList.add('active');
  // Show/hide pages
  ['home','trade','settings','market','signals'].forEach(p=>{
    const el=$('page-'+p);
    if(el)el.classList.toggle('active',p===page);
  });
  // Side effects
  if(page==='market'){const fr=$('market-iframe');if(fr&&!fr.getAttribute('src'))fr.src='/market';}
  if(page==='settings'){fetchLog();fetchDiag();loadSettings();}
  if(page==='trade'){loadTradeConfig();syncModelToggles();window.fetchPnl();loadPaperConfig();fetchPaperData();fetchHistory();}
  if(page==='signals'){renderSigsPage();fetchMonitor();}
};
...
async function poll(){tick++;const ps=[fetchSigs(),fetchStats(),fetchState()];if(tick%(priceRefreshSecs)===0)ps.push(fetchPrices());if(tick%3===0)ps.push(fetchPnlTicker());if(activePage==="settings")ps.push(fetchLog());if(activePage==="settings"&&tick%3===0)ps.push(fetchDiag());if(activePage==="signals"&&tick%2===0)ps.push(fetchMonitor());if(activePage==="trade"&&tick%3===0)ps.push(window.fetchPnl());if(activePage==="trade"&&tick%5===0)ps.push(fetchHistory());if(activePage==="trade"&&tick%2===0)ps.push(fetchPaperData());await Promise.all(ps);setTimeout(poll,1000);}
...
def api_debug_prices():
    """Debug endpoint: shows which pairs have prices and which are missing."""
    with price_cache_lock:
    ...
    for sym in TOP_PAIRS:
        entry = cache.get(sym)
        result[sym] = {
            "has_price": entry is not None and entry.get("price", 0) > 0,
            "data": entry,
            "mexc_name": MEXC_SYMBOL_MAP.get(sym, sym),
        }
...
.ss-popup{position:fixed;z-index:125;background:var(--card);border:1px solid var(--border);border-radius:14px;padding:14px;display:none;flex-direction:column;gap:10px;box-shadow:0 8px 36px rgba(0,0,0,.6);backdrop-filter:blur(16px);min-width:220px}
  ...
  <div class="main">
    ...
    <div class="page hidden-right" id="page-chart">
      ...
      <div class="ss-popup" id="ss-popup">
        ...
        <div id="ss-border-row"><div class="ss-title" style="margin-bottom:4px">Border colour</div>
          <div class="ss-palette" id="ss-border-palette"></div></div>
        <div id="ss-fill-row"><div class="ss-title" style="margin-bottom:4px">Fill colour</div>
          <div class="ss-palette" id="ss-fill-palette"></div></div>
        ...
        <div class="ss-btns">
          ...
          <button class="ss-apply" style="background:var(--card);border:1px solid var(--border);color:var(--dim)" onclick="closeShapeSettings()">✕</button>
        </div>
      </div>
...
function updateCountdown(){
  ...
  if(el){el.style.color=remaining<=10?"var(--green)":"#ffc93c";}
}
...
// ══════════════════════════════════════════════════════════════════
// ── TradingView-style Drawing Engine ─────────────────────────────
// ══════════════════════════════════════════════════════════════════

let drawTool=null;     // 'rect' | 'trend' | 'select' | null
let shapes=[];         // [{type,t1,p1,t2,p2,borderColor,fillColor,midLine,color}]
let selectedIdx=-1;
...
window.applyShapeSettings=function(){
  ...
  if(sh.type==='rect'){if(b)sh.borderColor=b.dataset.color;if(f)sh.fillColor=f.dataset.color;sh.midLine=document.getElementById('ss-midline').checked;}
  else{if(b)sh.color=b.dataset.color;}
...
window.deleteSelectedShape=function(){
  ...
  selectedIdx=-1;document.getElementById('ss-popup').style.display='none';redrawCanvas();
};
...
function openShapeSettings(idx){
  ...
  const sh=shapes[idx];
  document.getElementById('ss-popup-title').textContent=sh.type==='rect'?'Rectangle Settings':'Trend Line Settings';
  buildPalette('ss-border-palette',sh.borderColor||sh.color||'#4f6ef7');
  const fr=document.getElementById('ss-fill-row'),mr=document.getElementById('ss-midline-row');
  if(sh.type==='rect'){
    fr.style.display='block';mr.style.display='flex';
    buildPalette('ss-fill-palette',sh.fillColor||'#4f6ef7');
    document.getElementById('ss-midline').checked=!!sh.midLine;
  }else{fr.style.display='none';mr.style.display='none';}
...
function _drawShape(ctx,sh,isSel,isPrev){
  if(sh.type==='rect'){
    const x1=_timeToX(sh.t1),y1=_priceToY(sh.p1),x2=_timeToX(sh.t2),y2=_priceToY(sh.p2);
    if(x1==null||x2==null||y1==null||y2==null)return;
    const lx=Math.min(x1,x2),rx=Math.max(x1,x2),ty=Math.min(y1,y2),by=Math.max(y1,y2),w=rx-lx,h=by-ty;
    ctx.save();
    ctx.fillStyle=isPrev?'rgba(79,110,247,.08)':_hexA(sh.fillColor||'#4f6ef7',.12);
    ctx.fillRect(lx,ty,w,h);
    ctx.strokeStyle=isPrev?'rgba(79,110,247,.7)':(sh.borderColor||'#4f6ef7');
    ctx.lineWidth=isSel?2:1.5;ctx.setLineDash(isPrev?[5,3]:[]);
    ctx.strokeRect(lx,ty,w,h);ctx.setLineDash([]);
    if(sh.midLine&&!isPrev){
      ctx.beginPath();ctx.strokeStyle=sh.borderColor||'#4f6ef7';
      ctx.lineWidth=1;ctx.setLineDash([5,3]);
      ctx.moveTo(lx,ty+h/2);ctx.lineTo(rx,ty+h/2);ctx.stroke();ctx.setLineDash([]);
    }
    if(isSel&&!isPrev){
      ctx.fillStyle=sh.borderColor||'#4f6ef7';
      [[lx,ty],[rx,ty],[lx,by],[rx,by]].forEach(([hx,hy])=>{ctx.beginPath();ctx.arc(hx,hy,5,0,Math.PI*2);ctx.fill();});
    }
    ctx.restore();
    ...
    if(x1==null||x2==null||y1==null||y2==null)return;
    ctx.save();
    ctx.strokeStyle=isPrev?'rgba(255,201,60,.7)':(sh.color||'#ffc93c');
    ctx.lineWidth=isSel?2.5:1.8;ctx.setLineDash(isPrev?[6,3]:[]);
    ctx.beginPath();ctx.moveTo(x1,y1);ctx.lineTo(x2,y2);ctx.stroke();ctx.setLineDash([]);
    if(isSel&&!isPrev){
      ctx.fillStyle=sh.color||'#ffc93c';
      [[x1,y1],[x2,y2]].forEach(([hx,hy])=>{ctx.beginPath();ctx.arc(hx,hy,5,0,Math.PI*2);ctx.fill();});
    }
    ctx.restore();
  }
}
...
// ── Hit test ──────────────────────────────────────────────────────
function _hitTest(sh,cx,cy){
  const T=14;
  if(sh.type==='rect'){
    const x1=_timeToX(sh.t1),y1=_priceToY(sh.p1),x2=_timeToX(sh.t2),y2=_priceToY(sh.p2);
    if(x1==null||x2==null||y1==null||y2==null)return false;
    return cx>=Math.min(x1,x2)-T&&cx<=Math.max(x1,x2)+T&&cy>=Math.min(y1,y2)-T&&cy<=Math.max(y1,y2)+T;
  }else if(sh.type==='trend'){
    const x1=_timeToX(sh.t1),y1=_priceToY(sh.p1),x2=_timeToX(sh.t2),y2=_priceToY(sh.p2);
    if(x1==null||x2==null||y1==null||y2==null)return false;
    const dx=x2-x1,dy=y2-y1,len2=dx*dx+dy*dy;
    if(len2===0)return Math.hypot(cx-x1,cy-y1)<T;
    const t=Math.max(0,Math.min(1,((cx-x1)*dx+(cy-y1)*dy)/len2));
    return Math.hypot(cx-x1-t*dx,cy-y1-t*dy)<T;
  }
  return false;
}

function findShapeAt(cx,cy){
  for(let i=shapes.length-1;i>=0;i--)if(_hitTest(shapes[i],cx,cy))return i;
  return -1;
}
...
function initDrawCanvas(){
  ...
  let drawStart=null;    // {time,price} at pointer-down for draw mode
  let moveSnap=null;     // snapshot of shapes[] at move-start
  let moveMode=false;
  ...
  function onDown(e){
    const{cx,cy}=_cp(e);
    startCX=cx;startCY=cy;pDown=true;moveMode=false;drawStart=null;
    if(drawTool==='rect'||drawTool==='trend'){
      const c=_screenToChart(canvas,cx,cy);
      if(c){drawStart=c;}
      if(e.cancelable)e.preventDefault();
    }else{
      // select mode or no tool: check hit
      const bb=canvas.getBoundingClientRect();
      const hit=findShapeAt(cx-bb.left,cy-bb.top);
      if(hit>=0){
        selectedIdx=hit;moveMode=true;
        moveSnap=shapes.map(s=>({...s}));
        if(e.cancelable)e.preventDefault();
      }else{selectedIdx=-1;}
      redrawCanvas();
    }
  ...
  function onMove(e){
    if(!pDown)return;
    const{cx,cy}=_cp(e);
    if(Math.hypot(cx-startCX,cy-startCY)<4)return;
    if(drawTool==='rect'||drawTool==='trend'){
      if(!drawStart)return;
      if(e.cancelable)e.preventDefault();
      const c=_screenToChart(canvas,cx,cy);
      ...
      previewShape=drawTool==='rect'
        ?{type:'rect',t1:drawStart.time,p1:drawStart.price,t2:c.time,p2:c.price,borderColor:'#4f6ef7',fillColor:'#4f6ef7',midLine:false}
        :{type:'trend',t1:drawStart.time,p1:drawStart.price,t2:c.time,p2:c.price,color:'#ffc93c'};
      redrawCanvas();
    }else if(moveMode&&selectedIdx>=0){
      if(e.cancelable)e.preventDefault();
      const dx=cx-startCX,dy=cy-startCY;
      const orig=moveSnap[selectedIdx];
      const bb=canvas.getBoundingClientRect();
      const ox1=_timeToX(orig.t1),oy1=_priceToY(orig.p1);
      if(ox1==null||oy1==null)return;
      const nc1=_screenToChart(canvas,ox1+bb.left+dx,oy1+bb.top+dy);
      if(!nc1)return;
      const sh=shapes[selectedIdx];
      ...
      if(orig.t2!=null){
        ...
        const nc2=_screenToChart(canvas,ox2+bb.left+dx,oy2+bb.top+dy);
        if(!nc2)return;
  ...
  function onUp(e){
    ...
    if((drawTool==='rect'||drawTool==='trend')&&previewShape&&moved){
      shapes.push({...previewShape});selectedIdx=shapes.length-1;
      previewShape=null;drawStart=null;
      // Single-shot: exit draw mode automatically after drawing
      drawTool=null;_setActiveToolUI(null);
      redrawCanvas();return;
    }
    previewShape=null;drawStart=null;
    if(moveMode){moveMode=false;moveSnap=null;redrawCanvas();return;}
    // Tap: single=select, double=settings
    if(!moved){
      const bb=canvas.getBoundingClientRect();
      const hit=findShapeAt(cx-bb.left,cy-bb.top);
      const now=Date.now();
      if(hit>=0&&hit===lastTapHit&&now-lastTapMs<400){
        lastTapMs=0;lastTapHit=-1;openShapeSettings(hit);
      }else{
        selectedIdx=hit>=0?hit:-1;lastTapMs=now;lastTapHit=hit;redrawCanvas();
      }
    }
  }
