...
<div class="tab-wrap">
  <div class="tabs">
    ...
    <div class="tabs-row row2">
      ...
      <button class="tab" onclick="sw('paper',this)">📝 Paper</button>
  ...
  <div id="tab-market" style="display:none">
    <div style="border-radius:16px;overflow:hidden;height:80vh;border:2px solid var(--border)">
...
window.sw=function(tab,btn){activeTab=tab;document.querySelectorAll(".tab").forEach(b=>b.classList.remove("active"));btn.classList.add("active");["signals","market","trades","monitor","history","trade-cfg","paper","log","settings"].forEach(t=>{const el=$("tab-"+t);if(el)el.style.display=t===tab?"block":"none";});if(tab==="market"){const fr=$("market-iframe");if(fr&&!fr.getAttribute("src"))fr.src="/market";}if(tab==="log"){fetchLog();fetchDiag();}if(tab==="trades")window.fetchPnl();if(tab==="monitor")fetchMonitor();if(tab==="history")fetchHistory();if(tab==="trade-cfg"){loadTradeConfig();syncModelToggles();window.fetchPnl();}if(tab==="paper"){loadPaperConfig();fetchPaperData();}if(tab==="settings")loadSettings();};
