...
function initDrawCanvas(){
  ...
  // ── KEY DESIGN: canvas is ALWAYS pointer-events:none ─────────
  // We attach ALL pointer listeners to the chart-wrap div instead.
  // The chart underneath handles its own pan/zoom natively.
  // We only intercept events when a draw tool is active OR the
  // pointer is directly over an existing shape.
  ...
  // ── mousemove on wrap: cursor only, never preventDefault ─────
  wrap.addEventListener('mousemove',function(e){
    if(!pDown) _updateCursor(e.clientX,e.clientY);
  });

  // ── onDown: only intercept if draw tool active OR over shape ──
  function onDown(e){
    const{cx,cy}=_cp(e);
    const overShape=_overShape(cx,cy);
    const isDrawing=drawTool==='rect'||drawTool==='trend';

    // If neither drawing nor over a shape → let chart handle it
    if(!isDrawing&&!overShape) return;
  ...
  // ── onMove: only runs when we own the pointer ─────────────────
  function onMove(e){
    if(!pDown)return;
    const{cx,cy}=_cp(e);
  ...
  // Attach to WRAP not canvas — chart gets all events it doesn't see
  ...
  // Redraw shapes when chart scrolls/zooms (keeps shapes locked to price/time)
  if(chart){
