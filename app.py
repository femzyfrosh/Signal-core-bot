...
// ── News Center ─────────────────────────────────────────────────────────
// All fetching is client-side (Railway blocks all server-side outbound HTTP).
// Direct fetch for APIs with real CORS headers; proxy fallback chain for RSS.
// sessionStorage cache so articles survive tab switches instantly.
let _newsData=[];
let _newsSrc=\'all\';
let _newsLoaded=false;
const _NEWS_CACHE_KEY=\'sc_news_v2\';
...
// Try proxies in order; first success wins per feed
const _PROXIES=[
...
function _loadCache(){
  try{
    ...
    return(Date.now()-t<_NEWS_CACHE_TTL)?items:null;
  }catch{return null;}
}
...
async function loadNews(force=false){
  ...
  if(!list)return;
  // Show cached instantly, skip fetch if cache fresh and not forced
  if(!force){
  ...
  const srcOk=results.filter(r=>r.status===\'fulfilled\'&&r.value?.length>0).length;
  if(status){
    status.style.display=\'block\';
    if(all.length===0){status.textContent=\'⚠️ All sources failed — tap Refresh to retry\';status.style.color=\'#ef4444\';}
...
function renderNews(){
  ...
  list.innerHTML=items.map((n,i)=>{
    ...
    const badge=(i===0||isNew)?`<span style="background:rgba(56,189,248,.12);border:1px solid rgba(56,189,248,.35);color:#38bdf8;font-family:\'JetBrains Mono\',monospace;font-size:.55rem;font-weight:700;padding:2px 7px;border-radius:5px;letter-spacing:.05em;margin-bottom:5px;display:inline-block">BREAKING</span><br>`:\'\';
    const hash=n.title.split(\'\').reduce((a,c)=>a+c.charCodeAt(0),0);
    const up=hash%3!==0;const ac=up?\'#10b981\':\'#ef4444\';
    const arrowSvg=up
    ...
    const bc=i===0?\'#38bdf8\':up?\'rgba(16,185,129,.6)\':\'rgba(239,68,68,.4)\';
    return`<a href="${n.url||\'#\'}" target="_blank" rel="noopener noreferrer" style="display:block;background:var(--s1);border:1.5px solid var(--border);border-left:3px solid ${bc};border-radius:14px;padding:13px 14px;text-decoration:none;transition:background .18s" onmouseover="this.style.background=\'var(--s2)\';" onmouseout="this.style.background=\'var(--s1)\';">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
        <div style="flex:1;min-width:0">${badge}<div style="font-family:\'Nunito\',sans-serif;font-size:.87rem;font-weight:800;color:var(--text);line-height:1.42">${_esc(n.title)}</div>${desc}</div>
      ...
      <div style="display:flex;align-items:center;gap:7px;flex-wrap:wrap;margin-top:8px">
        <span style="background:rgba(124,58,237,.1);border:1px solid rgba(124,58,237,.2);border-radius:20px;padding:2px 9px;font-size:.58rem;font-family:\'JetBrains Mono\',monospace;color:#a78bfa;display:inline-flex;align-items:center;gap:4px"><span style="width:5px;height:5px;border-radius:50%;background:#a78bfa;display:inline-block"></span>${_esc(n.source)}</span>
