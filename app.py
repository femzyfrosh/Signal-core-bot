...
SETTINGS_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "settings_saved.json")
...
MEXC_FUTURES_PUBLIC = "https://contract.mexc.com/api/v1/contract"
...
# ── MODEL #2 MONITOR ─────────────────────────────────────────────────
# Phase 1: Sharp sweep spotted → monitor pair for FVG formation
# Phase 2: FVG formed → monitor until price taps the FVG → instant market order
m2_monitor  = {}   # symbol → {phase:1|2, fvg_top, fvg_bot, direction, ...}
...
def _find_first_touch_extreme(candles, zone_top, zone_bot, direction, lookback=60):
    for c in candles[-lookback:]:
        if direction == "SELL" and c["high"] >= zone_bot and c["low"] < zone_top:
            return c["high"]
        if direction == "BUY"  and c["low"] <= zone_top and c["high"] > zone_bot:
            return c["low"]
    return None

def _sweep_and_fvg(candles, first_extreme, direction, lookback=60):
    """Find sweep of first_extreme then first FVG after it showing displacement."""
    sweep_idx = None
    for i, c in enumerate(candles):
        if sweep_idx is None:
            if direction == "SELL" and c["high"] > first_extreme:
                sweep_idx = i; sweep_extreme = c["high"]
            elif direction == "BUY" and c["low"] < first_extreme:
                sweep_idx = i; sweep_extreme = c["low"]
        else:
            # After sweep, look for FVG
            if i < sweep_idx + 2: continue
            c1 = candles[i-2]; c3 = c
            if direction == "SELL" and c3["high"] < c1["low"]:
                fvg_top = c1["low"]; fvg_bot = c3["high"]
                return sweep_extreme, fvg_top, fvg_bot, fvg_bot   # tip = bot for SELL
            if direction == "BUY"  and c3["low"] > c1["high"]:
                fvg_top = c3["low"]; fvg_bot = c1["high"]
                return sweep_extreme, fvg_top, fvg_bot, fvg_top   # tip = top for BUY
    return None, None, None, None

def _swing_before_touch(candles, direction, lookback=80):
    """Liquidity pool on the other side of the range (TP2 target)."""
    c = candles[-lookback:]
    if direction == "SELL": return min(x["low"]  for x in c)
    else:                   return max(x["high"] for x in c)

def scan_pair_model2(symbol):
    results = []
    ref = get_candles(symbol, "Hour4", limit=200)
    if not ref or len(ref) < 30: return results
    trend, sh, sl = detect_trend(ref)
    ...
    for htf in ["Hour4", "Min60"]:
        ...
        for kl in kls[:8]:
            ...
            for ltf in M2_LTF_ORDER:
                ltf_c = get_candles(symbol, ltf, limit=200)
                if not ltf_c or len(ltf_c) < 20: continue

                first_extreme = _find_first_touch_extreme(ltf_c, zone_top, zone_bot, direction)
                if first_extreme is None: continue

                sweep_extreme, fvg_top, fvg_bot, fvg_tip = _sweep_and_fvg(ltf_c, first_extreme, direction)
                if sweep_extreme is None: continue

                entry = round(fvg_tip, 8)
                buf   = (zone_top - zone_bot) * 0.05
                sl_p  = round(sweep_extreme + buf, 8) if direction == "SELL" else round(sweep_extreme - buf, 8)
                liq   = _swing_before_touch(ltf_c, direction)
                if liq is None: continue

                if direction == "SELL":
                    range_sz = abs(sweep_extreme - liq)
                    tp1 = round(sweep_extreme - range_sz * 0.50, 8)
                    tp2 = round(liq, 8)
                    sl_trail = tp1
                else:
                    range_sz = abs(liq - sweep_extreme)
                    tp1 = round(sweep_extreme + range_sz * 0.50, 8)
                    tp2 = round(liq, 8)
                    sl_trail = tp1

                if range_sz <= 0: continue
                risk   = abs(entry - sl_p)
                if risk <= 0: continue
                ...
                score = 70  # base: HTF KL + sweep + FVG all confirmed
                details = ["✅ HTF Key Level tap (+20)", "✅ First-touch sweep (+20)", f"✅ Displacement FVG on {ltf} (+20)"]
                if in_pd:   score += 10; details.append(f"✅ {pd_name} (+10)")
                if rr >= 4: score += 10; details.append(f"✅ {rr}R (+10)")
                elif rr>=3: score += 7;  details.append(f"⚠️ {rr}R (+7)")
                else:       score += 4;  details.append(f"⚠️ {rr}R (+4)")
                score = min(score, 100)
                grade = "A+" if score>=75 else "A" if score>=60 else "B" if score>=45 else "C"

                results.append({
                    "model":"2","symbol":symbol,"tf":htf,"ob_tf":ltf,
                    "ob_zone":pd_name+" · "+kl.get("kl_type","KL"),
                    "zone_type":kl.get("kl_type","KL"),"direction":direction,"trend":trend,
                    "entry":entry,"entry_type":f"Model #2 (FVG Tip · {ltf})",
                    "sl":sl_p,"tp":tp2,"tp1":tp1,"tp2":tp2,"sl_shift_at":sl_trail,
                    "rr":rr,"crh":round(zone_top,8),"crl":round(zone_bot,8),
                    "ob_top":round(zone_top,8),"ob_bot":round(zone_bot,8),
                    "fvg_found":True,"fvg_type":"M2-FVG","fvg_entry":entry,
                    "fvg_top":round(fvg_top,8),"fvg_bot":round(fvg_bot,8),
                    "sweep_extreme":round(sweep_extreme,8),
                    "tbs_found":False,"tbs_tf":"–","tbs_entry":"–","tbs_sl":"–",
                    "choch_found":False,"choch_level":"–","liq_swept":True,
                    "ob_respected":False,"continuous":True,
                    "score":score,"grade":grade,"details":details,
                    "timestamp":datetime.now(LOCAL_TZ).strftime("%Y-%m-%d %H:%M UTC+1"),
                })
                break
...
def detect_m2_sharp_sweep(symbol):
    """
    Phase 1 of Model #2 monitoring:
    Detect a sharp sweep of a HTF key level without a subsequent FVG yet.
    Returns list of pending monitors to watch for FVG.
    """
    pending = []
    ref = get_candles(symbol, "Hour4", limit=200)
    ...
    for htf in ["Hour4", "Min60"]:
        ...
        for kl in kls[:8]:
            ...
            # Check each LTF for a recent sweep WITHOUT a subsequent FVG yet
            for ltf in M2_LTF_ORDER:
                ltf_c = get_candles(symbol, ltf, limit=200)
                if not ltf_c or len(ltf_c) < 20:
                    continue
                first_extreme = _find_first_touch_extreme(ltf_c, zone_top, zone_bot, direction)
                if first_extreme is None:
                    continue
                # Check if sweep happened but NO FVG yet
                sweep_found = False
                for i, c in enumerate(ltf_c):
                    if direction == "SELL" and c["high"] > first_extreme:
                        sweep_found = True; sweep_idx = i; sweep_extreme = c["high"]; break
                    elif direction == "BUY" and c["low"] < first_extreme:
                        sweep_found = True; sweep_idx = i; sweep_extreme = c["low"]; break
                if not sweep_found:
                    continue
                # Check if FVG already formed after sweep
                sweep_extreme2, fvg_top, fvg_bot, fvg_tip = _sweep_and_fvg(ltf_c, first_extreme, direction)
                if fvg_tip is not None:
                    continue  # FVG already formed — scan_pair_model2 handles this
                # Sweep found but no FVG yet → add to monitor phase 1
                pending.append({
                    "symbol": symbol, "phase": 1,
                    "direction": direction, "trend": trend,
                    "htf": htf, "ltf": ltf,
                    ...
                    "sweep_extreme": round(sweep_extreme, 8),
                    "first_extreme": round(first_extreme, 8),
                    "added_at": datetime.now(LOCAL_TZ).strftime("%H:%M UTC+1"),
                })
                break
        if pending:
...
def m2_monitor_loop():
    """
    Model #2 background monitor:
    Phase 1 — sweep spotted, watching for FVG to form
    Phase 2 — FVG formed, watching for price to tap the FVG zone → instant market order
    """
    log("🔥 Model #2 monitor started")
    while True:
        try:
            with m2_lock:
            ...
            for symbol in symbols:
                ...
                if mon["phase"] == 1:
                    # Phase 1: Watch for FVG to form after the sweep
                    sweep_extreme2, fvg_top, fvg_bot, fvg_tip = _sweep_and_fvg(
                        ltf_c, mon["first_extreme"], direction)
                    if fvg_tip is None:
                        continue  # FVG not yet formed
                    # FVG formed! Upgrade to phase 2
                    log(f"🔥 M#2 FVG FORMED: {symbol} {direction} LTF:{ltf} FVG:{round(fvg_top,6)}/{round(fvg_bot,6)}")
                    with m2_lock:
                        if symbol in m2_monitor:
                            m2_monitor[symbol].update({
                                "phase": 2,
                                "fvg_top":  round(fvg_top, 8),
                                "fvg_bot":  round(fvg_bot, 8),
                                "fvg_tip":  round(fvg_tip, 8),
                                "sweep_extreme": round(sweep_extreme2, 8),
                                "fvg_formed_at": datetime.now(LOCAL_TZ).strftime("%H:%M UTC+1"),
                            })

                elif mon["phase"] == 2:
                    # Phase 2: Watch for price to TAP the FVG
                    fvg_top = mon["fvg_top"]
                    fvg_bot = mon["fvg_bot"]
                    fvg_tip = mon["fvg_tip"]
                    ...
                    price = ticker["price"]
                    tapped = False
                    if direction == "SELL" and price >= fvg_bot:
                        tapped = True
                    elif direction == "BUY" and price <= fvg_top:
                        tapped = True

                    if not tapped:
                        continue

                    # PRICE TAPPED THE FVG — fire market order instantly
                    log(f"🔥 M#2 FVG TAPPED: {symbol} {direction} price={price} FVG:{fvg_bot}/{fvg_top}")

                    # Calculate entry/sl/tp/rr
                    zone_top = mon["zone_top"]; zone_bot = mon["zone_bot"]
                    entry  = round(price, 8)
                    buf    = (zone_top - zone_bot) * 0.05
                    sweep_extreme = mon["sweep_extreme"]
                    sl_p   = round(sweep_extreme + buf, 8) if direction == "SELL" else round(sweep_extreme - buf, 8)
                    liq    = _swing_before_touch(ltf_c, direction)
                    if liq is None:
                    ...
                    if rr < 2.0:
                        log(f"⚠️ M#2 {symbol} RR {rr}R < 2R minimum — skipping trade")
                        with m2_lock: m2_monitor.pop(symbol, None)
                        continue

                    score = 75; grade = "A+" if rr >= 4 else "A"
                    sig = {
                        "model": "2",
                        "symbol": symbol,
                        "tf": mon["htf"],
                        "ob_tf": ltf,
                        "ob_zone": mon["zone_name"],
                        "zone_type": mon["kl_type"],
                        "direction": direction,
                        "trend": mon["trend"],
                        "entry": entry,
                        "entry_type": f"Model #2 FVG Tap · {ltf} (Monitor)",
                        "sl": sl_p,
                        "tp": tp2,
                        "tp1": tp1,
                        "tp2": tp2,
                        ...
                        "details": [
                            "✅ HTF Key Level tap",
                            f"✅ Sharp sweep: {round(sweep_extreme,6)}",
                            f"✅ FVG formed on {ltf}",
                            f"✅ Price tapped FVG — instant entry",
                            f"✅ {rr}R · Min 2R gate PASSED",
                        ],
                        "from_m2_monitor": True,
                        "timestamp": datetime.now(LOCAL_TZ).strftime("%Y-%m-%d %H:%M UTC+1"),
                    }

                    signals.appendleft(sig)
                    send_telegram(fmt_tg_m2(sig), kind="signal")
                    log(f"🔥 M#2 MONITOR SIGNAL: {direction} {symbol} RR:{rr}R {grade}")

                    # Fire instant market order
                    ready, reason = live_trade_ready()
                    if ready:
                        ok_t, msg_t = place_order(sig)
                        log(f"{'✅' if ok_t else '❌'} M#2 Auto-trade: {msg_t}")
                        if not ok_t:
                            send_telegram(f"M#2 Trade REJECTED: {symbol} {direction}\nReason: {msg_t}", kind="trade")
                    else:
                        log(f"⚠️ M#2 live trade skipped ({symbol}): {reason}")

                    if paper_config["enabled"] and paper_config["auto_trade"]:
                        ok2, msg2 = place_paper_order(sig)
                        if ok2: log(f"📝 M#2 Paper auto: {msg2}")

                    with m2_lock:
                        m2_monitor.pop(symbol, None)

        except Exception as e:
            log(f"❌ M2 monitor error: {e}")
...
def mexc_request(method, path, params=None, signed=True):
    ...
    Signature: HMAC-SHA256(apiKey + timestamp + requestParam, secretKey)
    GET:  requestParam = sorted urlencode query string
    POST: requestParam = exact JSON body string (sorted keys)
    """
    ...
    if method == "GET":
        ...
        # POST: sort keys so signature matches body exactly
        sorted_dict = dict(sorted(params.items()))
        body_str    = _json.dumps(sorted_dict, separators=(",", ":"))
        query_str   = body_str

    raw = str(api_key) + str(ts) + str(query_str)
    ...
    try:
        ...
        if method == "GET":
            ...
            r = session.post(url, data=body_str, timeout=15)

        raw_text = r.text.strip()
        if not raw_text:
            return None, f"Empty response (HTTP {r.status_code}) on {path}"

        # Detect proxy "Access Denied" HTML pages
        if raw_text.startswith("<") or "Access Denied" in raw_text or "access denied" in raw_text.lower():
            return None, f"Proxy/firewall blocked request to MEXC — try a VPS or different network. Raw: {raw_text[:80]}"

        try:
            data = _json.loads(raw_text)
        except Exception:
            return None, f"Non-JSON response: {raw_text[:120]}"
...
def place_order(sig):
    ...
    lev_data, lev_err = mexc_request("POST", "/position/change_leverage", {
        "symbol":       sig["symbol"],
        "leverage":     leverage,
        "openType":     open_type,
        "positionType": side,
...
def place_paper_order(sig):
    ...
    with paper_lock:
        ...
        # Realistic sizing: match live trading math
        # contracts = risk_amount / (sl_distance * contract_size)
        # contract_size fetched from MEXC, fallback=1
        ...
        # Size in contracts so that: contracts * contract_size * sl_distance = risk_amount
        size_raw  = risk_amount / (sl_distance * contract_size)
        size_raw  = max(min_vol, round(size_raw / min_vol) * min_vol)
        contracts = int(size_raw)

        # Verify actual max loss doesn't exceed risk_amount (no sugar coating)
        actual_loss = contracts * contract_size * sl_distance
        if actual_loss > risk_amount * 1.10:
            contracts = max(int(min_vol), int(contracts * (risk_amount / actual_loss)))
            actual_loss = contracts * contract_size * sl_distance
...
def close_paper_trade(symbol, reason="Manual", close_price=None):
    ...
    contract_size = trade.get("contract_size", get_symbol_info(symbol)["contract_size"])
    contract_size = max(contract_size, 1e-8)
    risk_amount   = trade["risk_amount"] if trade["risk_amount"] > 0 else 1.0
    leverage      = trade.get("leverage", trade_config.get("leverage", 35))

    # Realistic futures PnL: size(contracts) * contract_size * price_diff
    if direction == "BUY":
        raw_pnl = size * contract_size * (close_price - entry)
    else:
        raw_pnl = size * contract_size * (entry - close_price)

    # No sugar coating — full loss is possible (capped at position_value, not risk_amount)
    position_value = size * contract_size * entry
    max_loss       = position_value / leverage if leverage > 0 else risk_amount
    pnl            = max(raw_pnl, -max_loss)   # can lose at most the margin
        ...
        try:
            ...
            for i,symbol in enumerate(pairs):
                ...
                try:
                    ...
                    # ── M2 Phase-1 monitoring: sweep found but no FVG yet ──
                    if scan_settings.get("model2_enabled", True) and not m2:
                        with m2_lock:
                            already_m2 = symbol in m2_monitor
                            m2_full    = len(m2_monitor) >= MAX_M2_MONITORED
                        if not already_m2 and not m2_full:
                            sweeps = detect_m2_sharp_sweep(symbol)
                            if sweeps:
                                sw = sweeps[0]
                                with m2_lock:
                                    m2_monitor[symbol] = sw
                                log(f"🔥 M#2 SWEEP MONITOR: {symbol} {sw['direction']} {sw['ltf']} — watching for FVG")
...
<div class="tab-wrap">
  ...
  <div id="tab-monitor" style="display:none">
    ...
    <div class="panel" style="margin-top:14px">
      <div class="panel-ttl">🔥 Model #2 — Sweep/FVG Monitor <span id="m2mon-count" style="font-family:'JetBrains Mono',monospace;font-size:.72rem;color:var(--dim)">(0/6)</span><span style="margin-left:auto;font-family:'JetBrains Mono',monospace;font-size:.62rem;color:var(--dim)">Phase 1=Awaiting FVG · Phase 2=Awaiting FVG Tap</span></div>
      <div id="m2monitor-wrap"><div class="empty" style="padding:40px"><div class="empty-ico">🔭</div><div class="empty-t">No M#2 pairs monitored</div><div class="empty-s">Sharp sweeps of HTF key levels appear here — Phase 1 watches for FVG, Phase 2 fires instantly when price taps the FVG</div></div></div>
    </div>
  </div>
  <!-- HISTORY -->
  ...
  <div id="tab-trade-cfg" style="display:none">
    <div class="panel">
      ...
      <!-- Model Engine Toggles -->
      <div style="background:rgba(124,58,237,.06);border:2px solid rgba(124,58,237,.2);border-radius:14px;padding:16px;margin-bottom:14px">
        <div style="font-family:'Fredoka One',sans-serif;font-size:.9rem;color:#a78bfa;margin-bottom:12px;letter-spacing:.05em">🤖 Scan Engine Toggles</div>
        <div style="display:flex;gap:12px;flex-wrap:wrap">
          <label style="display:flex;align-items:center;gap:10px;cursor:pointer;background:rgba(16,185,129,.07);border:1.5px solid rgba(16,185,129,.25);border-radius:12px;padding:10px 16px;min-width:200px">
            <input type="checkbox" id="t-model1" style="width:18px;height:18px;accent-color:#10b981" onchange="saveEngineToggles()"/>
            <div><div style="font-family:'Fredoka One',sans-serif;font-size:.9rem;color:#10b981">🎯 Model #1 Engine</div><div style="font-size:.65rem;color:var(--dim);margin-top:2px">CRT + TBS + Manipulation Monitor</div></div>
          </label>
          <label style="display:flex;align-items:center;gap:10px;cursor:pointer;background:rgba(249,115,22,.07);border:1.5px solid rgba(249,115,22,.25);border-radius:12px;padding:10px 16px;min-width:200px">
            <input type="checkbox" id="t-model2" style="width:18px;height:18px;accent-color:#f97316" onchange="saveEngineToggles()"/>
            <div><div style="font-family:'Fredoka One',sans-serif;font-size:.9rem;color:#f97316">🔥 Model #2 Engine</div><div style="font-size:.65rem;color:var(--dim);margin-top:2px">HTF KL Sweep + FVG Tap Monitor</div></div>
          </label>
        </div>
        <div id="engine-toggle-msg" style="margin-top:8px;font-family:'JetBrains Mono',monospace;font-size:.65rem;color:var(--green);display:none"></div>
      </div>
...
async function fetchMonitor(){if(activeTab!=="monitor")return;try{const[r1,r2]=await Promise.all([fetch("/api/monitor"),fetch("/api/m2-monitor")]);const data=await r1.json();const m2data=await r2.json();$("smon").textContent=data.length+m2data.length;$("mon-count").textContent=`(${data.length}/4)`;if($("m2mon-count"))$("m2mon-count").textContent=`(${m2data.length}/6)`;const wrap=$("monitor-wrap");if(wrap){if(!data.length){wrap.innerHTML='<div class="empty" style="padding:40px"><div class="empty-ico">🌙</div><div class="empty-t">Nothing monitored</div><div class="empty-s">Pairs in manipulation phase appear here automatically</div></div>';}else{wrap.innerHTML=`<div class="monitor-grid">${data.map(m=>{const dir=(m.direction||"BUY").toUpperCase();const tf=TFM[m.crt_tf]||m.crt_tf||"–";return`<div class="mon-card ${dir.toLowerCase()}"><div class="mon-sym">${dir==="BUY"?"🟢":"🔴"} ${m.symbol||"–"}</div><div class="mon-row"><span>Mad Man TF</span><span>${tf}</span></div><div class="mon-row"><span>Key Level</span><span>${m.kl_type||"–"}</span></div><div class="mon-row"><span>Trend</span><span>${m.trend||"–"}</span></div><div class="mon-row"><span>CRH</span><span>${fmt(m.crh)}</span></div><div class="mon-row"><span>CRL</span><span>${fmt(m.crl)}</span></div><div class="mon-row"><span>Zone</span><span>${m.zone_name||"–"}</span></div><div class="mon-row"><span>Added</span><span>${m.added_at||"–"}</span></div><div class="mon-status">⏳ AWAITING C2 CLOSE</div></div>`;}).join("")}</div>`;}}
const m2wrap=$("m2monitor-wrap");if(m2wrap){if(!m2data.length){m2wrap.innerHTML='<div class="empty" style="padding:40px"><div class="empty-ico">🔭</div><div class="empty-t">No M#2 pairs monitored</div></div>';}else{m2wrap.innerHTML=`<div class="monitor-grid">${m2data.map(m=>{const dir=(m.direction||"BUY").toUpperCase();const ph=m.phase||1;const phLabel=ph===2?"⚡ PHASE 2: AWAIT FVG TAP":"👁 PHASE 1: AWAIT FVG";const phColor=ph===2?"var(--orange)":"var(--cyan)";return`<div class="mon-card ${dir.toLowerCase()}"><div class="mon-sym">${dir==="BUY"?"🟢":"🔴"} ${m.symbol||"–"}</div><div class="mon-row"><span>HTF/LTF</span><span>${TFM[m.htf]||m.htf||"–"} / ${TFM[m.ltf]||m.ltf||"–"}</span></div><div class="mon-row"><span>Zone</span><span>${m.kl_type||"–"}</span></div><div class="mon-row"><span>Sweep</span><span>${fmt(m.sweep_extreme)}</span></div>${ph===2?`<div class="mon-row"><span>FVG</span><span style="color:var(--orange)">${fmt(m.fvg_bot)} – ${fmt(m.fvg_top)}</span></div>`:""}<div class="mon-row"><span>Added</span><span>${m.added_at||"–"}</span></div><div class="mon-status" style="background:rgba(249,115,22,.12);color:${phColor};border-color:rgba(249,115,22,.3)">${phLabel}</div></div>`;}).join("")}</div>`;}}
}catch{}}
...
async function loadTradeConfig(){try{const r=await fetch("/api/trade-config");const d=await r.json();if(d.api_key)$("t-apikey").value=d.api_key;$("t-risk").value=d.risk_pct||1;if($("t-leverage"))$("t-leverage").value=d.leverage||0;updateTradeBadge(d.enabled);checkRiskWarning();}catch{}}
async function loadEngineToggles(){try{const r=await fetch("/api/settings");const d=await r.json();const m1=document.getElementById("t-model1");const m2=document.getElementById("t-model2");if(m1)m1.checked=d.model1_enabled!==false;if(m2)m2.checked=d.model2_enabled!==false;}catch{}}
window.saveEngineToggles=async function(){const m1El=document.getElementById("t-model1");const m2El=document.getElementById("t-model2");const payload={model1_enabled:m1El?m1El.checked:true,model2_enabled:m2El?m2El.checked:true};try{const r=await fetch("/api/settings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});const d=await r.json();const msg=document.getElementById("engine-toggle-msg");if(msg){msg.textContent=d.ok?"✅ Engine settings saved!":"❌ Save failed";msg.style.display="block";setTimeout(()=>{msg.style.display="none"},3000);}toast(d.ok?"🤖 Engine settings saved!":"❌ Error","trade");}catch{toast("❌ Error saving","trade");}};
...
window.sw=function(tab,btn){activeTab=tab;document.querySelectorAll(".tab").forEach(b=>b.classList.remove("active"));btn.classList.add("active");["signals","trades","monitor","history","trade-cfg","paper","log","settings"].forEach(t=>{const el=$("tab-"+t);if(el)el.style.display=t===tab?"block":"none";});if(tab==="log"){fetchLog();fetchDiag();}if(tab==="trades")window.fetchPnl();if(tab==="monitor")fetchMonitor();if(tab==="history")fetchHistory();if(tab==="trade-cfg"){loadTradeConfig();loadEngineToggles();}if(tab==="paper"){loadPaperConfig();fetchPaperData();}if(tab==="settings")loadSettings();};
...
def api_monitor():
    with manip_lock:
        m1_list = list(manip_monitor.values())
    with m2_lock:
        m2_list = list(m2_monitor.values())
    return jsonify(m1_list)
...
def start_scanner():
    ...
    log("🚀 Scanner + manipulation monitor + paper monitor + M#2 monitor threads launched.")
