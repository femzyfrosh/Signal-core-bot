...
async function loadChart(sym,tf){
  ...
  drawTool=null;_setToolUI(null);_mode='idle';previewShape=null;
...
function _draw(ctx,sh,isSel,isPrev){
  ...
  if(sh.type==='rect'){
    ...
    if(sh.label&&!isPrev){
      ...
      const lbx=lx+w/2-tw/2, lby=ty+h/2;
      ctx.fillStyle='rgba(7,8,16,.72)';
      ctx.fillRect(lbx-4,lby-12,tw+8,16);
      ctx.fillStyle=lc;
    ...
    if(sh.label&&!isPrev){
      ...
      const tw=ctx.measureText(sh.label).width;
      ctx.fillStyle='rgba(7,8,16,.72)';
      ctx.fillRect(midX-tw/2-4,midY-14,tw+8,16);
      ctx.fillStyle=lc;
...
function initDrawCanvas(){
  ...
  function onDown(e){
    ...
    const overShape=_overShape(cx,cy);

    // Not our event — let chart/nav/watchlist handle it normally
    if(!isDrawMode&&!overShape){
      // Tapping empty space while a shape is selected → deselect it
      if(selectedIdx>=0){
        selectedIdx=-1;
        redrawCanvas();
      }
    ...
    // Our event — prevent default (scroll) but NOT stopPropagation
    e.preventDefault();
