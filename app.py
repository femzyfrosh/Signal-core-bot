...
def _find_ob_zone_before_choch(candles, choch_idx, direction, lookback=20):
    ...
    if direction == "SELL":
        ...
        ob_entry = round(ob_bot, 8)   # entry at bottom of OB (retest)
    else:
        ob_top   = max(c["high"]  for c in ob_candles)
        ob_bot   = min(c["low"]   for c in ob_candles)
        ob_entry = round(ob_top, 8)   # entry at top of OB (retest)
    ...
    while True:
        try:
            ...
            for symbol in symbols:
                ...
                # ════════════════════════════════════════════════════════
                if phase == "AWAIT_TBS":
                    # Invalidation: a new candle has broken decisively beyond
                    # CRH (SELL) or CRL (BUY) with its body → CRT voided
                    ...
                    # TBS confirmed — compute levels and advance to AWAIT_PRICE
                    ...
                    log(f"🐢 M1 TBS CONFIRMED: {symbol} on {tbs_tf} | "
                        f"TBS open:{tbs_entry} SL:{tbs_sl} TP:{tp} RR:{rr}R | "
                        f"→ AWAITING PRICE @ {tbs_entry}")
                    ...
                    with manip_lock:
                        if symbol in manip_monitor:
                            manip_monitor[symbol].update({
                                "phase":      "AWAIT_PRICE",
                                "tbs_tf":     tbs_tf,
                                "tbs_entry":  tbs_entry,   # price level to watch
                                "tbs_sl":     tbs_sl,
                                "tp":         tp,
                                "rr":         rr,
                                "score":      score,
                ...
                # ════════════════════════════════════════════════════════
                # PHASE: AWAIT_PRICE
                # TBS confirmed. Watch live price every tick.
                # Fire instant market order the moment price touches or
                # crosses the TBS open level.
                # ════════════════════════════════════════════════════════
                if phase == "AWAIT_PRICE":
                    tbs_entry = monitor.get("tbs_entry", 0)
                    tbs_sl    = monitor.get("tbs_sl",    0)
                    tp        = monitor.get("tp",        0)
                    rr        = monitor.get("rr",        0)
                    score     = monitor.get("score",     70)
                    ...
                    # Invalidation: price has closed beyond TBS SL (setup voided)
                    if direction == "SELL" and price < tbs_sl * 0.999:
                        log(f"❌ M1 AWAIT_PRICE INVALID: {symbol} — price {price} blew through TBS SL {tbs_sl}")
                        _reset_to_watching(symbol)
                        continue
                    if direction == "BUY" and price > tbs_sl * 1.001:
                        log(f"❌ M1 AWAIT_PRICE INVALID: {symbol} — price {price} blew through TBS SL {tbs_sl}")
                        _reset_to_watching(symbol)
                        continue

                    # Entry trigger: price has traded back to or through TBS open
                    # SELL: price rises back up to TBS open (TBS opened inside range above current)
                    # BUY:  price falls back down to TBS open (TBS opened inside range below current)
                    triggered = False
                    if direction == "SELL" and price >= tbs_entry:
                        triggered = True
                    if direction == "BUY"  and price <= tbs_entry:
                        triggered = True

                    if not triggered:
                        continue   # Keep watching

                    # ── FIRE: instant market order at current price ───────
                    log(f"🚀 M1 PRICE HIT TBS OPEN: {symbol} {direction} | "
                        f"price={price} tbs_open={tbs_entry} | FIRING MARKET ORDER")
                    ...
                    sig = {
                        ...
                        "trend":       trend,
                        "entry":       round(price, 8),
                        "entry_type":  "Model #1 (TBS — price returned to open)",
                        "sl":          round(tbs_sl, 8),
                        "tp":          round(tp, 8),
                        "tp1":         round((price + tp) / 2, 8),
                        "tp2":         round(tp, 8),
                        "rr":          rr,
                        "crh":         crh,
                        "crl":         crl,
                        ...
                        "details": [
                            ...
                            f"✅ TBS on {tbs_tf}: open={tbs_entry}, SL={tbs_sl}",
                            f"🚀 Price returned to TBS open → market order @ {price}",
                    ...
                    # Advance to RUNNING — keep zone alive for next CRT after this trade
                    with manip_lock:
                        if symbol in manip_monitor:
                            manip_monitor[symbol].update({
                                "phase":        "RUNNING",
                                "signal_entry": round(price, 8),
...
/* Tools popup */
.tools-popup{position:fixed;z-index:120;background:var(--s2);border:1px solid var(--border);border-radius:12px;padding:8px;display:none;flex-direction:column;gap:4px;box-shadow:0 8px 32px rgba(0,0,0,.65);backdrop-filter:blur(14px);min-width:160px}
.tp-item{display:flex;align-items:center;gap:8px;padding:9px 10px;border-radius:8px;border:none;background:transparent;color:var(--text);font-size:.78rem;font-weight:600;cursor:pointer;transition:background .15s;text-align:left;width:100%}
.tp-item:hover{background:rgba(255,255,255,.07)}
.tp-item.active{background:rgba(79,110,247,.18);color:var(--accent)}
...
/* Shape settings popup — enhanced TradingView-style */
.ss-popup{position:fixed;z-index:125;background:var(--s2);border:1px solid var(--border);border-radius:14px;padding:14px;display:none;flex-direction:column;gap:10px;box-shadow:0 8px 36px rgba(0,0,0,.7);backdrop-filter:blur(16px);min-width:240px;max-width:92vw}
...
.ss-swatch.sel{border-color:#fff;box-shadow:0 0 0 2px rgba(255,255,255,.3)}
...
/* Thickness slider */
.ss-slider{-webkit-appearance:none;appearance:none;width:100%;height:4px;border-radius:2px;background:var(--muted);outline:none}
.ss-slider::-webkit-slider-thumb{-webkit-appearance:none;width:16px;height:16px;border-radius:50%;background:var(--accent);cursor:pointer}
/* Line style select */
.ss-select{background:var(--s3);border:1px solid var(--border);border-radius:7px;color:var(--text);font-family:var(--mono);font-size:.72rem;padding:5px 8px;outline:none;width:100%}
/* Info tooltip overlay on canvas */
#draw-info-tip{position:absolute;z-index:10;background:rgba(13,15,30,.92);border:1px solid rgba(79,110,247,.35);border-radius:8px;padding:5px 9px;font-family:var(--mono);font-size:.62rem;color:var(--text);pointer-events:none;display:none;white-space:nowrap;line-height:1.7;backdrop-filter:blur(6px)}
...
#chart-container{position:absolute;inset:0;width:100%;height:100%}
...
<div class="app">
  ...
  <div class="main">
    ...
    <div class="page hidden-right" id="page-chart">
      ...
      <!-- Shape settings popup (double-tap on shape) — enhanced -->
      <div class="ss-popup" id="ss-popup">
        <div style="display:flex;align-items:center;justify-content:space-between">
          <div class="ss-title" id="ss-popup-title">Shape Settings</div>
          <button onclick="closeShapeSettings()" style="background:none;border:none;color:var(--dim);cursor:pointer;font-size:.9rem;padding:2px 6px">✕</button>
        </div>
        <!-- Colour row: border/line -->
        <div id="ss-border-row">
          <div class="ss-title" style="margin-bottom:4px" id="ss-color-lbl">Line colour</div>
          <div class="ss-palette" id="ss-border-palette"></div>
        </div>
        <!-- Fill colour (rect only) -->
        <div id="ss-fill-row">
          <div class="ss-title" style="margin-bottom:4px">Fill colour</div>
          <div class="ss-palette" id="ss-fill-palette"></div>
        </div>
        <!-- Opacity (rect only) -->
        <div id="ss-opacity-row" class="ss-row" style="flex-direction:column;align-items:stretch;gap:4px">
          <div style="display:flex;justify-content:space-between;font-size:.72rem;color:var(--dim)"><label>Fill opacity</label><span id="ss-opacity-val">12%</span></div>
          <input type="range" class="ss-slider" id="ss-opacity" min="0" max="60" step="2" value="12" oninput="document.getElementById('ss-opacity-val').textContent=this.value+'%'"/>
        </div>
        <!-- Thickness -->
        <div class="ss-row" style="flex-direction:column;align-items:stretch;gap:4px">
          <div style="display:flex;justify-content:space-between;font-size:.72rem;color:var(--dim)"><label>Thickness</label><span id="ss-thick-val">2px</span></div>
          <input type="range" class="ss-slider" id="ss-thickness" min="1" max="6" step="1" value="2" oninput="document.getElementById('ss-thick-val').textContent=this.value+'px'"/>
        </div>
        <!-- Line style (trend only) -->
        <div id="ss-style-row" class="ss-row">
          <label style="font-size:.72rem;color:var(--dim)">Style</label>
          <select class="ss-select" id="ss-linestyle">
            <option value="solid">Solid</option>
            <option value="dashed">Dashed</option>
            <option value="dotted">Dotted</option>
          </select>
        </div>
        <!-- Extend options (trend only) -->
        <div id="ss-extend-row" class="ss-row" style="gap:12px">
          <label style="font-size:.72rem;color:var(--dim);flex:1">Ray (extend right)</label>
          <input type="checkbox" id="ss-ray" style="width:16px;height:16px;cursor:pointer">
        </div>
        <!-- Mid-line (rect only) -->
        <div id="ss-midline-row" class="ss-row">
          <label>Middle line</label>
          <input type="checkbox" id="ss-midline" style="width:16px;height:16px;cursor:pointer">
        </div>
        <!-- Lock -->
        <div class="ss-row">
          <label>Lock (prevent move)</label>
          <input type="checkbox" id="ss-lock" style="width:16px;height:16px;cursor:pointer">
        </div>
        <!-- Info readout -->
        <div id="ss-info-box" style="background:var(--s3);border:1px solid var(--border);border-radius:8px;padding:7px 9px;font-family:var(--mono);font-size:.62rem;color:var(--dim);line-height:1.8;display:none"></div>
        <div class="ss-btns">
      ...
      <!-- Info tooltip shown on hover/select on canvas -->
      <div id="draw-info-tip"></div>
      ...
      <div class="chart-wrap">
        ...
        <!-- Drawing canvas overlay (pointer-events enabled only when a tool is active) -->
        <canvas id="draw-canvas" style="position:absolute;inset:0;z-index:6;pointer-events:none;display:block"></canvas>
...
async function loadChart(sym,tf){
  ...
  // Create chart — read dimensions from parent chart-wrap if container not yet sized
  const chartWrap=container.parentElement;
  const cw=container.clientWidth||chartWrap.clientWidth||window.innerWidth;
  const ch=container.clientHeight||chartWrap.clientHeight||(window.innerHeight-200);
  ...
  // Resize observer — watch the chart-wrap (always has a size) not just the container
  ...
  // Redraw drawing overlay whenever chart pans or zooms
  chart.timeScale().subscribeVisibleTimeRangeChange(()=>redrawCanvas());
  // Also redraw on price scale change (vertical scroll / autoscale)
...
// ══════════════════════════════════════════════════════════════════
// ── TradingView-style Drawing Engine v2  ─────────────────────────
// ══════════════════════════════════════════════════════════════════
//
// Shapes stored as plain objects persisted in the `shapes` array.
// Each shape has a "schema" union:
//
//  RECTANGLE: { type:'rect', t1,p1, t2,p2,
//               borderColor, fillColor, opacity(0-60),
//               thickness(1-6), midLine, locked }
//
//  TRENDLINE: { type:'trend', t1,p1, t2,p2,
//               color, thickness(1-6), style('solid'|'dashed'|'dotted'),
//               ray(bool), locked }
//
// Interaction states (drag sub-modes):
//   'none'        – no drag in progress
//   'draw'        – creating a new shape (first point placed)
//   'move'        – dragging the whole shape
//   'resize-[id]' – dragging a specific handle
//     rect handles: 'tl','tr','bl','br'  (corners)
//                   'tm','bm','lm','rm'  (edge midpoints)
//     trend handles: 'p1', 'p2'
//
// Mobile notes:
//   • All interactions are driven by a single unified pointer model
//     (touch → touchstart/move/end, mouse → mousedown/move/up).
//   • Touch handles are oversized (HIT_R=22px) for fat-finger accuracy.
//   • Pointer capture (setPointerCapture) is used when available so
//     drag doesn't lose tracking if pointer leaves the canvas rect.
//   • canvas.style.pointerEvents toggled between 'auto' and 'none'
//     so chart panning/pinch-zoom still works when no tool active.
// ──────────────────────────────────────────────────────────────────

let drawTool   = null;       // 'rect' | 'trend' | 'select' | null
let shapes     = [];         // persistent shape array
let selectedIdx= -1;         // index into shapes[] of selected shape
let previewShape = null;     // shape being drawn (not yet committed)

// ── Constants ─────────────────────────────────────────────────────
const SHAPE_COLORS = ['#4f6ef7','#00d4aa','#ff4d6a','#ffc93c','#a855f7','#f97316','#e2e8f0','#06b6d4'];
const HIT_R      = 22;   // pixels — hit radius for handles and lines (mobile friendly)
const HANDLE_R   = 6;    // pixels — drawn handle circle radius
const HANDLE_R_M = 8;    // pixels — slightly bigger on touch

// ── Default shape factory ─────────────────────────────────────────
function _defaultRect(t1,p1,t2,p2){
  return{type:'rect',t1,p1,t2,p2,borderColor:'#4f6ef7',fillColor:'#4f6ef7',
         opacity:12,thickness:2,midLine:false,locked:false};
}
function _defaultTrend(t1,p1,t2,p2){
  return{type:'trend',t1,p1,t2,p2,color:'#ffc93c',
         thickness:2,style:'solid',ray:false,locked:false};
}
...
// ── Palette builder ───────────────────────────────────────────────
function buildPalette(containerId,initial){
...
// ── Active tool UI sync ───────────────────────────────────────────
function _setActiveToolUI(tool){
  ...
  if(canvas){
    // Canvas intercepts pointer only when a tool is active OR a shape is selected
    canvas.style.pointerEvents=(tool||selectedIdx>=0)?'auto':'none';
    if(!tool)           canvas.style.cursor='default';
...
window.activateTool=function(tool){
  drawTool=tool;selectedIdx=-1;previewShape=null;
  document.getElementById('tools-popup').style.display='none';
  _setActiveToolUI(tool);
  redrawCanvas();
};

window.clearAllShapes=function(){
  shapes=[];selectedIdx=-1;previewShape=null;drawTool=null;
  document.getElementById('tools-popup').style.display='none';
  _setActiveToolUI(null);
  _hideInfoTip();
  redrawCanvas();
};
...
window.applyShapeSettings=function(){
  ...
  const b=document.querySelector('#ss-border-palette .ss-swatch.sel');
  const f=document.querySelector('#ss-fill-palette .ss-swatch.sel');
  const thick=parseInt(document.getElementById('ss-thickness').value)||2;
  const lock=document.getElementById('ss-lock').checked;
  if(sh.type==='rect'){
    if(b) sh.borderColor=b.dataset.color;
    if(f) sh.fillColor=f.dataset.color;
    sh.opacity=parseInt(document.getElementById('ss-opacity').value)||12;
    sh.thickness=thick;
    sh.midLine=document.getElementById('ss-midline').checked;
    sh.locked=lock;
  } else {
    if(b) sh.color=b.dataset.color;
    sh.thickness=thick;
    sh.style=document.getElementById('ss-linestyle').value||'solid';
    sh.ray=document.getElementById('ss-ray').checked;
    sh.locked=lock;
  }
...
window.deleteSelectedShape=function(){
  if(selectedIdx>=0)shapes.splice(selectedIdx,1);
  selectedIdx=-1;
  document.getElementById('ss-popup').style.display='none';
  _setActiveToolUI(drawTool);
  redrawCanvas();
};
...
function openShapeSettings(idx){
  if(idx<0||idx>=shapes.length)return;
  const sh=shapes[idx];
  const isRect=sh.type==='rect';

  document.getElementById('ss-popup-title').textContent=isRect?'Rectangle Settings':'Trend Line Settings';
  document.getElementById('ss-color-lbl').textContent=isRect?'Border colour':'Line colour';

  buildPalette('ss-border-palette',sh.borderColor||sh.color||'#4f6ef7');

  // Show/hide rect-only rows
  document.getElementById('ss-fill-row').style.display   = isRect?'block':'none';
  document.getElementById('ss-opacity-row').style.display= isRect?'flex':'none';
  document.getElementById('ss-midline-row').style.display= isRect?'flex':'none';
  // Show/hide trend-only rows
  ...
  if(isRect){
    ...
    const op=sh.opacity??12;
    document.getElementById('ss-opacity').value=op;
    document.getElementById('ss-opacity-val').textContent=op+'%';
    document.getElementById('ss-midline').checked=!!sh.midLine;
  } else {
    document.getElementById('ss-linestyle').value=sh.style||'solid';
    document.getElementById('ss-ray').checked=!!sh.ray;
  }

  const thick=sh.thickness||2;
  document.getElementById('ss-thickness').value=thick;
  document.getElementById('ss-thick-val').textContent=thick+'px';
  document.getElementById('ss-lock').checked=!!sh.locked;

  // Info readout
  const infoBox=document.getElementById('ss-info-box');
  const info=_shapeInfo(sh);
  if(info){infoBox.innerHTML=info;infoBox.style.display='block';}
...
// ── Shape info string (prices, dimensions) ─────────────────────────
function _shapeInfo(sh){
  if(!sh)return'';
  const fp=v=>{const n=Number(v);if(!n)return'–';return n>=1?n.toFixed(4):n.toFixed(8);};
  if(sh.type==='rect'){
    const pHigh=Math.max(sh.p1,sh.p2),pLow=Math.min(sh.p1,sh.p2);
    const height=Math.abs(sh.p1-sh.p2);
    const pct=(height/pLow*100).toFixed(2);
    return`Top: ${fp(pHigh)}<br>Bot: ${fp(pLow)}<br>Δ: ${fp(height)} (${pct}%)`;
  } else {
    const dx=sh.t2-sh.t1,dy=sh.p2-sh.p1;
    const ang=Math.atan2(-dy,dx<0?-1:1)*180/Math.PI; // negate dy: price up = screen up
    const slope=dx?((sh.p2-sh.p1)/(sh.p2>0?sh.p2:1)*100).toFixed(4):'∞';
    return`P1: ${fp(sh.p1)}<br>P2: ${fp(sh.p2)}<br>Δ: ${fp(Math.abs(sh.p2-sh.p1))}<br>Angle: ${ang.toFixed(1)}°`;
  }
}
...
document.addEventListener('click',function(e){
  ...
  const sp=document.getElementById('ss-popup');
  if(sp&&sp.style.display==='flex'&&!sp.contains(e.target)&&e.target.id!=='draw-canvas')
    sp.style.display='none';
});

// ── Coordinate helpers ─────────────────────────────────────────────
// _screenToChart converts a screen pixel (cx,cy) → {price, time, px, py}
function _screenToChart(canvas,cx,cy){
...
function _priceToY(p){ return candleSeries?candleSeries.priceToCoordinate(p):null; }
function _timeToX(t) { return chart?chart.timeScale().timeToCoordinate(t):null; }
...
// ── Line dash helper ───────────────────────────────────────────────
function _lineDash(style,thick){
  if(style==='dashed') return [thick*4,thick*3];
  if(style==='dotted') return [thick,thick*2];
  return [];
}

// ── Compute extended trendline endpoints for "ray" mode ──────────
// When ray=true, extends from p1 through p2 to the right canvas edge.
function _trendEndpoints(sh,W){
  const x1=_timeToX(sh.t1), y1=_priceToY(sh.p1);
  const x2=_timeToX(sh.t2), y2=_priceToY(sh.p2);
  if(x1==null||x2==null||y1==null||y2==null)return null;
  if(!sh.ray)return{x1,y1,x2,y2};
  // Extend to x=W (right edge)
  const dx=x2-x1,dy=y2-y1;
  if(dx===0)return{x1,y1,x2:x2,y2:W>x2?(y2+(W-x2)*dy/dx):y2};
  const scale=(W-x1)/dx;
  return{x1,y1,x2:x1+dx*scale,y2:y1+dy*scale};
}

// ── Render one shape ───────────────────────────────────────────────
function _drawShape(ctx,sh,isSel,isPrev,W,H){
  ctx.save();
  if(sh.type==='rect'){
    const x1=_timeToX(sh.t1),y1=_priceToY(sh.p1);
    const x2=_timeToX(sh.t2),y2=_priceToY(sh.p2);
    ...
    const col=isPrev?'#4f6ef7':(sh.borderColor||'#4f6ef7');
    const op =(sh.opacity??12)/100;

    // Fill
    ctx.fillStyle=isPrev?'rgba(79,110,247,.08)':_hexA(col,op);
    ctx.fillRect(lx,ty,w,h);

    // Border
    ctx.strokeStyle=isPrev?'rgba(79,110,247,.65)':col;
    ctx.lineWidth=isPrev?1.5:(sh.thickness||2);
    ...
    if(isSel&&!isPrev){
      const hr=HANDLE_R;
      const handles=[
        [lx,ty,'tl'],[rx,ty,'tr'],[lx,by,'bl'],[rx,by,'br'],  // corners
        [(lx+rx)/2,ty,'tm'],[(lx+rx)/2,by,'bm'],               // edge mid
        [lx,(ty+by)/2,'lm'],[rx,(ty+by)/2,'rm'],
      ];
      ...
      // Lock badge
      if(sh.locked){ctx.font='12px sans-serif';ctx.fillText('🔒',lx+4,ty+14);}
    }

    // Price label when selected
    if(isSel&&!isPrev&&h>20){
      const pHigh=Math.max(sh.p1,sh.p2),pLow=Math.min(sh.p1,sh.p2);
      const fp=v=>{const n=Number(v);return n>=1?n.toFixed(4):n.toFixed(8);};
      ctx.font='600 10px JetBrains Mono,monospace';
      ctx.fillStyle='rgba(255,255,255,.75)';
      ctx.fillText(fp(pHigh),lx+5,ty+13);
      ctx.fillText(fp(pLow) ,lx+5,by-5);
    }

  } else if(sh.type==='trend'){
    const ep=_trendEndpoints(sh,W||2000);
    if(!ep){ctx.restore();return;}
    const{x1,y1,x2,y2}=ep;
    const col=isPrev?'rgba(255,201,60,.7)':(sh.color||'#ffc93c');
    const thick=isPrev?1.8:(sh.thickness||2);

    ctx.strokeStyle=col;
    ctx.lineWidth=thick;
    ctx.setLineDash(isPrev?[6,3]:_lineDash(sh.style||'solid',thick));
    ctx.beginPath();ctx.moveTo(x1,y1);ctx.lineTo(x2,y2);ctx.stroke();
    ctx.setLineDash([]);

    // Endpoints (always use actual t1/p1 and t2/p2 coords, not ray extension)
    const rx1=_timeToX(sh.t1),ry1=_priceToY(sh.p1);
    const rx2=_timeToX(sh.t2),ry2=_priceToY(sh.p2);

    // Handles when selected
    if(isSel&&!isPrev&&rx1!=null&&rx2!=null){
      [[rx1,ry1,'p1'],[rx2,ry2,'p2']].forEach(([hx,hy])=>{
        ctx.fillStyle='#fff';ctx.strokeStyle=col;ctx.lineWidth=2;
      ...
      ctx.fillText(ang.toFixed(1)+'°',mx+6,my-6);
      // Lock badge
      if(sh.locked){ctx.font='12px sans-serif';ctx.fillText('🔒',rx1+6,ry1-8);}
    }

    // Small dot at each endpoint for the ray arrow
    if(sh.ray&&rx2!=null&&ry2!=null){
      ctx.fillStyle=col;
      ctx.beginPath();ctx.arc(rx2,ry2,3,0,Math.PI*2);ctx.fill();
      // Arrow head along extension
      const dx=x2-rx2,dy=y2-ry2,len=Math.hypot(dx,dy)||1;
      if(len>20){
...
function redrawCanvas(){
  ...
  shapes.forEach((sh,i)=>_drawShape(ctx,sh,i===selectedIdx,false,W,H));
  if(previewShape)_drawShape(ctx,previewShape,false,true,W,H);
}

// ── Info tooltip ───────────────────────────────────────────────────
function _showInfoTip(html,canvasX,canvasY){
  const tip=document.getElementById('draw-info-tip');
  if(!tip||!html)return;
  tip.innerHTML=html;
  tip.style.display='block';
  // tip is inside .chart-wrap (position:relative), so left/top are canvas-relative px
  let tx=canvasX+14, ty=Math.max(8,canvasY-10);
  const cw=document.getElementById('draw-canvas');
  const maxX=cw?cw.offsetWidth-170:400;
  if(tx>maxX)tx=canvasX-170;
  tip.style.left=tx+'px'; tip.style.top=ty+'px';
}
function _hideInfoTip(){
  const t=document.getElementById('draw-info-tip');
  if(t)t.style.display='none';
}

// ── Hit testing ────────────────────────────────────────────────────
// Returns {hit: true, handleId:'tl'|'p1'|...} or {hit:false}
function _hitTestShape(sh,cx,cy){
  const T=HIT_R;
  if(sh.type==='rect'){
    const x1=_timeToX(sh.t1),y1=_priceToY(sh.p1);
    const x2=_timeToX(sh.t2),y2=_priceToY(sh.p2);
    if(x1==null||x2==null||y1==null||y2==null)return{hit:false};
    const lx=Math.min(x1,x2),rx=Math.max(x1,x2),ty=Math.min(y1,y2),by=Math.max(y1,y2);
    // Corner handles (highest priority)
    const corners=[['tl',lx,ty],['tr',rx,ty],['bl',lx,by],['br',rx,by]];
    for(const[id,hx,hy]of corners){
      if(Math.hypot(cx-hx,cy-hy)<T)return{hit:true,handleId:id};
    }
    // Edge mid handles
    const mids=[['tm',(lx+rx)/2,ty],['bm',(lx+rx)/2,by],
                ['lm',lx,(ty+by)/2],['rm',rx,(ty+by)/2]];
    for(const[id,hx,hy]of mids){
      if(Math.hypot(cx-hx,cy-hy)<T)return{hit:true,handleId:id};
    }
    // Interior (body) hit → move
    if(cx>=lx-T&&cx<=rx+T&&cy>=ty-T&&cy<=by+T)return{hit:true,handleId:'body'};
    return{hit:false};
  } else if(sh.type==='trend'){
    const x1=_timeToX(sh.t1),y1=_priceToY(sh.p1);
    const x2=_timeToX(sh.t2),y2=_priceToY(sh.p2);
    if(x1==null||x2==null||y1==null||y2==null)return{hit:false};
    // Endpoint handles
    if(Math.hypot(cx-x1,cy-y1)<T)return{hit:true,handleId:'p1'};
    if(Math.hypot(cx-x2,cy-y2)<T)return{hit:true,handleId:'p2'};
    // Line body
    const dx=x2-x1,dy=y2-y1,len2=dx*dx+dy*dy;
    if(len2===0)return{hit:Math.hypot(cx-x1,cy-y1)<T,handleId:'body'};
    const t=Math.max(0,Math.min(1,((cx-x1)*dx+(cy-y1)*dy)/len2));
    if(Math.hypot(cx-x1-t*dx,cy-y1-t*dy)<T)return{hit:true,handleId:'body'};
    return{hit:false};
  }
  return{hit:false};
}
...
function findShapeAt(cx,cy){
  // Search newest first so top-most shape wins
  for(let i=shapes.length-1;i>=0;i--){
    const r=_hitTestShape(shapes[i],cx,cy);
    if(r.hit)return{idx:i,handleId:r.handleId};
  }
  return{idx:-1,handleId:null};
}

// ── Main pointer/touch handler ─────────────────────────────────────
function initDrawCanvas(){
  ...
  // ── Drag state ────────────────────────────────────────────────
  let pDown     = false;
  let startCX   = 0, startCY = 0;    // screen coords at pointer-down
  let dragMode  = 'none';            // 'none'|'draw'|'move'|'resize'
  let dragHandle= null;              // which handle is being dragged
  let drawStart = null;              // {time,price} for first draw point
  let origShapes= null;              // deep snapshot of shapes[] at drag start (for undo)
  let lastTapMs = 0, lastTapHit=-1; // double-tap tracking

  // Return canvas-relative coords from any event
  function _cp(e){
    const s=e.touches?e.touches[0]:(e.changedTouches?e.changedTouches[0]:e);
    return{cx:s.clientX,cy:s.clientY};
  }

  // Snapshot shapes for move/resize undo fallback
  function _snapshot(){return shapes.map(s=>({...s}));}

  // ── Cursor hinting based on what's under the pointer ──────────
  function _updateCursor(cx,cy){
    if(drawTool&&drawTool!=='select'){canvas.style.cursor='crosshair';return;}
    const bb=canvas.getBoundingClientRect();
    const lx=cx-bb.left,ly=cy-bb.top;
    if(selectedIdx>=0){
      const r=_hitTestShape(shapes[selectedIdx],lx,ly);
      if(r.hit){
        if(r.handleId==='body')  {canvas.style.cursor='move';return;}
        if(['tl','br'].includes(r.handleId)){canvas.style.cursor='nwse-resize';return;}
        if(['tr','bl'].includes(r.handleId)){canvas.style.cursor='nesw-resize';return;}
        if(['tm','bm'].includes(r.handleId)){canvas.style.cursor='ns-resize';return;}
        if(['lm','rm'].includes(r.handleId)){canvas.style.cursor='ew-resize';return;}
        canvas.style.cursor='crosshair';return;
      }
    }
    const found=findShapeAt(lx,ly);
    canvas.style.cursor=found.idx>=0?'pointer':'default';
  }
  ...
  function onDown(e){
    ...
    pDown=true;dragMode='none';drawStart=null;origShapes=null;

    // Close settings popup on canvas tap
    const sp=document.getElementById('ss-popup');
    if(sp&&sp.style.display==='flex')return; // let popup handle its own events

    if(drawTool==='rect'||drawTool==='trend'){
      // Begin drawing
      const c=_screenToChart(canvas,cx,cy);
      if(c){drawStart=c;dragMode='draw';}
    ...
    // First: check if we're hitting a handle on the already-selected shape
    if(selectedIdx>=0&&selectedIdx<shapes.length){
      const sh=shapes[selectedIdx];
      if(!sh.locked){
        const r=_hitTestShape(sh,lx,ly);
        if(r.hit&&r.handleId!=='body'){
          dragMode='resize';dragHandle=r.handleId;
          origShapes=_snapshot();
          if(e.cancelable)e.preventDefault();
          return;
        }
        if(r.hit&&r.handleId==='body'){
          dragMode='move';origShapes=_snapshot();
          if(e.cancelable)e.preventDefault();
          return;
        }
      }
    ...
    // Hit any shape
    const{idx,handleId}=findShapeAt(lx,ly);
    if(idx>=0){
      selectedIdx=idx;
      _setActiveToolUI(drawTool); // re-enable canvas pointer events
      if(!shapes[idx].locked){
        if(handleId&&handleId!=='body'){
          dragMode='resize';dragHandle=handleId;origShapes=_snapshot();
        } else {
          dragMode='move';origShapes=_snapshot();
        }
      }
  ...
  function onMove(e){
    ...
    // Update cursor (only on mouse, not touch)
    if(!e.touches)_updateCursor(cx,cy);

    if(!pDown)return;
    const delta=Math.hypot(cx-startCX,cy-startCY);
    if(delta<2)return;  // dead zone

    if(e.cancelable)e.preventDefault();

    // ── DRAW mode: update preview ────────────────────────────────
    if(dragMode==='draw'&&drawStart){
      const c=_screenToChart(canvas,cx,cy);
      if(!c)return;
      previewShape=drawTool==='rect'
        ?_defaultRect(drawStart.time,drawStart.price,c.time,c.price)
        :_defaultTrend(drawStart.time,drawStart.price,c.time,c.price);
      redrawCanvas();
      return;
    }
    ...
    // ── MOVE mode: translate whole shape ─────────────────────────
    if(dragMode==='move'){
      const orig=origShapes[selectedIdx];
      // Compute delta in price-space by mapping orig screen pos + pixel delta back
      const ox1=_timeToX(orig.t1), oy1=_priceToY(orig.p1);
      if(ox1==null||oy1==null)return;
      const nc1=_screenToChart(canvas,ox1+bb.left+(cx-startCX),oy1+bb.top+(cy-startCY));
      if(!nc1)return;
      sh.t1=nc1.time; sh.p1=nc1.price;
      if(orig.t2!=null){
        const ox2=_timeToX(orig.t2),oy2=_priceToY(orig.p2);
        if(ox2==null||oy2==null)return;
        const nc2=_screenToChart(canvas,ox2+bb.left+(cx-startCX),oy2+bb.top+(cy-startCY));
        if(!nc2)return;
        sh.t2=nc2.time; sh.p2=nc2.price;
      }
      redrawCanvas();
      return;
    }

    // ── RESIZE mode: drag a specific handle ──────────────────────
    if(dragMode==='resize'){
      const c=_screenToChart(canvas,cx,cy);
      if(!c)return;
      if(sh.type==='trend'){
        if(dragHandle==='p1'){sh.t1=c.time;sh.p1=c.price;}
        else                 {sh.t2=c.time;sh.p2=c.price;}
      } else if(sh.type==='rect'){
        // Each corner/edge handle controls specific t/p components
        const nh=dragHandle;
        // Corners: recompute t1,p1,t2,p2 preserving the opposite corner
        const orig=origShapes[selectedIdx];
        const ox1=_timeToX(orig.t1),oy1=_priceToY(orig.p1);
        const ox2=_timeToX(orig.t2),oy2=_priceToY(orig.p2);
        if(ox1==null||ox2==null||oy1==null||oy2==null)return;
        const lx2=Math.min(ox1,ox2),rx2=Math.max(ox1,ox2),
              ty2=Math.min(oy1,oy2),by2=Math.max(oy1,oy2);

        if(nh==='tl'){
          // drag top-left: t1/p1 move, t2/p2 fixed at br
          sh.t1=c.time; sh.p1=c.price;
          const nc2=_screenToChart(canvas,rx2+bb.left,by2+bb.top);
          if(nc2){sh.t2=nc2.time;sh.p2=nc2.price;}
        } else if(nh==='tr'){
          const nc2=_screenToChart(canvas,lx2+bb.left,by2+bb.top);
          if(nc2){sh.t1=nc2.time;sh.p1=nc2.price;}
          sh.t2=c.time; sh.p2=c.price;
        } else if(nh==='bl'){
          sh.t1=c.time; sh.p1=c.price;
          const nc2=_screenToChart(canvas,rx2+bb.left,ty2+bb.top);
          if(nc2){sh.t2=nc2.time;sh.p2=nc2.price;}
        } else if(nh==='br'){
          const nc2=_screenToChart(canvas,lx2+bb.left,ty2+bb.top);
          if(nc2){sh.t1=nc2.time;sh.p1=nc2.price;}
          sh.t2=c.time; sh.p2=c.price;
        } else if(nh==='tm'){
          // top-mid: only move top price
          const topOrig=_screenToChart(canvas,(lx2+rx2)/2+bb.left,ty2+bb.top);
          if(topOrig){
            // shift p1 or p2 whichever is the top
            if(orig.p1>orig.p2) sh.p1=c.price; else sh.p2=c.price;
          }
        } else if(nh==='bm'){
          if(orig.p1<orig.p2) sh.p1=c.price; else sh.p2=c.price;
        } else if(nh==='lm'){
          // left-mid: only move left time
          if(orig.t1<orig.t2) sh.t1=c.time; else sh.t2=c.time;
        } else if(nh==='rm'){
          if(orig.t1>orig.t2) sh.t1=c.time; else sh.t2=c.time;
        }
      }
  ...
  function onUp(e){
    ...
    // Finish draw
    if(dragMode==='draw'&&previewShape&&moved){
      shapes.push({...previewShape});
      selectedIdx=shapes.length-1;
      previewShape=null;drawStart=null;
      // Auto-exit draw mode → back to select so chart panning isn't blocked
      drawTool=null;_setActiveToolUI(null);
      redrawCanvas();
      return;
    }
    ...
    // Single/double tap logic (no drag occurred)
    if(!moved){
      const bb=canvas.getBoundingClientRect();
      const lx=cx-bb.left,ly=cy-bb.top;
      const{idx}=findShapeAt(lx,ly);
      const now=Date.now();

      if(idx>=0&&idx===lastTapHit&&now-lastTapMs<420){
        // Double tap → open settings
        lastTapMs=0;lastTapHit=-1;
        openShapeSettings(idx);
      } else {
        selectedIdx=idx>=0?idx:-1;
        lastTapMs=now;lastTapHit=idx;
        _setActiveToolUI(drawTool);
        // Show info tip on single select
        if(idx>=0){
          const info=_shapeInfo(shapes[idx]);
          _showInfoTip(info,lx,ly);
          // hide after 3s
          setTimeout(_hideInfoTip,3000);
        } else {
          _hideInfoTip();
        }
        redrawCanvas();
  ...
  // ── Shift+drag on a trendline = parallel copy ─────────────────
  // Handled in onDown: if shift held and we hit a trend body, clone it
  function onDownShift(e){
    if(!e.shiftKey)return false;
    const{cx,cy}=_cp(e);
    const bb=canvas.getBoundingClientRect();
    const lx=cx-bb.left,ly=cy-bb.top;
    const{idx,handleId}=findShapeAt(lx,ly);
    if(idx<0||shapes[idx].type!=='trend'||handleId!=='body')return false;
    // Clone the shape
    const clone={...shapes[idx]};
    shapes.push(clone);
    selectedIdx=shapes.length-1;
    origShapes=_snapshot();
    dragMode='move';
  ...
  // Attach events — unified mouse + touch approach
  canvas.addEventListener('mousedown',e=>{
    if(!onDownShift(e))onDown(e);
  },{passive:false});
  ...
  // Note: chart pan/zoom subscriptions are set up in loadChart() after initDrawCanvas() is called.
}
