...
function _showInfoTip(html,canvasX,canvasY){
  ...
  if(!cw)return;
  const bb=cw.getBoundingClientRect();
  let tx=bb.left+canvasX+14, ty=bb.top+canvasY-10;
  // keep in viewport
  if(tx+160>window.innerWidth)tx=bb.left+canvasX-170;
