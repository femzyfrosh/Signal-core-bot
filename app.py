...
//
//  Shape schema:
//    rect  → {type,t1,p1,t2,p2, borderColor,fillColor,
//              lineWidth, fillOpacity, midLine, locked}
//    trend → {type,t1,p1,t2,p2, color, lineWidth,
//              lineStyle ('solid'|'dashed'|'dotted'),
//              extendRight, extendLeft, locked}
//
//  Interaction states:
//    drawTool  = 'rect'|'trend'|'select'|null
//    interactionMode = 'idle'|'drawing'|'moving'|'resizing'
//    resizeHandle  = null | 0-3 (rect corner) | 0-1 (trend endpoint)
//    parallelGhost = shape being created as Shift+drag parallel clone
...
// ── interaction state ──────────────────────────────────────────
let _intMode         = 'idle';   // 'idle'|'drawing'|'moving'|'resizing'
let _resizeHandle    = null;     // which handle is being dragged
let _moveSnap        = null;     // deep copy of shape at drag-start
let _dragStartChart  = null;     // {time,price} at pointer-down
let _parallelPending = false;    // Shift held at trend drag-start → parallel clone
let _parallelShape   = null;     // the live parallel ghost

const SHAPE_COLORS = ['#4f6ef7','#00d4aa','#ff4d6a','#ffc93c','#a855f7','#f97316','#e2e8f0','#06b6d4'];
const HANDLE_R     = 6;   // handle circle radius px
const HIT_T        = 12;  // hit-test tolerance px
const HIT_CORNER   = 14;  // corner handle hit radius px
...
// Convert screen (clientX/Y) → {time,price,px,py} in chart space
...
// ── Line-dash mapping ──────────────────────────────────────────
function _dashForStyle(style,lw){
  if(style==='dashed')return[lw*4+2,lw*2+2];
  if(style==='dotted')return[1,lw*3+2];
  return[];
}

// ── Compute the angle of a trendline in degrees ───────────────
function _trendAngle(x1,y1,x2,y2){
  return Math.round(Math.atan2(-(y2-y1),(x2-x1))*180/Math.PI);
}

// ── Extend a trend line to canvas edges ───────────────────────
function _extendedEndpoints(x1,y1,x2,y2,W,H,extRight,extLeft){
  // Returns {sx,sy,ex,ey} after applying extension flags
  let sx=x1,sy=y1,ex=x2,ey=y2;
  const dx=x2-x1,dy=y2-y1;
  if(dx===0&&dy===0)return{sx,sy,ex,ey};

  function xAtY(yy){return x1+(yy-y1)*dx/dy;}
  function yAtX(xx){return y1+(xx-x1)*dy/dx;}
...
// ── Render one shape ───────────────────────────────────────────
function _drawShape(ctx,sh,isSel,isPrev){
  if(!ctx)return;
  ...
  if(sh.type==='rect'){
    ...
    const bColor=isPrev?'rgba(79,110,247,.7)':(sh.borderColor||'#4f6ef7');
    const fOpacity=isPrev?0.08:(sh.fillOpacity!=null?sh.fillOpacity/100:0.12);
    const lw=sh.lineWidth||1.5;

    ctx.save();
    // Fill
    ctx.fillStyle=isPrev?`rgba(79,110,247,${fOpacity})`:_hexA(sh.fillColor||'#4f6ef7',fOpacity);
    ...
    if(isSel&&!isPrev){
      ...
      // Price labels on the edges
      ctx.font='500 10px JetBrains Mono,monospace';
      ctx.fillStyle=bColor;
      const topP=_priceFromY(ty),botP=_priceFromY(by);
      if(topP!=null){ctx.fillText(_fmtPrice(topP),rx+6,ty+4);}
      if(botP!=null){ctx.fillText(_fmtPrice(botP),rx+6,by+4);}
      // Width/Height info badge
      const heightPct=topP!=null&&botP!=null?Math.abs(((topP-botP)/botP)*100):null;
      if(heightPct!=null){
        const txt=heightPct.toFixed(2)+'%';
        ctx.font='700 10px JetBrains Mono,monospace';
        const tw=ctx.measureText(txt).width;
        const bx=lx+(rx-lx)/2-tw/2-4, by2=ty-8;
        ctx.fillStyle='rgba(7,8,16,.75)';
        ctx.fillRect(bx,by2-11,tw+8,14);
        ctx.fillStyle=bColor;
        ctx.fillText(txt,bx+4,by2);
      }
    }
    ctx.restore();
  ...
  // ── Trendline ──────────────────────────────────────────────
    ...
    const color=isPrev?'rgba(255,201,60,.7)':(sh.color||'#ffc93c');
    const lw=sh.lineWidth||(isSel?2.5:1.8);

    ctx.save();
    ctx.strokeStyle=color;
    ctx.lineWidth=lw;

    // Apply line style dash
    if(isPrev){
      ctx.setLineDash([6,3]);
    }else{
      ctx.setLineDash(_dashForStyle(sh.lineStyle||'solid',lw));
    }

    // Compute extended endpoints
    const ext=_extendedEndpoints(x1,y1,x2,y2,W,H,
      !isPrev&&!!sh.extendRight, !isPrev&&!!sh.extendLeft);
    ...
    if(isSel&&!isPrev){
      ...
      // Angle label near p2
      const angle=_trendAngle(x1,y1,x2,y2);
      const txt=angle+'°';
      ctx.font='700 10px JetBrains Mono,monospace';
      const tw=ctx.measureText(txt).width;
      const lbx=x2+10, lby=y2-8;
      ctx.fillStyle='rgba(7,8,16,.8)';
      ctx.fillRect(lbx-2,lby-11,tw+8,14);
      ctx.fillStyle=color;
      ctx.fillText(txt,lbx+2,lby);

      // Price labels at endpoints
      const p1v=sh.p1,p2v=sh.p2;
      ctx.font='500 9.5px JetBrains Mono,monospace';
      ctx.fillStyle=color;
      if(p1v!=null)ctx.fillText(_fmtPrice(p1v),x1+8,y1-6);
      if(p2v!=null)ctx.fillText(_fmtPrice(p2v),x2+8,y2+14);

      // Extend-right arrow hint
      if(sh.extendRight){
        ctx.beginPath();
        ctx.strokeStyle=color;ctx.lineWidth=1;
        ctx.setLineDash([3,3]);
        ctx.moveTo(x2,y2);ctx.lineTo(Math.min(W,x2+40),y2+((y2-y1)/(x2-x1||1))*40);
        ctx.stroke();ctx.setLineDash([]);
      }
    }
    ctx.restore();
...
// Reverse coordinate helper (pixel→price)
function _priceFromY(py){return candleSeries?candleSeries.coordinateToPrice(py):null;}
...
function redrawCanvas(){
  ...
  shapes.forEach((sh,i)=>_drawShape(ctx,sh,i===selectedIdx,false));
  if(previewShape)_drawShape(ctx,previewShape,false,true);
  // Draw parallel ghost if in progress
  if(_parallelShape)_drawShape(ctx,_parallelShape,false,true);
}

// ── Hit test ──────────────────────────────────────────────────
//  Returns: false | 'body' | 'corner-0'..'corner-3' | 'ep-0' | 'ep-1'
function _hitTestDetail(sh,cx,cy){
  if(sh.type==='rect'){
    const x1=_timeToX(sh.t1),y1=_priceToY(sh.p1);
    const x2=_timeToX(sh.t2),y2=_priceToY(sh.p2);
    ...
    for(let i=0;i<corners.length;i++){
      if(Math.hypot(cx-corners[i][0],cy-corners[i][1])<HIT_CORNER)return`corner-${i}`;
    }
    // Body
    if(cx>=lx-HIT_T&&cx<=rx+HIT_T&&cy>=ty-HIT_T&&cy<=by+HIT_T)return'body';
    return false;
    ...
    if(Math.hypot(cx-x1,cy-y1)<HIT_CORNER)return'ep-0';
    if(Math.hypot(cx-x2,cy-y2)<HIT_CORNER)return'ep-1';
    // Line body
    const dx=x2-x1,dy=y2-y1,len2=dx*dx+dy*dy;
    if(len2===0)return Math.hypot(cx-x1,cy-y1)<HIT_T?'body':false;
    const t=Math.max(0,Math.min(1,((cx-x1)*dx+(cy-y1)*dy)/len2));
    return Math.hypot(cx-x1-t*dx,cy-y1-t*dy)<HIT_T?'body':false;
  }
  return false;
}

function findShapeAt(cx,cy){
  for(let i=shapes.length-1;i>=0;i--)if(_hitTestDetail(shapes[i],cx,cy))return i;
  return -1;
}

// ── Cursor based on what's under pointer ──────────────────────
function _updateCursor(canvas,cx,cy){
  if(drawTool==='rect'||drawTool==='trend'){canvas.style.cursor='crosshair';return;}
  if(drawTool==='select'||drawTool===null){
    for(let i=shapes.length-1;i>=0;i--){
      const h=_hitTestDetail(shapes[i],cx,cy);
      if(!h)continue;
...
function _setActiveToolUI(tool){
  ['tp-trend','tp-rect','tp-sel'].forEach(id=>{
    ...
    if(el)el.classList.toggle('active',
      (tool==='trend'&&id==='tp-trend')||
      (tool==='rect' &&id==='tp-rect' )||
      (tool==='select'&&id==='tp-sel'));
  });
  ...
  if(canvas){
    ...
    canvas.style.cursor=(tool==='select'||!tool)?'default':'crosshair';
  }
}
...
window.activateTool=function(tool){
  ...
  _intMode='idle';_resizeHandle=null;
  document.getElementById('tools-popup').style.display='none';
  _setActiveToolUI(tool);redrawCanvas();
};
...
window.clearAllShapes=function(){
  ...
  _intMode='idle';
  document.getElementById('tools-popup').style.display='none';
  _setActiveToolUI(null);redrawCanvas();
};

// ── Thickness / style selectors (called from HTML buttons) ─────
window.selectThickness=function(w){
  document.querySelectorAll('.ss-thick-btn').forEach(b=>{
...
window.selectLineStyle=function(s){
  document.querySelectorAll('.ss-style-btn').forEach(b=>{
    b.classList.toggle('sel',b.dataset.style===s);
  });
};
...
window.applyShapeSettings=function(){
  ...
  if(sh.type==='rect'){
    ...
    sh.lineStyle=document.querySelector('.ss-style-btn.sel')?.dataset.style||'solid';
  ...
  // Thickness (shared)
  const thickBtn=document.querySelector('.ss-thick-btn.sel');
  if(thickBtn)sh.lineWidth=parseFloat(thickBtn.dataset.w);
...
function openShapeSettings(idx){
  ...
  const fillRow  =document.getElementById('ss-fill-row');
  const midRow   =document.getElementById('ss-midline-row');
  const opacRow  =document.getElementById('ss-opacity-row');
  const styleRow =document.getElementById('ss-style-row');
  const extRow   =document.getElementById('ss-extend-row');
  const extLRow  =document.getElementById('ss-extleft-row');
  const infoRow  =document.getElementById('ss-info-row');

  if(isRect){
    fillRow.style.display='block';
    midRow.style.display='flex';
    opacRow.style.display='block';
    styleRow.style.display='none';
    extRow.style.display='none';
    extLRow.style.display='none';
    buildPalette('ss-fill-palette',sh.fillColor||'#4f6ef7');
    document.getElementById('ss-midline').checked=!!sh.midLine;
    const opEl=document.getElementById('ss-opacity');
    const opVal=document.getElementById('ss-opacity-val');
    opEl.value=sh.fillOpacity!=null?sh.fillOpacity:12;
    opVal.textContent=opEl.value+'%';
    opEl.oninput=()=>opVal.textContent=opEl.value+'%';

    // Info row: price levels + height %
    const p1=sh.p1, p2=sh.p2;
    if(p1!=null&&p2!=null){
      const hi=Math.max(p1,p2),lo=Math.min(p1,p2);
      const pct=Math.abs(((hi-lo)/lo)*100).toFixed(2);
      infoRow.innerHTML=`Top: ${_fmtPrice(hi)}<br>Bot: ${_fmtPrice(lo)}<br>Height: ${pct}%`;
      infoRow.className='ss-info-row show';
    }else{infoRow.className='ss-info-row';}
  }else{
    ...
    // Mark current line style
    selectLineStyle(sh.lineStyle||'solid');

    // Info row: angle
    const x1=_timeToX(sh.t1),y1=_priceToY(sh.p1);
    const x2=_timeToX(sh.t2),y2=_priceToY(sh.p2);
    const angle=(x1!=null&&x2!=null)?_trendAngle(x1,y1,x2,y2):null;
    infoRow.innerHTML=`P1: ${_fmtPrice(sh.p1)}<br>P2: ${_fmtPrice(sh.p2)}`+(angle!=null?`<br>Angle: ${angle}°`:'');
    infoRow.className='ss-info-row show';
  }

  // Thickness — mark current
  const curW=sh.lineWidth||1.5;
  document.querySelectorAll('.ss-thick-btn').forEach(b=>{
    b.classList.toggle('sel',parseFloat(b.dataset.w)===curW||
      (curW<1.6&&b.dataset.w==='1')||(curW>=1.6&&curW<2.5&&b.dataset.w==='2')||(curW>=2.5&&b.dataset.w==='3'));
  });
  // If none selected, default to 1
  if(!document.querySelector('.ss-thick-btn.sel'))
    document.querySelector('.ss-thick-btn[data-w="1"]').classList.add('sel');

  const popup=document.getElementById('ss-popup');
  popup.style.cssText+='display:flex;top:50%;left:50%;transform:translate(-50%,-50%)';
}

document.addEventListener('click',function(e){
  const tp=document.getElementById('tools-popup');
  if(tp&&tp.style.display==='flex'&&!tp.contains(e.target)&&e.target.id!=='pen-btn')tp.style.display='none';
});
...
function initDrawCanvas(){
  ...
  function _canvasXY(cx,cy){
    const bb=canvas.getBoundingClientRect();
    return{x:cx-bb.left,y:cy-bb.top};
  }

  // ── Mouse-move: cursor feedback even before press ──────────
  canvas.addEventListener('mousemove',function(e){
    if(!pDown){
      const{cx,cy}=_cp(e);
      const{x,y}=_canvasXY(cx,cy);
      _updateCursor(canvas,x,y);
    }
  });
  ...
  function onDown(e){
    ...
    const{x,y}=_canvasXY(cx,cy);
    const chartPt=_screenToChart(canvas,cx,cy);

    // ── Drawing mode ──────────────────────────────────────
    if(drawTool==='rect'||drawTool==='trend'){
      if(chartPt){
        _dragStartChart=chartPt;
        _intMode='drawing';
      }
    ...
    const hitIdx=findShapeAt(x,y);

    if(hitIdx>=0){
      selectedIdx=hitIdx;
      const hitType=_hitTestDetail(shapes[hitIdx],x,y);

      // Check if locked
      if(shapes[hitIdx].locked){
        redrawCanvas();
        if(e.cancelable)e.preventDefault();
        return;
      }
      ...
      if(hitType==='body'){
        // ── Move ─────────────────────────────────────────
        _intMode='moving';
        _moveSnap=JSON.parse(JSON.stringify(shapes[hitIdx]));
        _dragStartChart=chartPt;

        // Trendline: Shift held → create parallel clone
        if(shapes[hitIdx].type==='trend'&&(e.shiftKey||e.ctrlKey)){
  ...
  function onMove(e){
    if(!pDown)return;
    const{cx,cy}=_cp(e);
    const moved=Math.hypot(cx-startCX,cy-startCY)>3;
    if(!moved)return;
    ...
    // ── Drawing preview ─────────────────────────────────
    if(_intMode==='drawing'&&_dragStartChart&&chartPt){
      const ds=_dragStartChart;
      if(drawTool==='rect'){
        previewShape={type:'rect',
          t1:ds.time,p1:ds.price,t2:chartPt.time,p2:chartPt.price,
          borderColor:'#4f6ef7',fillColor:'#4f6ef7',midLine:false,
          lineWidth:1.5,fillOpacity:12};
      }else{
        previewShape={type:'trend',
          t1:ds.time,p1:ds.price,t2:chartPt.time,p2:chartPt.price,
          color:'#ffc93c',lineWidth:1.8,lineStyle:'solid',
    ...
    if(!chartPt||selectedIdx<0)return;
    const sh=shapes[selectedIdx];

    // ── Move ────────────────────────────────────────────
    if(_intMode==='moving'&&_moveSnap&&_dragStartChart){
      const dtTime=chartPt.time-_dragStartChart.time;
      const dtPrice=chartPt.price-_dragStartChart.price;
      sh.t1=_moveSnap.t1+dtTime;
      sh.p1=_moveSnap.p1+dtPrice;
      sh.t2=_moveSnap.t2+dtTime;
      sh.p2=_moveSnap.p2+dtPrice;

      // Parallel ghost for trend lines (Shift held)
      if(_parallelPending&&sh.type==='trend'){
        if(!_parallelShape){
          // On first significant move, spawn the parallel clone at original position
          _parallelShape=JSON.parse(JSON.stringify(_moveSnap));
        }
        // parallel shape stays at original position — the moved line is the new one
      }
      redrawCanvas();
      return;
    }
    ...
    // ── Resize / endpoint drag ──────────────────────────
    if(_intMode==='resizing'&&_moveSnap&&_resizeHandle){
      const h=_resizeHandle;
      if(sh.type==='rect'){
        // corner-0=topLeft, corner-1=topRight, corner-2=botLeft, corner-3=botRight
        // We track which original corner each handle maps to based on current p1/p2 layout
        const orig=_moveSnap;
        const lx=Math.min(_timeToX(orig.t1)||0,_timeToX(orig.t2)||0);
        const rx=Math.max(_timeToX(orig.t1)||0,_timeToX(orig.t2)||0);
        const ty=Math.min(_priceToY(orig.p1)||0,_priceToY(orig.p2)||0);
        const by=Math.max(_priceToY(orig.p1)||0,_priceToY(orig.p2)||0);
        // Determine which anchor point is fixed (opposite corner)
        // corner-0=[lx,ty], corner-1=[rx,ty], corner-2=[lx,by], corner-3=[rx,by]
        const fixed={
          'corner-0':{x:rx,y:by},'corner-1':{x:lx,y:by},
          'corner-2':{x:rx,y:ty},'corner-3':{x:lx,y:ty}
        }[h];
        if(fixed){
          // Convert fixed corner pixel back to time/price
          const bb=canvas.getBoundingClientRect();
          const fc=_screenToChart(canvas,fixed.x+bb.left,fixed.y+bb.top);
          if(fc){
            sh.t1=fc.time; sh.p1=fc.price;
            sh.t2=chartPt.time; sh.p2=chartPt.price;
          }
        }
      }else if(sh.type==='trend'){
        if(h==='ep-0'){sh.t1=chartPt.time;sh.p1=chartPt.price;}
        else{sh.t2=chartPt.time;sh.p2=chartPt.price;}
      }
      redrawCanvas();
      return;
    }
  ...
  function onUp(e){
    ...
    // ── Commit drawing ──────────────────────────────────
    if(_intMode==='drawing'&&previewShape&&moved){
      shapes.push({...previewShape});
      selectedIdx=shapes.length-1;
      previewShape=null;
      _dragStartChart=null;
      _intMode='idle';
      // Single-shot: stay in draw mode so user can draw multiple
      // shapes without re-activating — matches TradingView behaviour.
      // (Comment this out if you prefer auto-exit.)
    ...
    if(_parallelPending&&_parallelShape&&moved){
      shapes.push(_parallelShape);
      _parallelShape=null;
      _parallelPending=false;
    }
    ...
    if(_intMode==='moving'||_intMode==='resizing'){
      _intMode='idle';
      _moveSnap=null;
      _resizeHandle=null;
      redrawCanvas();
    ...
    // ── Tap: select or double-tap settings ─────────────
    if(!moved){
      const{x,y}=_canvasXY(cx,cy);
      const hit=findShapeAt(x,y);
      const now=Date.now();
      if(hit>=0&&hit===lastTapHit&&now-lastTapMs<400){
        lastTapMs=0;lastTapHit=-1;
        openShapeSettings(hit);
      }else{
        selectedIdx=hit>=0?hit:-1;
        lastTapMs=now;lastTapHit=hit;
        redrawCanvas();
      }
    }
  }
  ...
  // ── Keyboard: Delete selected shape, Escape to exit tool ──
  document.addEventListener('keydown',function(e){
    ...
    if(e.key==='Escape'){
      previewShape=null;drawTool=null;_setActiveToolUI(null);
