...
def get_candles(symbol, interval, limit=150):
    ...
    try:
        ...
        for i in range(len(times)):
            try:
                out.append({"time":int(times[i])//1000,"open":float(opens[i]),
...
async function loadChart(sym,tf){
  ...
  // Reset drawing state for this chart load.
  // Listeners were attached once at startup by initDrawCanvas().
  resetDrawCanvas();
...
// ══════════════════════════════════════════════════════════════════
// ── Drawing Engine v5 — TradingView-style (clean rewrite) ────────
// ══════════════════════════════════════════════════════════════════
//
//  KEY DESIGN DECISIONS:
//  1. Move uses PIXEL delta (not chart-domain delta) so dragging never
//     stalls when the pointer approaches the chart edge where
//     coordinateToTime() returns null.
//  2. Hit-test always runs first — clicking any existing shape selects
//     it even while a draw tool is active.
//  3. drawTool resets to null after each shape is committed, so the
//     next click selects rather than drawing over.
//  4. Coordinate clamping uses the visible time-range midpoint to
//     decide left-vs-right, and actual price scale for top-vs-bottom.

let drawTool    = null;   // 'rect' | 'trend' | null
let shapes      = [];     // committed shape objects
let selectedIdx = -1;     // index of selected shape, -1 = none
let previewShape= null;   // live ghost while drawing
let _mode       = 'idle'; // 'idle'|'drawing'|'moving'|'resizing'
let _dragStart  = null;   // {cx,cy,px,py,cp} at pointer-down
let _snapShape  = null;   // deep-copy of shape at drag-start
let _resizeHandle=null;   // 'corner-0..3' | 'ep-0' | 'ep-1'
let _parallelClone=null;  // ghost for Shift+drag parallel trendline
let _shiftDown  = false;

const COLORS=['#4f6ef7','#00d4aa','#ff4d6a','#ffc93c','#a855f7','#f97316','#e2e8f0','#06b6d4'];
const HR=6, HIT=12, HCORNER=16;

// ── Raw coordinate converters ──────────────────────────────────
...
// Safe converters: clamp off-screen values to canvas edges.
// These are used for hit-testing and drawing — shapes that scroll
// partially off-screen must still be visible and selectable.
function _sx(t){
  const v=_t2x(t); if(v!=null)return v;
  if(!chart)return 0;
  const rng=chart.timeScale().getVisibleRange();
  return(!rng||t<=rng.from)?0:(document.getElementById('draw-canvas')||{width:2000}).width;
}
function _sy(p){
  const v=_p2y(p); if(v!=null)return v;
  if(!candleSeries)return 0;
  // Higher price = smaller y (screen coords are inverted)
  const top=candleSeries.priceToCoordinate(1e18);
  const bot=candleSeries.priceToCoordinate(0);
  const T=top!=null?top:0;
  const B=bot!=null?bot:(document.getElementById('draw-canvas')||{height:2000}).height;
  return p>0?T:B;
}

// Client-space pointer → canvas-local pixels {x,y}
function _canvasXY(canvas,cx,cy){
  const bb=canvas.getBoundingClientRect();
  return{x:cx-bb.left,y:cy-bb.top};
}

// Client-space pointer → chart domain {time,price,px,py}
// Returns null only if chart is uninitialised.
function _xyToChart(canvas,cx,cy){
  ...
  // price can be null near edges — use pixel-fallback price
  const safeprice=price!=null?price:candleSeries.coordinateToPrice(Math.max(0,Math.min(py,bb.height)));
  const safetime=time!=null?time:chart.timeScale().coordinateToTime(Math.max(0,Math.min(px,bb.width)));
  if(safetime==null)return null;
  return{px,py,price:safeprice||0,time:safetime};
}
...
function resizeDrawCanvas(){
  ...
  if(w>0&&h>0&&(c.width!==w||c.height!==h)){c.width=w;c.height=h;c.style.width=w+'px';c.style.height=h+'px';}
}
...
function _fp(p){
  ...
  return p>=1000?p.toLocaleString('en',{maximumFractionDigits:2}):p>=1?p.toFixed(4):p.toFixed(8);
}
function _extendLine(x1,y1,x2,y2,W,H,extR,extL){
  ...
  const yAtX=x=>y1+(x-x1)*(dy/dx);
  const xAtY=y=>x1+(y-y1)*(dx/dy);
  const cH=y=>Math.max(0,Math.min(H,y));
  if(extR&&Math.abs(dx)>0.001){const yr=yAtX(W);if(yr>=0&&yr<=H){ex=W;ey=yr;}else{const yy=cH(yr);ex=xAtY(yy);ey=yy;}}
  if(extL&&Math.abs(dx)>0.001){const yl=yAtX(0);if(yl>=0&&yl<=H){sx=0;sy=yl;}else{const yy=cH(yl);sx=xAtY(yy);sy=yy;}}
  return{sx,sy,ex,ey};
}

// ── Hit test ───────────────────────────────────────────────────
// Returns false | 'body' | 'corner-0..3' | 'ep-0' | 'ep-1'
function _hit(sh,x,y){
  if(sh.type==='rect'){
    const x1=_sx(sh.t1),y1=_sy(sh.p1),x2=_sx(sh.t2),y2=_sy(sh.p2);
    const lx=Math.min(x1,x2),rx=Math.max(x1,x2),ty=Math.min(y1,y2),by=Math.max(y1,y2);
    // Corners first (TL,TR,BL,BR)
    const corners=[[lx,ty],[rx,ty],[lx,by],[rx,by]];
    for(let i=0;i<4;i++)if(Math.hypot(x-corners[i][0],y-corners[i][1])<HCORNER)return`corner-${i}`;
    // Edges
    if(Math.abs(y-ty)<HIT&&x>=lx-HIT&&x<=rx+HIT)return'body';
    if(Math.abs(y-by)<HIT&&x>=lx-HIT&&x<=rx+HIT)return'body';
    if(Math.abs(x-lx)<HIT&&y>=ty-HIT&&y<=by+HIT)return'body';
    if(Math.abs(x-rx)<HIT&&y>=ty-HIT&&y<=by+HIT)return'body';
    // Interior
  ...
  if(sh.type==='trend'){
    ...
    return Math.hypot(x-x1-t*dx,y-y1-t*dy)<HIT?'body':false;
  }
  return false;
}
function _findShape(x,y){for(let i=shapes.length-1;i>=0;i--)if(_hit(shapes[i],x,y))return i;return -1;}

// ── Cursor ─────────────────────────────────────────────────────
function _cursor(canvas,x,y){
  if(drawTool==='rect'||drawTool==='trend'){canvas.style.cursor='crosshair';return;}
  for(let i=shapes.length-1;i>=0;i--){
    const h=_hit(shapes[i],x,y);if(!h)continue;
...
function _draw(ctx,sh,isSel,isPrev){
  ...
  if(sh.type==='rect'){
    const x1=_sx(sh.t1),y1=_sy(sh.p1),x2=_sx(sh.t2),y2=_sy(sh.p2);
    const lx=Math.min(x1,x2),rx=Math.max(x1,x2),ty=Math.min(y1,y2),by=Math.max(y1,y2),w=rx-lx,h=by-ty;
    ...
    ctx.save();
    ctx.fillStyle=_rgba(sh.fillColor||'#4f6ef7',fo);ctx.fillRect(lx,ty,w,h);
    ctx.strokeStyle=bc;ctx.lineWidth=isSel?Math.max(lw,2):lw;
    ctx.setLineDash(isPrev?[5,3]:[]);ctx.strokeRect(lx,ty,w,h);ctx.setLineDash([]);
    if(sh.midLine&&!isPrev){
      ctx.beginPath();ctx.strokeStyle=bc;ctx.lineWidth=Math.max(1,lw*0.6);
      ctx.setLineDash([4,4]);ctx.moveTo(lx,ty+h/2);ctx.lineTo(rx,ty+h/2);ctx.stroke();ctx.setLineDash([]);
    }
    if(isSel&&!isPrev){
      // Corner handles
      [[lx,ty],[rx,ty],[lx,by],[rx,by]].forEach(([hx,hy])=>{
        ctx.beginPath();ctx.arc(hx,hy,HR,0,Math.PI*2);
        ctx.fillStyle=bc;ctx.fill();ctx.strokeStyle='#fff';ctx.lineWidth=1.5;ctx.stroke();
      });
      // Δ% badge
      if(sh.p1!=null&&sh.p2!=null&&Math.min(sh.p1,sh.p2)>0){
        const pct=(Math.abs((sh.p1-sh.p2)/Math.min(sh.p1,sh.p2))*100).toFixed(2)+'%';
        ctx.font='bold 10px JetBrains Mono,monospace';
        const tw=ctx.measureText(pct).width;
        const bx=lx+w/2-(tw+12)/2, by2=Math.max(4,ty-18);
        ctx.fillStyle='rgba(7,8,16,.85)';ctx.fillRect(bx,by2,tw+12,16);
        ctx.fillStyle=bc;ctx.fillText(pct,bx+6,by2+11);
      }
      // Price labels
      ctx.font='500 9.5px JetBrains Mono,monospace';ctx.fillStyle=bc;
    ...
    const ext=_extendLine(x1,y1,x2,y2,W,H,!isPrev&&!!sh.extendRight,!isPrev&&!!sh.extendLeft);
    ctx.save();
    ctx.strokeStyle=col;ctx.lineWidth=lw;
    ctx.setLineDash(isPrev?[6,3]:_dash(sh.lineStyle||'solid',lw));
    ctx.beginPath();ctx.moveTo(ext.sx,ext.sy);ctx.lineTo(ext.ex,ext.ey);ctx.stroke();ctx.setLineDash([]);
    // Ray arrowhead
    if(!isPrev&&sh.extendRight){
      const dx=ext.ex-ext.sx,dy=ext.ey-ext.sy,len=Math.hypot(dx,dy)||1;
      const ux=dx/len,uy=dy/len,nx=-uy,ny=ux;
      ctx.beginPath();ctx.moveTo(ext.ex,ext.ey);
      ctx.lineTo(ext.ex-10*ux+7*nx,ext.ey-10*uy+7*ny);
      ctx.lineTo(ext.ex-10*ux-7*nx,ext.ey-10*uy-7*ny);
    ...
    if(isSel&&!isPrev){
      ...
      const atxt=ang+'°';ctx.font='bold 10px JetBrains Mono,monospace';
      const tw=ctx.measureText(atxt).width;
      const bx=Math.min(x2+10,W-tw-14),badgeY=Math.max(4,y2-16);
      ctx.fillStyle='rgba(7,8,16,.85)';ctx.fillRect(bx,badgeY,tw+10,16);
      ctx.fillStyle=col;ctx.fillText(atxt,bx+5,badgeY+11);
      // Price labels
      ctx.font='500 9.5px JetBrains Mono,monospace';ctx.fillStyle=col;
      ctx.fillText(_fp(sh.p1),x1+9,y1-7);ctx.fillText(_fp(sh.p2),x2+9,y2+16);
    }
    ctx.restore();
  }
}
...
function _setToolUI(tool){
  ['tp-trend','tp-rect','tp-sel'].forEach(id=>{
    ...
    if(el)el.classList.toggle('active',(tool==='trend'&&id==='tp-trend')||(tool==='rect'&&id==='tp-rect')||(tool==='select'&&id==='tp-sel'));
  });
  const pb=document.getElementById('pen-btn');if(pb)pb.classList.toggle('active-mode',!!tool);
  const canvas=document.getElementById('draw-canvas');if(!canvas)return;
  canvas.style.cursor=(tool==='rect'||tool==='trend')?'crosshair':'default';
}
...
window.toggleToolsPopup=function(e){
  ...
  p.style.display='flex';p.style.top=(r.bottom+6)+'px';
  p.style.right=(window.innerWidth-r.right)+'px';p.style.left='';
};
...
window.applyShapeSettings=function(){
  ...
  const cv2=document.getElementById('draw-canvas');
  if(cv2)cv2.style.pointerEvents='auto';
  redrawCanvas();
};
...
window.deleteSelectedShape=function(){
  ...
  document.getElementById('ss-popup').style.display='none';
  const cv3=document.getElementById('draw-canvas');
  if(cv3)cv3.style.pointerEvents='auto';
  redrawCanvas();
};
window.closeShapeSettings=function(){
  document.getElementById('ss-popup').style.display='none';
...
function _openSettings(idx){
  ...
  if(isRect){
    ...
    const hi2=Math.max(sh.p1,sh.p2),lo2=Math.min(sh.p1,sh.p2);
    const pct2=(Math.abs((hi2-lo2)/lo2)*100).toFixed(2);
    const ir=document.getElementById('ss-info-row');
    ir.innerHTML=`Top: ${_fp(hi2)}&nbsp;&nbsp;Bot: ${_fp(lo2)}&nbsp;&nbsp;Δ ${pct2}%`;
    ...
    const ang=Math.round(Math.atan2(-(y2-y1),(x2-x1))*180/Math.PI);
    const ir=document.getElementById('ss-info-row');
    ir.innerHTML=`P1: ${_fp(sh.p1)}&nbsp;&nbsp;P2: ${_fp(sh.p2)}&nbsp;&nbsp;Angle: ${ang}°`;
    ir.className='ss-info-row show';
  }
  ...
  const match=[...document.querySelectorAll('.ss-thick-btn')]
    .sort((a,b2)=>Math.abs(parseFloat(a.dataset.w)-cw)-Math.abs(parseFloat(b2.dataset.w)-cw));
  if(match[0])match[0].classList.add('sel');
  const popup=document.getElementById('ss-popup');
  popup.style.display='flex';popup.style.top='50%';popup.style.left='50%';
  popup.style.transform='translate(-50%,-50%)';
  // Disable canvas pointer-events while popup is open so popup buttons are tappable
  const cv=document.getElementById('draw-canvas');
...
// ══════════════════════════════════════════════════════════════
//
// CRITICAL: initDrawCanvas() attaches ALL event listeners exactly
// ONCE at app startup.  resetDrawCanvas() resets only the mutable
// drag/selection state and is safe to call on every chart reload.
// Calling initDrawCanvas() multiple times was the root cause of
// duplicate listeners that corrupted drag state.

// Called once per chart load — resets draw state, NOT listeners.
function resetDrawCanvas(){
  ...
  _shiftDown=false;
  previewShape=null;
  // Keep shapes[] and selectedIdx so drawings survive TF switches.
  // (Clear them too if you want a fresh canvas on every pair change.)
  resizeDrawCanvas();
  redrawCanvas();
}

// Called ONCE after the DOM is ready — never again.
function initDrawCanvas(){
  const canvas=document.getElementById('draw-canvas');if(!canvas)return;
  resizeDrawCanvas();

  let pDown=false,startCX=0,startCY=0,startPX=0,startPY=0;
  let lastTapMs=0,lastTapIdx=-1;

  // Shift key tracked globally — one listener, attached once.
  ...
  function _cp(e){const s=e.touches?e.touches[0]:(e.changedTouches?e.changedTouches[0]:e);return{cx:s.clientX,cy:s.clientY};}

  canvas.addEventListener('mousemove',function(e){
    if(pDown)return;
    const{cx,cy}=_cp(e);const{x,y}=_canvasXY(canvas,cx,cy);_cursor(canvas,x,y);
  });

  function onDown(e){
    const{cx,cy}=_cp(e);
    startCX=cx;startCY=cy;
    // NOTE: pDown is set to true ONLY when WE handle the event.
    // If we're going to pass through to the chart, pDown stays false
    // so onMove also passes through and chart pan/zoom/scroll works.
    const{x,y}=_canvasXY(canvas,cx,cy);
    const cp=_xyToChart(canvas,cx,cy);

    // ── Hit-test FIRST — always, even with draw tool active ──
    const hi=_findShape(x,y);
    if(hi>=0){
      pDown=true;_mode='idle';startPX=x;startPY=y;
      e.preventDefault();
      selectedIdx=hi;
      const ht=_hit(shapes[hi],x,y);
      _dragStart={cx,cy,px:x,py:y,cp};
      if(ht==='body'){
        _mode='moving';
      ...
      }else if(ht&&(ht.startsWith('corner')||ht.startsWith('ep'))){
        _mode='resizing';_resizeHandle=ht;
        _snapShape=JSON.parse(JSON.stringify(shapes[hi]));
      }
      redrawCanvas();return;
    }

    // ── No shape hit — start drawing if tool active ──────────
    if(drawTool==='rect'||drawTool==='trend'){
      pDown=true;_mode='drawing';startPX=x;startPY=y;
      if(cp){_dragStart={cx,cy,px:x,py:y,cp};}
      e.preventDefault();return;
    }

    // ── Nothing hit, no tool — let chart handle pan/zoom ─────
    // pDown stays FALSE so onMove passes through too.
    selectedIdx=-1;redrawCanvas();
  }

  function onMove(e){
    if(!pDown)return;
    const{cx,cy}=_cp(e);
    const moved=Math.hypot(cx-startCX,cy-startCY);
    if(moved<3)return;
    // pDown is only true when WE own this drag (shape hit or draw tool active).
    // So reaching here means we always want to handle this move.
    e.preventDefault();
    const{x,y}=_canvasXY(canvas,cx,cy);
    const cp=_xyToChart(canvas,cx,cy);
    ...
    if(_mode==='drawing'&&_dragStart?.cp&&cp){
      ...
      previewShape=drawTool==='rect'
        ?{type:'rect',t1:ds.time,p1:ds.price,t2:cp.time,p2:cp.price,
          borderColor:'#4f6ef7',fillColor:'#4f6ef7',lineWidth:1.5,fillOpacity:12,midLine:false}
        :{type:'trend',t1:ds.time,p1:ds.price,t2:cp.time,p2:cp.price,
          color:'#ffc93c',lineWidth:1.8,lineStyle:'solid',extendRight:false,extendLeft:false};
      redrawCanvas();return;
    }

    if(selectedIdx<0||!_dragStart)return;
    const sh=shapes[selectedIdx];

    // ── Move — use PIXEL delta to avoid null from coordinateToTime ──
    // We convert the pixel delta back to chart-domain using the
    // snapshot's pixel positions, not the live pointer coords.
    // This is robust even when the pointer moves to chart edges.
    if(_mode==='moving'&&_snapShape){
      const dpx=x-_dragStart.px;   // pixel delta X since drag start
      const dpy=y-_dragStart.py;   // pixel delta Y since drag start
      const snap=_snapShape;

      // Convert snapshot endpoints to pixels, apply delta, convert back
      const nx1=_sx(snap.t1)+dpx, ny1=_sy(snap.p1)+dpy;
      const nx2=_sx(snap.t2)+dpx, ny2=_sy(snap.p2)+dpy;

      // pixel → chart domain via coordinateToTime/Price
      const nt1=chart.timeScale().coordinateToTime(nx1);
      const np1=candleSeries.coordinateToPrice(ny1);
      const nt2=chart.timeScale().coordinateToTime(nx2);
      const np2=candleSeries.coordinateToPrice(ny2);

      // Only update if conversions succeeded (non-null)
      if(nt1!=null&&np1!=null){sh.t1=nt1;sh.p1=np1;}
      if(nt2!=null&&np2!=null){sh.t2=nt2;sh.p2=np2;}
      redrawCanvas();return;
    }

    // ── Resize ───────────────────────────────────────────────
    if(_mode==='resizing'&&_snapShape&&_resizeHandle&&cp){
      const h=_resizeHandle;
      if(sh.type==='rect'){
        // Corner-pin: dragged corner → pointer; opposite corner stays locked.
        const snap=_snapShape;
        const ox1=_sx(snap.t1),oy1=_sy(snap.p1);
        const ox2=_sx(snap.t2),oy2=_sy(snap.p2);
        const plx=Math.min(ox1,ox2),prx=Math.max(ox1,ox2);
        const pty=Math.min(oy1,oy2),pby=Math.max(oy1,oy2);
        // Locked pixel = diagonally opposite corner
        let lkx,lky;
        if(h==='corner-0'){lkx=prx;lky=pby;}      // TL → lock BR
        else if(h==='corner-1'){lkx=plx;lky=pby;} // TR → lock BL
        else if(h==='corner-2'){lkx=prx;lky=pty;} // BL → lock TR
        else{lkx=plx;lky=pty;}                    // BR → lock TL
        // Convert locked pixel to chart domain
        const lt=chart.timeScale().coordinateToTime(lkx);
        const lp=candleSeries.coordinateToPrice(lky);
        if(lt!=null&&lp!=null){sh.t1=lt;sh.p1=lp;sh.t2=cp.time;sh.p2=cp.price;}
      }else if(sh.type==='trend'){
  ...
  function onUp(e){
    ...
    if(_mode==='drawing'&&previewShape&&moved){
      ...
      drawTool=null;_setToolUI(null); // reset so next click selects
      redrawCanvas();return;
    }

    // Commit parallel clone
    if(_parallelClone&&moved){shapes.push({..._parallelClone});_parallelClone=null;}
    ...
    _mode='idle';_parallelClone=null;
    ...
    if(!moved){
      ...
      if(hi>=0&&hi===lastTapIdx&&now-lastTapMs<400){
        lastTapMs=0;lastTapIdx=-1;_openSettings(hi);
      }else{
        selectedIdx=hi>=0?hi:-1;lastTapMs=now;lastTapIdx=hi;redrawCanvas();
      }
    }
  }
  ...
  canvas.addEventListener('touchmove', function(e){
    // Allow native pinch-zoom (2+ fingers) to reach the chart unblocked
...
init();
// Attach drawing canvas listeners exactly once at startup.
initDrawCanvas();
