...
def mexc_request(method, path, params=None, signed=True):
    ...
    try:
        ...
        if not raw_text:
            # MEXC returns empty body on some successful operations (e.g. change_leverage)
...
def place_order(sig):
    ...
    lev_params = {
        ...
        "leverage":     leverage,
        "openType":     open_type,
        "positionType": 1,   # 1=long side, 2=short side — use 1 for both (cross)
    }
            ...
            for symbol in symbols:
                ...
                if phase == "AWAIT_TAP":
                    ...
                    if model_tag == "2":
                        ...
                        entry_type = "Model #2b (Sweep→CHoCH→FVG Retest)"
