...
.tool-sep{width:1px;height:18px;background:var(--muted);margin:0 2px;flex-shrink:0;align-self:center}
.tool-btn{padding:5px 9px;border-radius:7px;border:1px solid var(--border);background:transparent;color:var(--dim);font-family:var(--mono);font-size:.65rem;font-weight:700;cursor:pointer;transition:all .18s;white-space:nowrap;flex-shrink:0}
.tool-btn.active{background:rgba(255,201,60,.12);border-color:rgba(255,201,60,.45);color:var(--yellow);box-shadow:0 0 10px rgba(255,201,60,.2)}
.tool-btn.clear-btn{color:var(--red)}
.tool-btn.clear-btn:hover{background:rgba(255,77,106,.12);border-color:rgba(255,77,106,.4);color:var(--red)}
  ...
  <div class="main">
    ...
    <div class="page hidden-right" id="page-chart">
      ...
      <div class="tf-bar" id="tf-bar">
        ...
        <button class="tool-btn" id="tool-line" onclick="setTool('line')" title="Draw horizontal line">/ Line</button>
        <button class="tool-btn" id="tool-rect" onclick="setTool('rect')" title="Draw zone rectangle">⊡ Rect</button>
        <button class="tool-btn clear-btn" onclick="clearDrawings()" title="Clear all drawings">✕ Clear</button>
      </div>
...
// ══════════════════════════════════════════════════════════════════
// ── Drawing Tools (Line / Rect) ───────────────────────────────────
// ══════════════════════════════════════════════════════════════════
let activeTool=null;   // 'line' | 'rect' | null
let drawnLines=[];     // [{price}]
let drawnRects=[];     // [{priceTop,priceBot}]
let rectStart=null;    // price of first tap while in rect mode

// Toggle a tool on/off (tapping the same button again deactivates)
window.setTool=function(tool){
  activeTool=(activeTool===tool)?null:tool;
  document.getElementById("tool-line").classList.toggle("active",activeTool==="line");
  document.getElementById("tool-rect").classList.toggle("active",activeTool==="rect");
  const canvas=document.getElementById("draw-canvas");
  if(canvas)canvas.style.pointerEvents=activeTool?"auto":"none";
  if(!activeTool)rectStart=null;
};

// Clear every user-drawn line and rectangle
...
// Size the canvas buffer to match its CSS display size (avoids blurriness)
...
// Re-paint all user drawings on top of the chart
function redrawCanvas(){
  const canvas=document.getElementById("draw-canvas");
  if(!canvas||!candleSeries)return;
  resizeDrawCanvas();
  ...
  for(const ln of drawnLines){
    ...
    ctx.save();
    ctx.beginPath();
    ctx.strokeStyle="rgba(255,201,60,.88)";
    ctx.lineWidth=1.5;
    ctx.setLineDash([7,4]);
    ctx.moveTo(0,y);ctx.lineTo(W,y);
    ctx.stroke();
    // Label pill
    const lbl=ln.price<0.01?ln.price.toFixed(8):ln.price<1?ln.price.toFixed(6):ln.price<100?ln.price.toFixed(4):ln.price.toFixed(2);
    ctx.setLineDash([]);
    ctx.font="bold 10px 'JetBrains Mono',monospace";
    const tw=ctx.measureText(lbl).width;
    ctx.fillStyle="rgba(255,201,60,.18)";
    ctx.fillRect(6,y-16,tw+10,16);
    ctx.strokeStyle="rgba(255,201,60,.5)";
    ctx.lineWidth=1;
    ctx.strokeRect(6,y-16,tw+10,16);
    ctx.fillStyle="#ffc93c";
    ctx.fillText(lbl,11,y-4);
    ctx.restore();
  }

  // ── Rectangles (zone boxes) ───────────────────────────────────────
  for(const rc of drawnRects){
    const y1=candleSeries.priceToCoordinate(rc.priceTop);
    const y2=candleSeries.priceToCoordinate(rc.priceBot);
    if(y1===null||y2===null)continue;
    const top=Math.min(y1,y2),bot=Math.max(y1,y2);
    ctx.save();
    ctx.fillStyle="rgba(79,110,247,.09)";
    ctx.fillRect(0,top,W,bot-top);
    ctx.strokeStyle="rgba(79,110,247,.5)";
    ctx.lineWidth=1;
    ctx.setLineDash([5,3]);
  ...
  // ── Rect-in-progress (first tap placed, waiting for second) ───────
  if(rectStart!==null){
    const y=candleSeries.priceToCoordinate(rectStart);
    if(y!==null&&y>=0&&y<=H){
      ctx.save();
      ctx.beginPath();
      ctx.strokeStyle="rgba(79,110,247,.7)";
      ctx.lineWidth=1;
      ctx.setLineDash([5,3]);
      ctx.moveTo(0,y);ctx.lineTo(W,y);
      ctx.stroke();
...
// Attach touch + click listeners to the drawing canvas
// Supports: tap-to-draw (line/rect), press-and-hold + drag to move any rect
function initDrawCanvas(){
  ...
  // ── Drag state ─────────────────────────────────────────────────────
  let dragIdx=-1;           // index into drawnRects being dragged
  let dragAnchorPrice=null; // price under finger at drag-start
  let dragTopOrig=null;     // rect priceTop at drag-start
  let dragBotOrig=null;     // rect priceBot at drag-start
  let isDragging=false;
  let lpTimer=null;         // long-press timer handle
  let touchMoved=false;     // true if finger moved before longpress fired

  function getPriceFromClient(clientY){
    if(!candleSeries)return null;
    const bb=canvas.getBoundingClientRect();
    return candleSeries.coordinateToPrice(clientY-bb.top);
  }

  function findRectAtPrice(price){
    // Return index of topmost rect containing this price (search newest first)
    for(let i=drawnRects.length-1;i>=0;i--){
      if(price>=drawnRects[i].priceBot&&price<=drawnRects[i].priceTop)return i;
    }
    return -1;
  }

  // ── Touch start: arm long-press for rect drag ───────────────────────
  canvas.addEventListener("touchstart",function(e){
    ...
    const price=getPriceFromClient(clientY);
    if(price===null)return;
    const idx=findRectAtPrice(price);
    if(idx<0)return; // no rect here — nothing to drag

    // Arm a 320 ms long-press; if the finger stays still, enter drag mode
    lpTimer=setTimeout(()=>{
      lpTimer=null;
      if(touchMoved)return; // finger already slid away
      isDragging=true;
      dragIdx=idx;
      dragAnchorPrice=price;
      dragTopOrig=drawnRects[idx].priceTop;
      dragBotOrig=drawnRects[idx].priceBot;
      // Brief haptic feedback if available
      if(navigator.vibrate)navigator.vibrate(40);
      redrawCanvas(); // highlight rect
    },320);
  },{passive:true});

  // ── Touch move: drag the rect ───────────────────────────────────────
  canvas.addEventListener("touchmove",function(e){
    touchMoved=true;
    if(lpTimer){clearTimeout(lpTimer);lpTimer=null;}
    if(!isDragging||dragIdx<0)return;
    e.preventDefault();
    const price=getPriceFromClient(e.touches[0].clientY);
    if(price===null)return;
    const delta=price-dragAnchorPrice;
    drawnRects[dragIdx]={priceTop:dragTopOrig+delta,priceBot:dragBotOrig+delta};
    redrawCanvas();
  },{passive:false});

  // ── Touch end: end drag OR handle tap-to-draw ───────────────────────
  canvas.addEventListener("touchend",function(e){
    if(lpTimer){clearTimeout(lpTimer);lpTimer=null;}
    if(isDragging){
      isDragging=false;
      dragIdx=-1;
      return; // don't fire a draw tap after finishing a drag
    }
    if(touchMoved)return; // scrolled — ignore
    handleTap(e);
  },{passive:false});
  ...
  // ── Core tap handler (draw lines / rects) ───────────────────────────
  function handleTap(e){
    if(!activeTool||!candleSeries)return;
    e.preventDefault();
    e.stopPropagation();
    ...
    if(activeTool==="line"){
      drawnLines.push({price});
      redrawCanvas();
    } else if(activeTool==="rect"){
      if(rectStart===null){
        rectStart=price;
        redrawCanvas(); // show dashed first-tap guide line
      } else {
        drawnRects.push({priceTop:Math.max(rectStart,price),priceBot:Math.min(rectStart,price)});
        rectStart=null;
        redrawCanvas();
      }
    }
