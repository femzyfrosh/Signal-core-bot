...
if(!keys.length){$("pgrid").innerHTML=TOP.map(sym=>`<div class="pc"><div class="pc-sym">${sym.replace("_USDT","")}/USDT</div><div class="pc-price" style="color:var(--dim)">Waiting...</div></div>`).join("");return;}
$("pgrid").innerHTML=TOP.map(sym=>{const d=data[sym],name=sym.replace("_USDT","");if(!d||!d.price)return`<div class="pc"><div class="pc-sym">${name}/USDT</div><div class="pc-price">--</div></div>`;const up=Number(d.change||0)>=0;return`<div class="pc ${up?"up":"dn"}"><div class="pc-sym">${name}/USDT</div><div class="pc-price">${fmtP(d.price)}</div><span class="pc-chg ${up?"up":"dn"}">${(up?"+":"")+Number(d.change||0).toFixed(2)}%</span></div>`}).join("");
}catch(err){console.error("Price feed failed:",err);if($("pgrid"))$("pgrid").innerHTML='<div class="empty">Price feed reconnecting...</div>';}
}
