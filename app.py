...
<div class="app">
  ...
  <div class="topbar">
    <div class="topbar-left">
      <span class="logo" id="topbar-logo">SignalCore</span>
      <div class="scanner-dot" id="topbar-dot"></div>
      <button class="back-btn" id="back-btn" onclick="goToWatchlist()">← Back</button>
      <span class="pair-label" id="pair-label" style="display:none"></span>
    </div>
...
function goToWatchlist(){
  ...
  const logo=document.getElementById("topbar-logo");const dot=document.getElementById("topbar-dot");
  if(logo)logo.style.display="";if(dot)dot.style.display="";
  stopChartUpdate();
  closeSettingsPanel();
}
...
async function goToChart(sym,tf){
  ...
  const logo=document.getElementById("topbar-logo");const dot=document.getElementById("topbar-dot");
  if(logo)logo.style.display="none";if(dot)dot.style.display="none";
...
function renderWatchlist(){
  ...
  if(!filtered.length){
    list.innerHTML='<div style="padding:40px 16px;text-align:center;color:var(--dim);font-family:var(--mono);font-size:.75rem">No pairs match "'+searchFilter+'"</div>';
    return;
  }
...
function init(){
  // Render watchlist immediately — do NOT call initCanvas() here.
  // initCanvas() touches the canvas which lives inside the hidden chart
  // page. On Android WebView / Telegram mini-app the ResizeObserver or
  // getContext('2d') on a 0-size off-screen element can throw a silent
  // exception that kills the whole IIFE, leaving the list blank.
  // Instead we defer canvas init to the first time the chart page opens.
  renderWatchlist();
