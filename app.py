  ...
  if(w1){
    if(!d1.length){
      ...
      w1.innerHTML=`<div class="monitor-grid">${d1.map(m=>{
        ...
        <div class="mon-status">⏳ AWAITING C2 CLOSE</div></div>`;
      }).join("")}</div>`;
    }
  }
  // ── M2/M3 Unified Monitor ──
...
window.sw=function(tab,btn){activeTab=tab;document.querySelectorAll(".tab").forEach(b=>b.classList.remove("active"));btn.classList.add("active");["signals","trades","monitor","history","trade-cfg","paper","log","settings"].forEach(t=>{const el=$("tab-"+t);if(el)el.style.display=t===tab?"block":"none";});if(tab==="log"){fetchLog();fetchDiag();}if(tab==="trades")window.fetchPnl();if(tab==="monitor")fetchMonitor();if(tab==="history")fetchHistory();if(tab==="trade-cfg"){loadTradeConfig();window.fetchPnl();}if(tab==="paper"){loadPaperConfig();fetchPaperData();}if(tab==="settings")loadSettings();};
...
def api_settings():
    if request.method == "POST":
        ...
        with settings_lock:
            ...
            for k in ["tg_signals","tg_trades","model2_enabled"]:
