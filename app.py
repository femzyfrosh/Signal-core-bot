...
function initDrawCanvas(){
  ...
  function onDown(e){
    ...
    // ── HIT TEST FIRST — always, regardless of active tool ─
    // Clicking an existing shape selects/moves/resizes it even
    // when a draw tool is active (exactly like TradingView).
    const hi=_findShape(x,y);

    if(hi>=0){
      // Shape hit — claim the event, select it, prepare drag
      e.preventDefault();
      selectedIdx=hi;
    ...
    // ── No shape hit — start drawing if a tool is active ───
    if(drawTool==='rect'||drawTool==='trend'){
      if(cp){ _dragStart={cx,cy,cp}; _mode='drawing'; }
      e.preventDefault();
      return;
    }

    // ── Nothing hit, no tool — deselect, let chart pan/zoom ─
    selectedIdx=-1;
    redrawCanvas();
    // DO NOT preventDefault → chart handles pan/zoom
  }
  ...
  function onUp(e){
    ...
    if(_mode==='drawing'&&previewShape&&moved){
      ...
      // Reset tool so next click selects instead of drawing again
      drawTool=null;
      _setToolUI(null);
      redrawCanvas();
      return;
