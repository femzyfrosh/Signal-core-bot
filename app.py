...
@app.route("/dashboard")
def dashboard():
    # FIX: Added auth check — /dashboard was publicly accessible without login
    token = request.cookies.get("session")
    if token:
        try:
            payload, sig = token.rsplit(".", 1)
...
# ── STARTUP ──────────────────────────────────────────────────────
# Called once when Gunicorn (workers=1) loads this module in the worker process.
# Direct call — no app_context wrapper which was firing in every worker and
# causing scan_state/signals to be out of sync with HTTP request handlers.
