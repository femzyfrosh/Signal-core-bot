  ...
  <div class="main">
    ...
    <div class="page hidden-right" id="page-chart">
      ...
      <div class="ss-popup" id="ss-popup">
        ...
        <!-- Label text (both tools) -->
        <div class="ss-row" style="flex-direction:column;align-items:flex-start;gap:4px">
          <div class="ss-lbl">Label (optional)</div>
          <input type="text" id="ss-label" placeholder="e.g. Support, OB, Entry…"
            style="width:100%;background:var(--s2);border:1px solid var(--border);border-radius:8px;
                   color:var(--text);font-size:.74rem;padding:6px 8px;outline:none;font-family:var(--mono)">
        </div>
      ...
      <div class="chart-wrap">
        ...
        <!-- Drawing canvas overlay — pointer-events toggled dynamically -->
        <canvas id="draw-canvas" style="position:absolute;inset:0;z-index:6;pointer-events:none;display:block;cursor:default"></canvas>
...
async function loadChart(sym,tf){
  ...
  // Reset draw tool state and unfreeze in case a tool was left active
  drawTool=null;_setToolUI(null);_mode='idle';previewShape=null;_freezeChart(false);
  ...
  // Initialise drawing canvas (first time only — guarded inside)
  initDrawCanvas();
  // Subscribe THIS chart instance to keep shapes price-locked on scroll/zoom
  if(chart){
...
// ══════════════════════════════════════════════════════════════════
// ── Drawing Engine v3 — TradingView-style ────────────────────────
// ══════════════════════════════════════════════════════════════════
//
//  HOW IT WORKS:
//  - Canvas always has pointer-events:auto
//  - When NO tool is active AND pointer is not over a shape → events
//    pass through to LightweightCharts for pan/zoom (we do this by
//    checking hitTest first; if miss AND no tool → pass through)
//  - drawTool = 'rect'|'trend'|'select'|null
//    null = smart mode: select/move shapes if hit, else pass through
//
//  Shape fields:
//    rect:  {type,t1,p1,t2,p2,borderColor,fillColor,lineWidth,fillOpacity,midLine}
//    trend: {type,t1,p1,t2,p2,color,lineWidth,lineStyle,extendRight,extendLeft}
...
let _dragStart     = null;   // {cx,cy,chartTime,chartPrice} at mousedown
let _snapShape     = null;   // deep-copy of shape at drag start
let _resizeHandle  = null;   // 'corner-0'..'corner-3' | 'ep-0'|'ep-1'
let _parallelClone = null;   // pending parallel trendline ghost
let _chartOwnsPointer = false; // true when a drag started on empty chart area

const COLORS  = ['#4f6ef7','#00d4aa','#ff4d6a','#ffc93c','#a855f7','#f97316','#e2e8f0','#06b6d4'];
const HR      = 6;   // handle draw radius px
const HIT     = 14;  // line/border hit tolerance px
const HCORNER = 28;  // corner/endpoint hit radius px — large for fat fingers
...
function _extendLine(x1,y1,x2,y2,W,H,extR,extL){
  ...
  if(extR){
    if(Math.abs(dx)>0.001){
      ...
      else{const yy=yr<0?0:H;ex=xAtY(yy);ey=yy;}
    }
  }
  if(extL){
    if(Math.abs(dx)>0.001){
      ...
      else{const yy=yl<0?0:H;sx=xAtY(yy);sy=yy;}
    }
  }
  return{sx,sy,ex,ey};
}

// ── Hit test — returns false | 'body' | 'corner-N' | 'ep-N' ───
...
function _cursor(canvas,x,y){
  ...
  for(let i=shapes.length-1;i>=0;i--){
    ...
    if(h.startsWith('corner')||h.startsWith('ep')){canvas.style.cursor='crosshair';return;}
  }
...
function _draw(ctx,sh,isSel,isPrev){
  ...
  if(sh.type==='rect'){
    ...
    const fo=isPrev?0.08:((sh.fillOpacity||12)/100);
    ...
    if(sh.midLine&&!isPrev){
      ctx.beginPath();ctx.strokeStyle=bc;ctx.lineWidth=Math.max(1,lw*.6);
      ctx.setLineDash([4,4]);ctx.moveTo(lx,ty+h/2);ctx.lineTo(rx,ty+h/2);
      ctx.stroke();ctx.setLineDash([]);
    }
    // Handles + labels when selected
    if(isSel&&!isPrev){
      const corners=[[lx,ty],[rx,ty],[lx,by],[rx,by]];
      ...
      // Height % badge
      const p1v=sh.p1,p2v=sh.p2;
      if(p1v!=null&&p2v!=null){
        const hi=Math.max(p1v,p2v),lo=Math.min(p1v,p2v);
        const pct=(Math.abs((hi-lo)/lo)*100).toFixed(2)+'%';
        ctx.font='bold 10px JetBrains Mono,monospace';
        const tw=ctx.measureText(pct).width;
        const bx=lx+w/2-tw/2-4,by2=ty-14;
        ctx.fillStyle='rgba(7,8,16,.82)';ctx.fillRect(bx-2,by2-2,tw+12,15);
        ctx.fillStyle=bc;ctx.fillText(pct,bx+4,by2+10);
      }
      // Price labels
    ...
    // Label — always shown if set
    if(sh.label&&!isPrev){
      const lc=sh.borderColor||'#4f6ef7';
      ctx.font='bold 11px JetBrains Mono,monospace';
      const tw=ctx.measureText(sh.label).width;
      const lbx=Math.round(lx+w/2-tw/2);
      const lby=Math.round(ty+h/2+4);
      ctx.fillStyle='rgba(7,8,16,.78)';
      ctx.fillRect(lbx-6,lby-13,tw+12,18);
      ctx.fillStyle=lc;
      ctx.fillText(sh.label,lbx,lby);
    }
    ctx.restore();
    ...
    ctx.beginPath();ctx.moveTo(ext.sx,ext.sy);ctx.lineTo(ext.ex,ext.ey);
    ctx.stroke();ctx.setLineDash([]);
    // Handles + labels when selected
    if(isSel&&!isPrev){
      ...
      ctx.fillStyle='rgba(7,8,16,.82)';ctx.fillRect(x2+8,y2-14,tw+10,15);
      ctx.fillStyle=col;ctx.fillText(atxt,x2+13,y2-2);
      // Price labels
      ctx.font='500 9.5px JetBrains Mono,monospace';ctx.fillStyle=col;
      ctx.fillText(_fp(sh.p1),x1+8,y1-6);
      ctx.fillText(_fp(sh.p2),x2+8,y2+14);
    }
    // Label — shown at midpoint of line if set
    if(sh.label&&!isPrev){
      const midX=Math.round((x1+x2)/2), midY=Math.round((y1+y2)/2);
      const lc=sh.color||'#ffc93c';
      ctx.font='bold 11px JetBrains Mono,monospace';
      const tw=ctx.measureText(sh.label).width;
      ctx.fillStyle='rgba(7,8,16,.78)';
      ctx.fillRect(midX-tw/2-6,midY-14,tw+12,18);
      ctx.fillStyle=lc;
      ctx.fillText(sh.label,midX-tw/2,midY+1);
    }
    ctx.restore();
  }
}
...
// ── Tool UI helpers ────────────────────────────────────────────
function _freezeChart(frozen){
  if(!chart)return;
  chart.applyOptions({
    handleScroll:{mouseWheel:!frozen,pressedMouseMove:!frozen,horzTouchDrag:!frozen,vertTouchDrag:!frozen},
    handleScale:{axisPressedMouseMove:!frozen,mouseWheel:!frozen,pinch:!frozen},
  });
}
...
window.applyShapeSettings=function(){
  ...
  sh.label=document.getElementById('ss-label').value.trim();
...
function _openSettings(idx){
  ...
  // Load label
  document.getElementById('ss-label').value=sh.label||'';
...
function initDrawCanvas(){
  ...
  // ── Guard: only attach listeners once ever ────────────────────
  if(canvas._drawInited) return;
  canvas._drawInited=true;

  resizeDrawCanvas();

  // Canvas is ALWAYS pointer-events:none — chart gets all native events
  ...
  function _overShape(cx,cy){
    const{x,y}=_canvasXY(canvas,cx,cy);
    return _findShape(x,y)>=0;
  }

  function _updateCursor(cx,cy){
    if(drawTool==='rect'||drawTool==='trend'){wrap.style.cursor='crosshair';return;}
    const{x,y}=_canvasXY(canvas,cx,cy);
    const hi=_findShape(x,y);
    if(hi<0){wrap.style.cursor='';return;}
    const h=_hit(shapes[hi],x,y);
    wrap.style.cursor=(h==='body')?'move':'crosshair';
  }

  wrap.addEventListener('mousemove',function(e){
    if(!pDown) _updateCursor(e.clientX,e.clientY);
  });

  function onDown(e){
    // Never intercept pinch/multi-touch — always let chart handle it
    if(e.touches&&e.touches.length>1) return;

    const{cx,cy}=_cp(e);
    const isDrawMode=drawTool==='rect'||drawTool==='trend';
    const{x,y}=_canvasXY(canvas,cx,cy);

    const hi=_findShape(x,y);
    const overShape=hi>=0;

    // Not our event → let chart handle; deselect if tapping empty space
    if(!isDrawMode&&!overShape){
      if(selectedIdx>=0){selectedIdx=-1;redrawCanvas();}
      return;
    }
    ...
    // Freeze chart scroll/pan so it doesn't fight our drag
    _freezeChart(true);
    ...
    if(hi<0){pDown=false;_freezeChart(false);return;}

    const hitType=_hit(shapes[hi],x,y);
    _dragStart={cx,cy,cp};
    _snapShape=JSON.parse(JSON.stringify(shapes[hi]));

    if(hitType&&(hitType.startsWith('corner')||hitType.startsWith('ep'))&&selectedIdx===hi){
      selectedIdx=hi;_mode='resizing';_resizeHandle=hitType;
    }else if(hitType&&(hitType.startsWith('corner')||hitType.startsWith('ep'))){
      selectedIdx=hi;_mode='moving';_resizeHandle=hitType;
    }else{
      selectedIdx=hi;_mode='moving';_resizeHandle=null;
      if(shapes[hi].type==='trend'&&e.shiftKey){
  ...
  function onMove(e){
    if(!pDown)return;
    // Never intercept pinch
    if(e.touches&&e.touches.length>1){pDown=false;_freezeChart(false);return;}
    const{cx,cy}=_cp(e);
    ...
    if(!cp||selectedIdx<0||!_dragStart?.cp)return;
    const sh=shapes[selectedIdx];
    const ds=_dragStart.cp;

    // Upgrade moving→resizing if a corner handle was stored
    if(_mode==='moving'&&_snapShape&&_resizeHandle) _mode='resizing';

    if(_mode==='moving'&&_snapShape){
      const dt=cp.time-ds.time, dp=cp.price-ds.price;
    ...
    if(_mode==='resizing'&&_snapShape&&_resizeHandle){
      ...
      if(sh.type==='rect'){
        ...
        const lxs=Math.min(_t2x(snap.t1)||0,_t2x(snap.t2)||0);
        const rxs=Math.max(_t2x(snap.t1)||0,_t2x(snap.t2)||0);
        const tys=Math.min(_p2y(snap.p1)||0,_p2y(snap.p2)||0);
        const bys=Math.max(_p2y(snap.p1)||0,_p2y(snap.p2)||0);
        const anchors={
          'corner-0':{ax:rxs,ay:bys},'corner-1':{ax:lxs,ay:bys},
          'corner-2':{ax:rxs,ay:tys},'corner-3':{ax:lxs,ay:tys}
        };
        const anch=anchors[h];
        if(anch){
          const bb=canvas.getBoundingClientRect();
          const ac=_xyToChart(canvas,anch.ax+bb.left,anch.ay+bb.top);
          if(ac){sh.t1=ac.time;sh.p1=ac.price;sh.t2=cp.time;sh.p2=cp.price;}
        }
      }else if(sh.type==='trend'){
  ...
  function onUp(e){
    ...
    // Touch has more jitter — use larger threshold
    const movedThresh=isTouch?12:6;
    const moved=Math.hypot(cx-startCX,cy-startCY)>movedThresh;
    ...
    if(_mode==='drawing'&&previewShape&&moved){
      ...
      drawTool=null;_setToolUI(null);
      _freezeChart(false);
      redrawCanvas();
      return;
    }

    if(_parallelClone&&moved) shapes.push({..._parallelClone});
    previewShape=null;_parallelClone=null;_dragStart=null;
    ...
    _mode='idle';
    // Always unfreeze on pointer up — chart should never stay locked
    _freezeChart(false);
    ...
    if(!moved){
      ...
      if(hi>=0&&hi===lastTapIdx&&now-lastTapMs<450){
        // Double-tap → open settings popup
        lastTapMs=0;lastTapIdx=-1;
        selectedIdx=hi;
        _openSettings(hi);
      }else if(hi>=0){
        // Single tap on shape → select it
        selectedIdx=hi;
  ...
  document.addEventListener('keydown',function(e){
    ...
    if((e.key==='Delete'||e.key==='Backspace')&&selectedIdx>=0){
      shapes.splice(selectedIdx,1);selectedIdx=-1;redrawCanvas();
    }
    if(e.key==='Escape'){
      previewShape=null;drawTool=null;selectedIdx=-1;
      _freezeChart(false);
      _setToolUI(null);wrap.style.cursor='';redrawCanvas();
    }
  });
}
