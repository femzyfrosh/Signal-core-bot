...
@app.route("/api/market-order-now", methods=["POST"])
def api_market_order_now():
    """
    Immediate market order bypass — called from the signal popup 'Place Order Now' button.
    Bypasses auto-trade enabled flag, max-trade cap, and uses a hardcoded market order (type=5).
    Forces direct HTTPS to avoid redirect-stripping the auth headers (the 403 root cause).
    """
    import json as _json
    data = request.get_json(silent=True) or {}
    symbol    = data.get("symbol", "")
    ...
    entry = float(entry_raw) if entry_raw else 0
    sl    = float(sl_raw)    if sl_raw    else 0
    tp    = float(tp_raw)    if tp_raw    else 0

    # If no entry price, use live price from cache
    if entry <= 0:
        with price_cache_lock:
            cached = price_cache.get(symbol, {})
        entry = cached.get("price", 0)
    if entry <= 0:
        ticker = get_ticker(symbol)
        entry  = ticker["price"] if ticker else 0
    if entry <= 0:
        return jsonify({"ok": False, "error": "Could not determine entry price"}), 400

    # Use live price as SL/TP fallback (1% each side)
    if sl <= 0:
        sl = round(entry * (0.99 if direction == "BUY" else 1.01), 8)
    if tp <= 0:
        tp = round(entry * (1.02 if direction == "BUY" else 0.98), 8)

    # Build a minimal signal dict for place_order
    sig = {
        "symbol":       symbol,
        "direction":    direction,
        "entry":        entry,
        ...
        "grade":        grade,
        "model":        model,
        "market_order": True,   # forces type=5 (market) in place_order
        "timestamp":    datetime.now(LOCAL_TZ).strftime("%Y-%m-%d %H:%M UTC+1"),
    }

    # ── Temporarily bypass the enabled/max-trade guard ────────────────
    orig_enabled    = trade_config.get("enabled", False)
    orig_max_trades = trade_config.get("max_trades", 2)
    try:
        with trade_lock:
            trade_config["enabled"]    = True
            trade_config["max_trades"] = max(orig_max_trades, len(open_trades) + 1)
        ok, msg = place_order(sig)
    finally:
        with trade_lock:
            trade_config["enabled"]    = orig_enabled
            trade_config["max_trades"] = orig_max_trades

    if ok:
        log(f"🚀 MANUAL MARKET ORDER PLACED via popup: {direction} {symbol} @ market | {msg}")
        send_telegram(
            f"🚀 <b>MANUAL MARKET ORDER</b>\n"
            f"<b>Pair:</b> {symbol}\n"
            f"<b>Side:</b> {direction}\n"
            f"<b>Entry:</b> ~{entry}\n"
            f"<b>SL:</b> {sl} | <b>TP:</b> {tp}\n"
            f"<b>RR:</b> {rr}R | Score: {score}/100 {grade}\n"
            f"<i>Placed via signal popup button</i>",
            kind="trade"
        )
    else:
        log(f"❌ MANUAL MARKET ORDER FAILED: {symbol} — {msg}")
  ...
  <div class="main">
    ...
    <div class="page hidden-right" id="page-chart">
      ...
      <div class="chart-wrap">
        ...
        <!-- Candle-close countdown — right side under current price label -->
        <div id="candle-countdown-overlay" style="display:none;position:absolute;top:8px;right:0;z-index:10;pointer-events:none;user-select:none;text-align:right;padding-right:6px">
          <div style="background:rgba(7,8,16,.82);border:1px solid rgba(79,110,247,.25);border-radius:6px;padding:3px 8px;display:inline-block;backdrop-filter:blur(8px)">
            <span style="font-family:var(--mono);font-size:.48rem;color:var(--dim);text-transform:uppercase;letter-spacing:.06em;display:block;line-height:1.2">closes in</span>
            <span id="candle-countdown" style="font-family:var(--mono);font-size:.85rem;font-weight:800;color:#ffc93c;letter-spacing:.05em;line-height:1">00:00</span>
          </div>
        </div>
...
// Update the candle-close countdown display (floating overlay on chart)
function updateCountdown(){
  const secs=TF_SECS[currentTF];
  const overlay=document.getElementById("candle-countdown-overlay");
  if(!secs||!currentPair){
  ...
  // Pulse yellow → green when < 10 seconds remaining
...
// Track which signals we've already alerted so we don't spam
const _seenSignalKeys=new Set();
let _signalPopupQueue=[];
let _signalPopupShowing=false;
...
async function fetchSignals(){
  try{
    ...
    // Check for new signals to pop up
    arr.slice(0,10).forEach(s=>{
      const key=(s.symbol||"")+"_"+(s.timestamp||"")+"_"+(s.direction||"");
      if(!_seenSignalKeys.has(key)){
        _seenSignalKeys.add(key);
        // Only alert if signal is recent (within last 3 minutes)
        if(s.timestamp){
          try{
            // timestamps like "2025-01-01 19:30:33 UTC+1"
            const ts=s.timestamp.replace(" UTC+1","").replace(" ","T");
            const age=(Date.now()-new Date(ts).getTime())/1000;
            if(age<180) _signalPopupQueue.push(s);
          }catch{_signalPopupQueue.push(s);}
        }
      }
    });
    if(_signalPopupQueue.length>0&&!_signalPopupShowing) showNextSignalPopup();
    renderWatchlist();
  }catch{}
}

function showNextSignalPopup(){
  if(_signalPopupQueue.length===0){_signalPopupShowing=false;return;}
  _signalPopupShowing=true;
  const sig=_signalPopupQueue.shift();
  showSignalPopup(sig);
}

function showSignalPopup(sig){
  // Remove any existing popup
  const old=document.getElementById("global-signal-popup");
  if(old)old.remove();

  const isBuy=sig.direction==="BUY";
  const clr=isBuy?"var(--green)":"var(--red)";
  const bg=isBuy?"rgba(0,212,170,.13)":"rgba(255,77,106,.13)";
  const border=isBuy?"rgba(0,212,170,.5)":"rgba(255,77,106,.5)";
  const arrow=isBuy?"▲":"▼";
  const sym=(sig.symbol||"").replace("_USDT","")+"/USDT";

  const popup=document.createElement("div");
  popup.id="global-signal-popup";
  popup.style.cssText=`
    position:fixed;bottom:70px;left:12px;right:12px;z-index:9999;
    background:rgba(7,8,16,.97);border:2px solid ${border};border-radius:16px;
    padding:14px 16px 12px;box-shadow:0 0 32px ${isBuy?"rgba(0,212,170,.35)":"rgba(255,77,106,.35)"},0 8px 32px rgba(0,0,0,.7);
    backdrop-filter:blur(24px);animation:popup-slide-up .35s cubic-bezier(.4,0,.2,1);
    font-family:var(--font);
  `;

  popup.innerHTML=`
    <style>
    @keyframes popup-slide-up{from{transform:translateY(100%);opacity:0}to{transform:translateY(0);opacity:1}}
    @keyframes popup-pulse{0%,100%{box-shadow:0 0 0 0 ${isBuy?"rgba(0,212,170,.6)":"rgba(255,77,106,.6)"}}50%{box-shadow:0 0 0 10px transparent}}
    </style>
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
      <div style="display:flex;align-items:center;gap:8px">
        <span style="font-size:1.3rem;animation:blink 1s infinite">${isBuy?"🟢":"🔴"}</span>
        <div>
          <div style="font-family:var(--display);font-size:.95rem;font-weight:800;color:${clr};letter-spacing:.04em">NEW SIGNAL ${arrow}</div>
          <div style="font-family:var(--mono);font-size:.72rem;color:var(--dim);margin-top:1px">Model #${sig.model||"1"} · ${sig.grade||"A"} · ${sig.rr||"?"}R</div>
        </div>
      </div>
      <div style="display:flex;align-items:center;gap:6px">
        <span style="font-family:var(--display);font-size:1rem;font-weight:800;color:var(--text)">${sym}</span>
        <button onclick="document.getElementById('global-signal-popup').remove();showNextSignalPopup();" style="background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.15);border-radius:6px;color:var(--dim);font-size:.8rem;cursor:pointer;padding:3px 8px;line-height:1">✕</button>
      </div>
    </div>
    <div style="display:flex;gap:8px;margin-bottom:12px;font-family:var(--mono);font-size:.68rem">
      <div style="flex:1;background:rgba(255,255,255,.04);border-radius:8px;padding:7px 10px">
        <div style="color:var(--dim);margin-bottom:2px">ENTRY</div>
        <div style="color:var(--yellow);font-weight:700">${fmtPrice(sig.entry)}</div>
      </div>
      <div style="flex:1;background:rgba(255,255,255,.04);border-radius:8px;padding:7px 10px">
        <div style="color:var(--dim);margin-bottom:2px">SL</div>
        <div style="color:var(--red);font-weight:700">${fmtPrice(sig.sl)}</div>
      </div>
      <div style="flex:1;background:rgba(255,255,255,.04);border-radius:8px;padding:7px 10px">
        <div style="color:var(--dim);margin-bottom:2px">TP</div>
        <div style="color:var(--green);font-weight:700">${fmtPrice(sig.tp)}</div>
      </div>
    </div>
    <button id="popup-place-btn" onclick="popupPlaceOrder(${JSON.stringify(sig).replace(/"/g,'&quot;')})" style="
      width:100%;padding:15px;border-radius:12px;border:none;cursor:pointer;
      background:linear-gradient(135deg,${isBuy?"#00d4aa,#00b894":"#ff4d6a,#e0003c"});
      color:#fff;font-family:var(--display);font-size:1.05rem;font-weight:800;
      letter-spacing:.06em;text-transform:uppercase;
      box-shadow:0 4px 20px ${isBuy?"rgba(0,212,170,.5)":"rgba(255,77,106,.5)"};
      animation:popup-pulse 2s infinite;
      display:flex;align-items:center;justify-content:center;gap:8px;
    ">
      <span style="font-size:1.2rem">${isBuy?"🚀":"📉"}</span>
      PLACE ORDER NOW · ${isBuy?"LONG":"SHORT"}
    </button>
    <div id="popup-status" style="font-family:var(--mono);font-size:.7rem;text-align:center;color:var(--dim);margin-top:7px;min-height:16px"></div>
  `;

  document.body.appendChild(popup);

  // Auto-dismiss after 60 seconds (but keep queue going)
  setTimeout(()=>{
    const p=document.getElementById("global-signal-popup");
    if(p){p.style.animation="none";p.style.opacity="0";p.style.transition="opacity .4s";setTimeout(()=>{if(p.parentNode)p.remove();showNextSignalPopup();},400);}
  },60000);
}

window.popupPlaceOrder=async function(sig){
  const btn=document.getElementById("popup-place-btn");
  const status=document.getElementById("popup-status");
  if(!btn||!status)return;
  btn.disabled=true;
  btn.style.opacity=".6";
  btn.textContent="⏳ Placing order...";
  status.textContent="Sending market order to MEXC...";
  status.style.color="var(--yellow)";
  try{
    const r=await fetch("/api/market-order-now",{
      method:"POST",
    ...
    if(d.ok){
      ...
      status.textContent=d.message||"Trade placed on MEXC ✓";
      ...
      btn.style.opacity="1";
      btn.disabled=false;
      status.textContent=d.error||d.message||"Order failed";
...
function updateChartHeader(){
  ...
  // High/Low — show from cache; if still blank trigger a direct ticker fetch
  const hi=d.high||0; const lo=d.low||0;
  const hiEl=document.getElementById("ch-high");
  const loEl=document.getElementById("ch-low");
  if(hiEl)hiEl.textContent=hi>0?fmtPrice(hi):"–";
  if(loEl)loEl.textContent=lo>0?fmtPrice(lo):"–";
  // If high/low are missing, fetch ticker directly from MEXC public API
  if((hi<=0||lo<=0)&&currentPair){
    fetch(`https://contract.mexc.com/api/v1/contract/ticker?symbol=${currentPair}`)
      .then(r=>r.json()).then(data=>{
        const items=data&&data.data?(Array.isArray(data.data)?data.data:[data.data]):[];
        const t=items[0]||{};
        const h2=parseFloat(t.high24h||t.high||0);
        const l2=parseFloat(t.low24h||t.low||0);
        if(h2>0&&hiEl)hiEl.textContent=fmtPrice(h2);
        if(l2>0&&loEl)loEl.textContent=fmtPrice(l2);
        // Also update the cache so it doesn't re-fetch every second
        if(prices[currentPair]){
          if(h2>0)prices[currentPair].high=h2;
          if(l2>0)prices[currentPair].low=l2;
        }
      }).catch(()=>{});
  }
}
