...
def get_outbound_ip():
    try:
        ...
        logging.info(f"🚀 SIGNALCORE OUTBOUND IP: {ip}")
        print(f"🚀 SIGNALCORE OUTBOUND IP: {ip}")
        return ip
    except Exception as e:
        logging.error(f"Failed to get outbound IP: {e}")
        return "Unknown"

async def send_ip_to_telegram(ip):
    try:
        ...
        if token and chat_id:
            ...
            message = f"🚀 SIGNALCORE OUTBOUND IP:\n{ip}\n\nAdd this IP to MEXC API whitelist."
            payload = {"chat_id": chat_id, "text": message, "parse_mode": "HTML"}

            response = requests.post(url, json=payload, timeout=10)
            if response.status_code == 200:
                logging.info("✅ IP address sent to Telegram successfully")
            else:
                logging.error(f"Failed to send Telegram message: {response.text}")
        else:
            logging.warning("Telegram credentials not found in environment variables")
    except Exception as e:
        logging.error(f"Error sending IP to Telegram: {e}")
...
try:
    ...
    if loop.is_running():
        asyncio.ensure_future(send_ip_to_telegram(_outbound_ip))
    else:
        loop.run_until_complete(send_ip_to_telegram(_outbound_ip))
except Exception as _ip_err:
    logging.error(f"IP Telegram send error: {_ip_err}")
...
def mexc_sign(api_key, timestamp_ms, request_param, secret):
    ...
    - requestParam = for GET: sorted urlencode query string
                     for POST: exact JSON body string (same bytes sent in body)
