...
async function saveAllSettings(){
  ...
  try{
    const r=await fetch("/api/settings",{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});
    const d=await r.json();
    if(d.ok){
      showSettingsMsg("✅ All settings saved!",true);
      if(g("s-secret"))g("s-secret").value="";
      loadSettings();priceRefreshSecs=Math.max(1,payload.price_interval||5);
    }else{showSettingsMsg("❌ Save failed",false);}
  }catch{showSettingsMsg("❌ Network error",false);}
}
