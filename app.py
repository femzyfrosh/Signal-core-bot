...
def api_ticker(symbol):
    """Live ticker for a single symbol — always fetches direct from MEXC, never cache."""
    t = get_ticker(symbol)
    if t:
        # Also update the price cache so the batch stays fresh
        with price_cache_lock:
            price_cache[symbol] = t
        return jsonify({"ok": True, **t})
    return jsonify({"ok": False, "price": 0, "change": 0, "high": 0, "low": 0}), 200
...
def api_set_margin_mode():
    ...
    # Push the change to MEXC for every watchlist pair (best-effort)
    errors = []
    successes = 0
    for symbol in TOP_PAIRS:
        # Try both long and short positions
        for pos_type in (1, 2):
            params = {
                "symbol":       symbol,
                "openType":     mode,
            ...
            _, err = mexc_request("POST", "/position/change_margin_type", params)
            if err:
                errors.append(f"{symbol} pos{pos_type}: {err}")
            else:
                successes += 1

    msg = f"Margin mode set to {mode_label} on MEXC. {successes} position slots updated."
    if errors:
        msg += f" {len(errors)} minor errors (normal if no open position for that pair)."
        log(f"⚠️ Margin mode errors (expected for pairs with no position): {errors[:3]}")
...
async function fetchTickerForChart(sym){
  try{
    ...
    const d=await r.json();
    if(d&&d.price>0){
      prices[sym]=d;          // inject into local prices map immediately
      updateChartHeader();    // render right away with real data
    }
  }catch{}
}
...
function updateChartHeader(){
  ...
  if(highEl){highEl.textContent=highVal;highEl.style.color=highVal!=="–"?"var(--green)":"var(--dim)";}
  if(lowEl){lowEl.textContent=lowVal;lowEl.style.color=lowVal!=="–"?"var(--red)":"var(--dim)";}
