...
def scan_pair(symbol):
    ...
    if candles_1d and len(candles_1d)>=10:
        ...
        for crt in crts:
            ...
            if tbs_entry:
                entry      = tbs_entry           # TBS candle OPEN (inside CRT range)
                entry_type = "Model #1 (TBS Open)"
                # SL = TBS candle CLOSE (body that closed outside the CRT range)
                sl_p       = tbs_sl
                # TP = opposite CRT level: SELL → CRL, BUY → CRH
                tp_p       = round(crt["crl"], 8) if direction == "SELL" else round(crt["crh"], 8)
            else:
                entry      = crt["entry"]
                entry_type = "C2 Close (fallback)"
                sl_p       = crt["sl"]
                tp_p       = round(crt["crl"], 8) if direction == "SELL" else round(crt["crh"], 8)
...
def mexc_request(method, path, params=None, signed=True):
    ...
    IMPORTANT: always use HTTPS explicitly — HTTP will 403 because
    POST body is dropped on the HTTP→HTTPS redirect.
    """
    ...
    # POST: exact JSON body string (separators must match what we send)
    if method == "GET":
        if params:
    ...
    try:
        # Always use HTTPS — HTTP causes 403 on POST because redirect drops the body
        base = MEXC_FUTURES.replace("http://", "https://")
        url  = f"{base}{path}"
        if method == "GET":
        ...
        # Hard-catch any remaining redirect that would strip the POST body
        if r.status_code in (301, 302, 307, 308):
            redirect_url = r.headers.get("Location", url).replace("http://", "https://")
            if method == "GET":
                r = requests.get(redirect_url, params=params, headers=headers, timeout=15)
            else:
                r = requests.post(redirect_url, json=params, headers=headers, timeout=15)

        raw_text = r.text.strip() if r.text else ""
        if not raw_text:
            if r.status_code in (200, 201):
                return {}, None
            return None, f"Empty response (HTTP {r.status_code})"
        try:
            data = r.json()
        except ValueError:
        ...
        log(f"MEXC API error on {path}: {err_msg} | code={data.get('code')} | raw={raw_text[:120]}")
        return None, err_msg
    ...
    while True:
        try:
            ...
            for symbol in symbols:
                ...
                # Entry = TBS candle OPEN (inside CRT range — limit entry)
                ...
                rr       = round(reward/risk, 2) if risk > 0 else 0
                log(f"   Entry:{tbs_entry} | SL:{sl_final} | TP:{tp} | RR:{rr}R")
                ...
                # Get current market price for market order entry
                ticker_now = get_ticker(symbol)
                market_price = round(ticker_now["price"], 8) if ticker_now else tbs_entry
                ...
                sig = {
                    ...
                    "trend":       trend,
                    "entry":       market_price,
                    "entry_type":  "Model #1 (TBS — Market Entry)",
                    "sl":          sl_final,
                    "tp":          round(tp, 8),
                    "tp1":         round((market_price + tp) / 2, 8),
                    "tp2":         round(tp, 8),
                    "rr":          rr,
                    "crh":         crh,
                    "crl":         crl,
                    ...
                    "details":     [
                        ...
                        f"✅ TBS on {tbs_tf}: opened {tbs_entry} inside CRT range, body closed {tbs_sl} outside",
                        f"Entry: {market_price} (market) | SL: {sl_final} (TBS close) | TP: {'CRL' if direction=='SELL' else 'CRH'} {tp}",
                        f"RR: {rr}R",
                    ],
                    "tbs_found":   True,
                    "tbs_tf":      tbs_tf,
                    ...
                    "from_monitor":True,
                    "market_order":True,   # TBS has already formed — execute at market
                    "timestamp":   datetime.now(LOCAL_TZ).strftime("%Y-%m-%d %H:%M UTC+1"),
                }

                signals.appendleft(sig)
                send_telegram(fmt_tg(sig))
                log(f"🎯 M1 SIGNAL: {direction} {symbol} | {grade} | {rr}R | TBS:{tbs_tf} | MARKET @ {market_price} | TBS open was {tbs_entry}")

                # ── Place MARKET order now that TBS has confirmed ─────────
                ready, reason = live_trade_ready()
                if ready:
                    ok, msg = place_order(sig)
