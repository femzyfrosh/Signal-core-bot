...
function initDrawCanvas(){
  ...
  function onDown(e){
    ...
    if(hi<0){pDown=false;return;}

    const hitType=_hit(shapes[hi],x,y);
    _dragStart={cx,cy,cp};
    _snapShape=JSON.parse(JSON.stringify(shapes[hi]));

    // If this shape is already selected AND tap landed on a corner/ep → resize immediately
    // If not selected yet → select it; corner resize activates on next tap
    if(hitType&&(hitType.startsWith('corner')||hitType.startsWith('ep'))&&selectedIdx===hi){
      // Already selected + corner hit → RESIZE
      selectedIdx=hi;
      _mode='resizing';
      _resizeHandle=hitType;
    }else if(hitType&&(hitType.startsWith('corner')||hitType.startsWith('ep'))&&selectedIdx!==hi){
      // Not selected yet + corner hit → just select (corners show, next tap resizes)
      selectedIdx=hi;
      _mode='moving'; // treat as move until they actually drag a corner
      _resizeHandle=hitType;
    }else{
      // Body hit → select + move
      selectedIdx=hi;
      _mode='moving';
  ...
  function onMove(e){
    ...
    if(_mode==='moving'&&_snapShape&&_resizeHandle){
      _mode='resizing';
    }
