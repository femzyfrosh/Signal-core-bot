...
function initDrawCanvas(){
  ...
  function onDown(e){
    ...
    // This matches TradingView: clicking an existing shape selects
    // it regardless of what tool is active.
    ...
    if(drawTool==='rect'||drawTool==='trend'){
      if(cp){_dragStart={cx,cy,px:x,py:y,cp};_mode='drawing';}
      e.preventDefault();return;
    }

    // ── Nothing — deselect, let chart pan/zoom ───────────────
    selectedIdx=-1;redrawCanvas();
    // no preventDefault → chart handles pan/zoom
  }
  ...
  function onMove(e){
    ...
    if(moved<3)return;
    // Only block the event if WE are handling it (drawing/moving/resizing).
    // If mode is idle and no draw tool, let chart pan/zoom/pinch work normally.
    if(_mode==='idle'&&!drawTool){return;}
    e.preventDefault();
    const{x,y}=_canvasXY(canvas,cx,cy);
    const cp=_xyToChart(canvas,cx,cy);
