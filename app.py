...
/* ── Drawing tools ── */
.pen-btn{margin-left:auto;flex-shrink:0;width:30px;height:30px;border-radius:8px;border:1px solid var(--border);background:transparent;color:var(--dim);cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:1rem;transition:all .18s}
.pen-btn.active-mode{background:rgba(79,110,247,.15);border-color:rgba(79,110,247,.5);color:var(--accent)}
/* Tools popup */
.tools-popup{position:fixed;z-index:120;background:var(--card);border:1px solid var(--border);border-radius:12px;padding:8px;display:none;flex-direction:column;gap:4px;box-shadow:0 8px 32px rgba(0,0,0,.55);backdrop-filter:blur(14px);min-width:148px}
.tp-item{display:flex;align-items:center;gap:8px;padding:8px 10px;border-radius:8px;border:none;background:transparent;color:var(--text);font-size:.78rem;font-weight:600;cursor:pointer;transition:background .15s;text-align:left;width:100%}
.tp-item:hover{background:rgba(255,255,255,.06)}
.tp-item.active{background:rgba(79,110,247,.15);color:var(--accent)}
.tp-item.danger{color:var(--red)}
.tp-item.danger:hover{background:rgba(255,77,106,.1)}
.tp-ico{font-size:1rem;width:20px;text-align:center}
/* Shape settings popup */
.ss-popup{position:fixed;z-index:125;background:var(--card);border:1px solid var(--border);border-radius:14px;padding:14px;display:none;flex-direction:column;gap:10px;box-shadow:0 8px 36px rgba(0,0,0,.6);backdrop-filter:blur(16px);min-width:240px;max-width:280px}
.ss-popup-header{display:flex;align-items:center;justify-content:space-between}
.ss-close-x{background:none;border:none;color:var(--dim);font-size:.85rem;cursor:pointer;padding:2px 5px;border-radius:5px;transition:color .15s}
.ss-close-x:hover{color:var(--text)}
.ss-title{font-size:.72rem;font-weight:700;color:var(--dim);text-transform:uppercase;letter-spacing:.08em;margin-bottom:2px}
.ss-lbl{font-size:.68rem;font-weight:700;color:var(--dim);text-transform:uppercase;letter-spacing:.07em;margin-bottom:4px;display:block}
.ss-row{display:flex;align-items:center;gap:8px;font-size:.75rem;color:var(--text)}
.ss-row label{flex:1}
.ss-palette{display:flex;gap:5px;flex-wrap:wrap}
.ss-swatch{width:22px;height:22px;border-radius:5px;cursor:pointer;border:2px solid transparent;transition:border-color .15s;flex-shrink:0}
.ss-swatch.sel{border-color:#fff}
/* Thickness */
.ss-thickness-group{display:flex;gap:4px;align-items:center}
.ss-thick-btn{background:var(--s2);border:1px solid var(--border);border-radius:6px;color:var(--dim);font-size:.85rem;padding:3px 9px;cursor:pointer;transition:all .15s;font-family:var(--mono);line-height:1.3}
.ss-thick-btn.sel{background:rgba(79,110,247,.2);border-color:var(--accent);color:var(--text)}
/* Line style */
.ss-style-group{display:flex;gap:4px}
.ss-style-btn{background:var(--s2);border:1px solid var(--border);border-radius:6px;color:var(--dim);font-size:.7rem;padding:4px 8px;cursor:pointer;transition:all .15s;font-family:var(--mono);letter-spacing:.08em}
.ss-style-btn.sel{background:rgba(79,110,247,.2);border-color:var(--accent);color:var(--text)}
/* Toggle */
.ss-toggle{position:relative;display:inline-block;width:32px;height:18px;flex-shrink:0}
.ss-toggle input{opacity:0;width:0;height:0}
.ss-toggle-track{position:absolute;inset:0;background:var(--muted);border-radius:9px;cursor:pointer;transition:background .2s}
.ss-toggle input:checked+.ss-toggle-track{background:var(--accent)}
.ss-toggle-track::after{content:'';position:absolute;width:12px;height:12px;background:#fff;border-radius:50%;top:3px;left:3px;transition:transform .2s}
.ss-toggle input:checked+.ss-toggle-track::after{transform:translateX(14px)}
/* Info row */
.ss-info-row{background:var(--s2);border-radius:8px;padding:6px 8px;font-size:.63rem;font-family:var(--mono);color:var(--dim);line-height:1.7;display:none}
.ss-info-row.show{display:block}
.ss-btns{display:flex;gap:6px;margin-top:4px}
.ss-apply{flex:1;padding:7px;border-radius:8px;background:var(--accent);border:none;color:#fff;font-size:.74rem;font-weight:700;cursor:pointer}
.ss-delete{padding:7px 10px;border-radius:8px;background:rgba(255,77,106,.15);border:1px solid rgba(255,77,106,.3);color:var(--red);font-size:.74rem;font-weight:700;cursor:pointer}
...
.chart-loading{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:var(--bg);z-index:5;flex-direction:column;gap:10px}
...
/* ─── EMPTY ─── */
.empty-chart{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;text-align:center;padding:24px}
  ...
  <div class="main">
    ...
    <div class="page hidden-right" id="page-chart">
      ...
      <div class="tf-bar" id="tf-bar">
        ...
        <button class="pen-btn" id="pen-btn" onclick="toggleToolsPopup(event)" title="Drawing tools">✏️</button>
      </div>
      <!-- Tools popup -->
      <div class="tools-popup" id="tools-popup">
        <button class="tp-item" id="tp-trend" onclick="activateTool('trend')"><span class="tp-ico">📏</span>Trend Line</button>
        <button class="tp-item" id="tp-rect"  onclick="activateTool('rect')"><span class="tp-ico">⊡</span>Rectangle</button>
        <button class="tp-item" id="tp-sel"   onclick="activateTool('select')"><span class="tp-ico">↖</span>Select / Move</button>
        <button class="tp-item danger"         onclick="clearAllShapes()"><span class="tp-ico">🗑</span>Clear All</button>
      </div>
      <!-- Shape settings popup (double-tap on shape) -->
      <div class="ss-popup" id="ss-popup">
        <div class="ss-popup-header">
          <div class="ss-title" id="ss-popup-title">Shape Settings</div>
          <button class="ss-close-x" onclick="closeShapeSettings()">✕</button>
        </div>
        <!-- Colour rows -->
        <div id="ss-border-row">
          <div class="ss-lbl">Colour</div>
          <div class="ss-palette" id="ss-border-palette"></div>
        </div>
        <div id="ss-fill-row">
          <div class="ss-lbl">Fill colour</div>
          <div class="ss-palette" id="ss-fill-palette"></div>
        </div>
        <!-- Thickness -->
        <div class="ss-row">
          <label class="ss-lbl" style="margin-bottom:0">Thickness</label>
          <div class="ss-thickness-group" id="ss-thick-group">
            <button class="ss-thick-btn" data-w="1" onclick="selectThickness(1)">─</button>
            <button class="ss-thick-btn" data-w="2" onclick="selectThickness(2)">─</button>
            <button class="ss-thick-btn" data-w="3" onclick="selectThickness(3)">─</button>
          </div>
        </div>
        <!-- Line style (trend only) -->
        <div id="ss-style-row" class="ss-row" style="flex-direction:column;align-items:flex-start;gap:5px">
          <div class="ss-lbl">Line style</div>
          <div class="ss-style-group">
            <button class="ss-style-btn" data-style="solid"  onclick="selectLineStyle('solid')">——</button>
            <button class="ss-style-btn" data-style="dashed" onclick="selectLineStyle('dashed')">- - -</button>
            <button class="ss-style-btn" data-style="dotted" onclick="selectLineStyle('dotted')">· · ·</button>
          </div>
        </div>
        <!-- Extend (trend only) -->
        <div id="ss-extend-row" class="ss-row">
          <label class="ss-lbl" style="margin-bottom:0">Extend right (ray)</label>
          <label class="ss-toggle">
            <input type="checkbox" id="ss-extend-right">
            <span class="ss-toggle-track"></span>
          </label>
        </div>
        <div id="ss-extleft-row" class="ss-row">
          <label class="ss-lbl" style="margin-bottom:0">Extend left</label>
          <label class="ss-toggle">
            <input type="checkbox" id="ss-extend-left">
            <span class="ss-toggle-track"></span>
          </label>
        </div>
        <!-- Midline + opacity (rect only) -->
        <div id="ss-midline-row" class="ss-row">
          <label class="ss-lbl" style="margin-bottom:0">Middle line</label>
          <label class="ss-toggle">
            <input type="checkbox" id="ss-midline">
            <span class="ss-toggle-track"></span>
          </label>
        </div>
        <div id="ss-opacity-row" style="display:none">
          <div class="ss-lbl">Fill opacity</div>
          <input type="range" id="ss-opacity" min="0" max="50" value="12" style="width:100%;accent-color:var(--accent)">
          <div style="display:flex;justify-content:space-between;font-size:.6rem;color:var(--dim);margin-top:1px"><span>0%</span><span id="ss-opacity-val">12%</span><span>50%</span></div>
        </div>
        <!-- Price info (read-only, rect only) -->
        <div id="ss-info-row" class="ss-info-row"></div>
        <!-- Action buttons -->
        <div class="ss-btns">
          <button class="ss-apply" onclick="applyShapeSettings()">Apply</button>
          <button class="ss-delete" onclick="deleteSelectedShape()">Delete</button>
        </div>
      </div>
      ...
      <div class="chart-wrap">
        ...
        <!-- Drawing canvas overlay — always receives events; passes through to chart when idle -->
        <canvas id="draw-canvas" style="position:absolute;inset:0;z-index:6;pointer-events:auto;display:block;cursor:default"></canvas>
...
// ── State ──────────────────────────────────────────────────────────
let prices={};      // {symbol: {price, change, high, low}}
let signals_map={}; // {symbol: latest_signal}
...
// Timeframe durations in seconds (used by countdown timer)
const TF_SECS={
  Min1:60,Min2:120,Min3:180,Min4:240,Min5:300,Min10:600,
  Min15:900,Min30:1800,Min45:2700,Min60:3600,
  Hour2:7200,Hour3:10800,Hour4:14400,Hour8:28800,Day1:86400
};

// Compute the right priceFormat for LightweightCharts based on the current price
...
// Update the candle-close countdown (in chart header, below price row)
// Display rules:
//   TF < 1 hr  (Min1–Min60)  → always MM:SS
//   TF >= 2 hr, remaining < 60 min → MM:SS
//   TF >= 2 hr, remaining >= 60 min → Xh YYm  (e.g. "2h 34m")
function updateCountdown(){
  ...
  let txt;
  const isLongTF=secs>=7200; // Hour2 and above
  if(isLongTF&&remaining>=60){
    const totalMins=Math.floor(remaining/60);
    if(totalMins>=60){
      // Show hours + minutes  e.g. "2h 34m"
      const h=Math.floor(totalMins/60);
      const m=totalMins%60;
      txt=h+'h '+String(m).padStart(2,'0')+'m';
    }else{
      // Under 60 minutes on a long TF — show MM:SS
      const m=totalMins;
      const s=remaining%60;
      txt=String(m).padStart(2,'0')+':'+String(s).padStart(2,'0');
    }
  }else{
    // Short TFs (< 2hr) or final minute of long TF — always MM:SS
...
// ── Persistence ────────────────────────────────────────────────────
function saveLastPair(sym,tf){
  try{localStorage.setItem("sc_last_pair",sym);localStorage.setItem("sc_last_tf",tf||"Min5");}catch{}
}
function loadLastPair(){
  try{return{pair:localStorage.getItem("sc_last_pair"),tf:localStorage.getItem("sc_last_tf")||"Min5"};}
  catch{return{pair:null,tf:"Min5"};}
}

// ── Nav ────────────────────────────────────────────────────────────
function goToWatchlist(){
  const wp=document.getElementById("page-watchlist");
  const cp=document.getElementById("page-chart");
  wp.className="page visible";
  cp.className="page hidden-right";
...
async function goToChart(sym,tf){
  const wp=document.getElementById("page-watchlist");
  const cp=document.getElementById("page-chart");
  wp.className="page hidden-left";
  cp.className="page visible";
...
function goToLastChart(){
  ...
  if(pair){goToChart(pair,tf);}
    ...
    // No last chart — animate to chart page showing empty state
    const wp=document.getElementById("page-watchlist");
    const cp=document.getElementById("page-chart");
    wp.className="page hidden-left";
    cp.className="page visible";
...
async function fetchSignals(){
  try{
    ...
    signals_map={};
    arr.forEach(s=>{
      if(!signals_map[s.symbol]) signals_map[s.symbol]=s;
    });
    renderWatchlist();
  }catch{}
}
...
function pairColor(sym){
  let h=0;for(let c of sym){h=(h*31+c.charCodeAt(0))&0xffff;}
  return PAIR_COLORS[h%PAIR_COLORS.length];
}
function pairEmoji(sym){
  const base=sym.replace("_USDT","");
  const map={BTC:"₿",ETH:"Ξ",SOL:"◎",BNB:"⬡",AVAX:"🏔",LINK:"🔗",INJ:"💉",SUI:"💧",NEAR:"🔮",KAS:"💎",XRP:"✦",ARB:"🔵",HBAR:"ℏ",SEI:"🌊",APT:"⚡",TAO:"τ",RENDER:"🎨",FET:"🤖"};
  return map[base]||base.slice(0,2).toUpperCase();
}
function fmtPrice(v){
...
function renderWatchlist(){
  ...
  list.innerHTML=filtered.map(sym=>{
    const d=prices[sym]||{};
    const sig=signals_map[sym];
    const [clr,bg]=pairColor(sym);
    const base=sym.replace("_USDT","");
    const chg=Number(d.change||0);
    const up=chg>0,dn=chg<0;
    const chgCls=up?"up":dn?"dn":"flat";
    const chgTxt=(up?"+":"")+chg.toFixed(2)+"%";
    const hasPrice=d.price&&d.price>0;
    const sigDir=sig?sig.direction:"";
    ...
    return`<div class="wl-row${sigClasses}" onclick="openPair('${sym}')">
      <div class="wl-info">
        <div class="wl-sym">${base}<span>/USDT</span>${sig?`<span class="wl-sig-dot" style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${badgeColor};margin-left:6px;vertical-align:middle;animation:blink 1.5s infinite"></span>`:""}</div>
...
async function loadChart(sym,tf){
  ...
  // Fetch ticker directly for this symbol — guarantees fresh high/low immediately
  ...
  // Determine precision from current price so right-axis shows exact digits
  ...
  // Draw all signal analysis lines from backend
  await drawSignalLines(sym);

  chart.timeScale().fitContent();
  // Sync draw canvas to chart immediately after render
  requestAnimationFrame(()=>{ resizeDrawCanvas(); redrawCanvas(); });
  document.getElementById("chart-loading").classList.add("hidden");
  ...
  // Initialise drawing canvas for this chart
  initDrawCanvas();
...
// ── Draw CRH/CRL/TBS/OB/Entry/SL/TP lines from signal data ─────────
  ...
  try{
    ...
    function pline(price,color,title,style,width){
      try{
        ...
        candleSeries.createPriceLine({price:v,color,lineWidth:width||1,lineStyle:style||LS.Dashed,axisLabelVisible:true,title});
      }catch(e){}
    }
    // CRT Range High / Low (yellow dotted)
    pline(d.crh,"rgba(255,201,60,.9)","CRH",LS.Dotted,1);
    pline(d.crl,"rgba(255,201,60,.9)","CRL",LS.Dotted,1);
    // Order Block zone top / bottom (purple dotted)
    pline(d.ob_top,"rgba(167,139,250,.85)","OB↑",LS.Dotted,1);
    pline(d.ob_bot,"rgba(167,139,250,.85)","OB↓",LS.Dotted,1);
    // TBS entry line (orange dashed)
    pline(d.tbs_entry,"rgba(251,146,60,.85)","TBS",LS.Dashed,1);
    // Entry / SL / TP (solid / colored)
...
// Fetch live ticker for the chart symbol directly — bypasses batch cache
// Guard: skip if a fetch for this symbol is already in-flight (prevents blink from double-render)
const _tickerFetching={};
async function fetchTickerForChart(sym){
  if(_tickerFetching[sym])return;
  _tickerFetching[sym]=true;
  try{
    const r=await fetch("/api/ticker/"+sym);
    if(!r.ok)return;
    const d=await r.json();
    if(d&&d.price>0){
      // Merge: only overwrite high/low if new value is valid so we never blank a good cached value
      const prev=prices[sym]||{};
      prices[sym]={
        ...prev,
        price: d.price,
        change: d.change,
        high:  (d.high  && d.high  > 0) ? d.high  : prev.high,
        low:   (d.low   && d.low   > 0) ? d.low   : prev.low,
      };
      updateChartHeader();
    }
...
function updateChartHeader(){
  ...
  const lowEl=document.getElementById("ch-low");
  const highVal=fmtHL(d.high);
  const lowVal=fmtHL(d.low);
  // Only update high/low DOM if the value actually changed — prevents blink on every poll
  if(highEl&&highVal!=="–"){
    if(highEl.textContent!==highVal){highEl.textContent=highVal;highEl.style.color="var(--green)";}
  }else if(highEl&&highEl.textContent===""){
    highEl.textContent="–";highEl.style.color="var(--dim)";
  }
  if(lowEl&&lowVal!=="–"){
    if(lowEl.textContent!==lowVal){lowEl.textContent=lowVal;lowEl.style.color="var(--red)";}
  }else if(lowEl&&lowEl.textContent===""){
    lowEl.textContent="–";lowEl.style.color="var(--dim)";
  }
  // If high/low still missing, fire a background fetch to fill them in
...
async function updateLatestCandle(sym,tf){
  ...
  try{
    ...
    if(candles.length>0){
      // Update last 2 candles (current forming + previous)
      const last2=candles.slice(-2);
      last2.forEach(c=>candleSeries.update(c));
    }
  }catch{}
  // Always refresh ticker so high/low stay current
  await fetchTickerForChart(sym);
}

function startChartUpdate(sym,tf){
  stopChartUpdate();
  // Candle OHLC data refreshes every 3s; price in header updates via priceTimer (1s)
...
// ══════════════════════════════════════════════════════════════════
// ── Drawing Engine v4 — TradingView-style ────────────────────────
// ══════════════════════════════════════════════════════════════════
//
//  ARCHITECTURE:
//  - A transparent canvas sits over the LightweightCharts container.
//  - When drawTool is active the canvas captures ALL pointer events.
//  - In smart/null mode: hit-test first; if a shape is hit we handle
//    the event (move/resize); if nothing is hit we let events fall
//    through to the chart for pan/zoom by NOT calling preventDefault.
//
//  RECTANGLE (TradingView behaviour):
//    Draw  : click-drag from corner A to corner B.
//    Move  : drag anywhere inside the body (not a corner handle).
//    Resize: drag any of the 4 corner handles — the opposite corner
//            stays fixed (true corner-pin resize).
//    Select: single-click selects, shows 4 corner handles + price
//            labels. Double-click opens settings panel.
//    Props : borderColor, fillColor, lineWidth, fillOpacity, midLine.
//
//  TRENDLINE (TradingView behaviour):
//    Draw     : click-drag P1→P2.
//    Move     : drag the line body — both endpoints translate.
//    Resize   : drag endpoint handle ep-0 (P1) or ep-1 (P2).
//    Ray      : extendRight=true extends the line to canvas edge.
//    Parallel : Shift+drag-body creates a parallel clone at the new
//               offset; released on mouseUp.
//    Angle    : displayed near P2 when line is selected.
//    Props    : color, lineWidth, lineStyle, extendRight, extendLeft.
//
//  STATE:
//    shapes[]     — persistent array of all drawn shape objects.
//    selectedIdx  — index into shapes[] of currently selected shape
//                   (-1 = none).
//    previewShape — transient shape drawn while mouse is held during
//                   a new-shape drag; committed on mouseUp.
//    _parallelClone — ghost of the original trendline while a
//                     Shift+drag parallel is being positioned.
...
// ── Internal drag state (reset on every mouseDown) ─────────────
let _mode          = 'idle'; // 'idle'|'drawing'|'moving'|'resizing'
let _dragStart     = null;   // {cx,cy,cp:{time,price}} captured at pointerDown
let _snapShape     = null;   // deep-copy of shape at drag start (for delta moves)
let _resizeHandle  = null;   // which handle is being dragged:
                             //   rect  → 'corner-0'..'corner-3'
                             //   trend → 'ep-0' | 'ep-1'
let _parallelClone = null;   // original trendline snapshot kept as ghost while
                             // Shift+drag positions the parallel copy
let _shiftDown     = false;  // live Shift key state for parallel-line shortcut

const COLORS  = ['#4f6ef7','#00d4aa','#ff4d6a','#ffc93c','#a855f7','#f97316','#e2e8f0','#06b6d4'];
const HR      = 6;   // handle circle radius (px) — drawn on canvas
const HIT     = 12;  // line/border hit tolerance (px)
const HCORNER = 16;  // corner/endpoint hit radius (px) — generous for touch

// ── Coordinate helpers ─────────────────────────────────────────
// Convert between chart-domain values and canvas pixel coordinates.
function _p2y(p){return candleSeries?candleSeries.priceToCoordinate(p):null;}
function _t2x(t){return chart?chart.timeScale().timeToCoordinate(t):null;}
function _y2p(y){return candleSeries?candleSeries.coordinateToPrice(y):null;}

// Convert a client-space pointer position to {px,py,price,time}.
// Returns null when the pointer is outside the canvas or the chart
// has not been initialised yet.
function _xyToChart(canvas,cx,cy){
  if(!candleSeries||!chart)return null;
  const bb=canvas.getBoundingClientRect();
  const px=cx-bb.left, py=cy-bb.top;
...
// Like _xyToChart but clamps client coords to the canvas boundary
// before converting. Used during drag-move so dragging past the edge
// of the canvas doesn't drop the shape.
function _clampedChartCoord(canvas,cx,cy){
  if(!candleSeries||!chart)return null;
  const bb=canvas.getBoundingClientRect();
  const px=Math.max(0,Math.min(bb.width, cx-bb.left));
  const py=Math.max(0,Math.min(bb.height,cy-bb.top));
  const price=candleSeries.coordinateToPrice(py);
  const time=chart.timeScale().coordinateToTime(px);
  if(price==null||time==null)return null;
  return{px,py,price,time};
...
// Convert client coords to canvas-local pixel coords {x,y}.
function _canvasXY(canvas,cx,cy){
  const bb=canvas.getBoundingClientRect();
  return{x:cx-bb.left,y:cy-bb.top};
}

// ── Canvas resize ──────────────────────────────────────────────
// Keeps the drawing canvas pixel-perfect with the chart container.
...
// ── Colour helper ──────────────────────────────────────────────
// Converts a #rrggbb hex string to rgba(r,g,b,a) with given alpha.
function _rgba(hex,a){
  if(!hex||hex.length<7)return`rgba(79,110,247,${a})`;
  const r=parseInt(hex.slice(1,3),16),g=parseInt(hex.slice(3,5),16),b=parseInt(hex.slice(5,7),16);
  return`rgba(${r},${g},${b},${a})`;
}

// ── Dash arrays ────────────────────────────────────────────────
// Returns a canvas setLineDash() array for the given style name.
function _dash(style,lw){
  if(style==='dashed')return[lw*5,lw*3];
  if(style==='dotted')return[1.5,lw*3];
  return[]; // solid
}
...
// ── Extend trendline to canvas edges ──────────────────────────
// Given the two screen-pixel endpoints (x1,y1)→(x2,y2) and the
// canvas dimensions W×H, returns the clipped start/end that reaches
// the canvas boundary in each requested direction.
// extR = extend toward positive-x (right / future prices = ray)
// extL = extend toward negative-x (left)
function _extendLine(x1,y1,x2,y2,W,H,extR,extL){
  let sx=x1,sy=y1,ex=x2,ey=y2;
  const dx=x2-x1,dy=y2-y1;
  if(Math.abs(dx)<0.001&&Math.abs(dy)<0.001)return{sx,sy,ex,ey};

  // y at a given x along the infinite line
  function yAtX(x){return y1+(x-x1)*(dy/dx);}
  // x at a given y
  function xAtY(y){return x1+(y-y1)*(dx/dy);}

  // Clamp a pixel coord to the nearest horizontal canvas edge
  function clampH(y){return Math.max(0,Math.min(H,y));}

  if(extR&&Math.abs(dx)>0.001){
    const yr=yAtX(W);
    if(yr>=0&&yr<=H){ex=W;ey=yr;}
    else{const yy=clampH(yr);ex=xAtY(yy);ey=yy;}
  }
  if(extL&&Math.abs(dx)>0.001){
    const yl=yAtX(0);
    if(yl>=0&&yl<=H){sx=0;sy=yl;}
    else{const yy=clampH(yl);sx=xAtY(yy);sy=yy;}
  }
  return{sx,sy,ex,ey};
}

// ── Hit test ───────────────────────────────────────────────────
// Returns false (no hit) or a string describing which part was hit:
//   'body'      — the line/border/interior (used for move)
//   'corner-N'  — rect corner handle 0..3  (used for resize)
//   'mid-N'     — rect mid-edge handle 0..3: 0=top 1=right 2=bot 3=left
//   'ep-N'      — trendline endpoint 0|1   (used for resize)
//
// Corner numbering (matches TradingView):
//   0=top-left  1=top-right  2=bottom-left  3=bottom-right
function _hit(sh,x,y){
  if(sh.type==='rect'){
    const x1=_t2x(sh.t1),y1=_p2y(sh.p1),x2=_t2x(sh.t2),y2=_p2y(sh.p2);
    ...
    // ── Corner handles take priority (checked first, generous radius) ──
    // Corners: [TL, TR, BL, BR]
    const corners=[[lx,ty],[rx,ty],[lx,by],[rx,by]];
    for(let i=0;i<4;i++){
      if(Math.hypot(x-corners[i][0],y-corners[i][1])<HCORNER)return`corner-${i}`;
    }

    // ── Mid-edge handles (checked next, same generous radius) ──
    // mid-0=top-centre, mid-1=right-centre, mid-2=bot-centre, mid-3=left-centre
    const midX=(lx+rx)/2, midY=(ty+by)/2;
    const mids=[[midX,ty],[rx,midY],[midX,by],[lx,midY]];
    for(let i=0;i<4;i++){
      if(Math.hypot(x-mids[i][0],y-mids[i][1])<HCORNER)return`mid-${i}`;
    }

    // ── Border edges ──
    const onTop  =Math.abs(y-ty)<HIT&&x>=lx-HIT&&x<=rx+HIT;
    const onBot  =Math.abs(y-by)<HIT&&x>=lx-HIT&&x<=rx+HIT;
    const onLeft =Math.abs(x-lx)<HIT&&y>=ty-HIT&&y<=by+HIT;
    const onRight=Math.abs(x-rx)<HIT&&y>=ty-HIT&&y<=by+HIT;
    if(onTop||onBot||onLeft||onRight)return'body';

    // ── Interior (allows dragging from anywhere inside) ──
    if(x>lx&&x<rx&&y>ty&&y<by)return'body';
    return false;
  }
  ...
  if(sh.type==='trend'){
    ...
    // ── Endpoint handles take priority ──
    if(Math.hypot(x-x1,y-y1)<HCORNER)return'ep-0';
    if(Math.hypot(x-x2,y-y2)<HCORNER)return'ep-1';

    // ── Line body: use point-to-segment distance ──
    const dx=x2-x1,dy=y2-y1,len2=dx*dx+dy*dy;
    if(len2<1)return Math.hypot(x-x1,y-y1)<HIT?'body':false;
    // Project point onto the segment; t ∈ [0,1] = within segment
    const t=Math.max(0,Math.min(1,((x-x1)*dx+(y-y1)*dy)/len2));
    const dist=Math.hypot(x-x1-t*dx,y-y1-t*dy);
    return dist<HIT?'body':false;
  }
  return false;
}

// Return the index of the topmost (last-drawn) shape under (x,y),
// or -1 if nothing is hit.
function _findShape(x,y){
  for(let i=shapes.length-1;i>=0;i--)if(_hit(shapes[i],x,y))return i;
  return -1;
}

// ── Cursor ─────────────────────────────────────────────────────
// Updates the canvas cursor to match the current hover context.
// Shows directional resize cursors for rect corners (matching
// TradingView), a move cursor for bodies, and crosshair when drawing.
function _cursor(canvas,x,y){
  if(drawTool==='rect'||drawTool==='trend'){canvas.style.cursor='crosshair';return;}
  for(let i=shapes.length-1;i>=0;i--){
    const h=_hit(shapes[i],x,y);
    if(!h)continue;
    if(h==='body'){canvas.style.cursor='move';return;}
    if(h==='corner-0'||h==='corner-3'){canvas.style.cursor='nwse-resize';return;}
    if(h==='corner-1'||h==='corner-2'){canvas.style.cursor='nesw-resize';return;}
    if(h==='mid-0'||h==='mid-2'){canvas.style.cursor='ns-resize';return;}   // top/bottom edges
    if(h==='mid-1'||h==='mid-3'){canvas.style.cursor='ew-resize';return;}   // left/right edges
    if(h==='ep-0'||h==='ep-1'){canvas.style.cursor='crosshair';return;}
  }
  canvas.style.cursor='default';
}

// ── Draw one shape ─────────────────────────────────────────────
// Renders a single shape onto ctx.
// isSel  = true → draw selection handles, labels, angle badge
// isPrev = true → draw as translucent "ghost" preview (while dragging)
function _draw(ctx,sh,isSel,isPrev){
  ...
  if(sh.type==='rect'){
    ...
    const bc=isPrev?'rgba(79,110,247,.75)':(sh.borderColor||'#4f6ef7');
    const fo=isPrev?0.06:((sh.fillOpacity||12)/100);
    const lw=sh.lineWidth||1.5;

    ctx.save();

    // ── Fill ──
    ctx.fillStyle=_rgba(sh.fillColor||'#4f6ef7',fo);
    ctx.fillRect(lx,ty,w,h);

    // ── Border ──
    ctx.strokeStyle=bc;
    // Selected border is slightly thicker for visibility
    ctx.lineWidth=isSel?Math.max(lw,2):lw;
    ctx.setLineDash(isPrev?[5,3]:[]);
    ctx.strokeRect(lx,ty,w,h);
    ctx.setLineDash([]);

    // ── Midline (horizontal dashed line through centre) ──
    if(sh.midLine&&!isPrev){
      ctx.beginPath();
      ctx.strokeStyle=bc;
      ctx.lineWidth=Math.max(1,lw*0.6);
      ctx.setLineDash([4,4]);
      ctx.moveTo(lx,ty+h/2);ctx.lineTo(rx,ty+h/2);
      ctx.stroke();
      ctx.setLineDash([]);
    }

    // ── Selection handles + labels ──
    if(isSel&&!isPrev){
      ...
      // ── Four mid-edge handles: top, right, bottom, left ──
      // These are the NEW mid-point handles for edge-only resize.
      const midX=(lx+rx)/2,midY=(ty+by)/2;
      const mids=[[midX,ty],[rx,midY],[midX,by],[lx,midY]];
      mids.forEach(([hx,hy])=>{
        ctx.beginPath();
        // Diamond shape (rotated square) to visually distinguish from corner circles
        ctx.save();
        ctx.translate(hx,hy);
        ctx.rotate(Math.PI/4);
        const s=HR-1; // diamond half-size
      ...
      // ── Δ% badge above the rectangle ──
      if(sh.p1!=null&&sh.p2!=null){
        const hi=Math.max(sh.p1,sh.p2),lo=Math.min(sh.p1,sh.p2);
        if(lo>0){
          const pct=(Math.abs((hi-lo)/lo)*100).toFixed(2)+'%';
          ctx.font='bold 10px JetBrains Mono,monospace';
          const tw=ctx.measureText(pct).width;
          const bx=lx+w/2-(tw+12)/2;
          const badgeY=Math.max(4,ty-18); // keep badge inside canvas
          ctx.fillStyle='rgba(7,8,16,.85)';
          ctx.fillRect(bx,badgeY,tw+12,16);
          ctx.fillStyle=bc;
          ctx.fillText(pct,bx+6,badgeY+11);
        }
      }
      ...
      ctx.fillStyle=bc;
      ctx.fillText(_fp(Math.max(sh.p1,sh.p2)),rx+6,ty+4);
      ctx.fillText(_fp(Math.min(sh.p1,sh.p2)),rx+6,by+4);
    }
    ctx.restore();
  ...
  // TRENDLINE
  // ════════════════════════════════════════
  }else if(sh.type==='trend'){
    const x1=_t2x(sh.t1),y1=_p2y(sh.p1),x2=_t2x(sh.t2),y2=_p2y(sh.p2);
    if(x1==null||x2==null||y1==null||y2==null)return;

    const col=isPrev?'rgba(255,201,60,.75)':(sh.color||'#ffc93c');
    const lw=sh.lineWidth||1.8;

    // Compute the potentially-extended start/end pixels
    // Preview never extends; committed shape respects extendRight/Left flags
    const ext=_extendLine(
      x1,y1,x2,y2,W,H,
      !isPrev&&!!sh.extendRight,
      !isPrev&&!!sh.extendLeft
    );

    ctx.save();
    ctx.strokeStyle=col;
    ctx.lineWidth=lw;
    ctx.setLineDash(isPrev?[6,3]:_dash(sh.lineStyle||'solid',lw));
    ctx.beginPath();
    ...
    // ── Ray arrowhead at the right-extended tip ──
    // Small filled triangle indicates the ray direction (like TradingView)
    if(!isPrev&&sh.extendRight){
      const aw=7; // arrowhead width (half)
      const al=10; // arrowhead length
      const dx=ext.ex-ext.sx,dy=ext.ey-ext.sy;
      const len=Math.hypot(dx,dy)||1;
      const ux=dx/len,uy=dy/len;   // unit vector along line
      const nx=-uy,ny=ux;          // normal
      const tip_x=ext.ex,tip_y=ext.ey;
      ctx.beginPath();
      ctx.moveTo(tip_x,tip_y);
      ctx.lineTo(tip_x-al*ux+aw*nx,tip_y-al*uy+aw*ny);
      ctx.lineTo(tip_x-al*ux-aw*nx,tip_y-al*uy-aw*ny);
      ctx.closePath();
      ctx.fillStyle=col;ctx.fill();
    }

    // ── Selection handles + angle badge ──
    if(isSel&&!isPrev){
      // Endpoint handles at the actual P1/P2 pixel coords (not extended)
      ...
      // ── Angle badge near P2 ──
      // atan2 uses screen Y which is inverted, so negate dy
      const ang=Math.round(Math.atan2(-(y2-y1),(x2-x1))*180/Math.PI);
      const atxt=ang+'°';
      ctx.font='bold 10px JetBrains Mono,monospace';
      const tw=ctx.measureText(atxt).width;
      // Position badge so it stays inside the canvas
      const bx=Math.min(x2+10,W-tw-14);
      const badgeY=Math.max(4,y2-16);
      ctx.fillStyle='rgba(7,8,16,.85)';
      ctx.fillRect(bx,badgeY,tw+10,16);
      ctx.fillStyle=col;
      ctx.fillText(atxt,bx+5,badgeY+11);

      // ── Price labels at each endpoint ──
      ctx.font='500 9.5px JetBrains Mono,monospace';
      ctx.fillStyle=col;
      // P1 label above-right of endpoint 1
      ctx.fillText(_fp(sh.p1),x1+9,y1-7);
      // P2 label below-right of endpoint 2 (avoids overlapping angle badge)
      ctx.fillText(_fp(sh.p2),x2+9,y2+16);
    }
    ctx.restore();
  }
}
...
function redrawCanvas(){
  ...
  shapes.forEach((sh,i)=>_draw(ctx,sh,i===selectedIdx,false));
  if(previewShape)_draw(ctx,previewShape,false,true);
  if(_parallelClone)_draw(ctx,_parallelClone,false,true);
}

// ── Palette builder ────────────────────────────────────────────
function buildPalette(id,initial){
  const el=document.getElementById(id);if(!el)return;
  el.innerHTML='';
  COLORS.forEach(c=>{
    const sw=document.createElement('div');
    sw.className='ss-swatch'+(c===initial?' sel':'');
    sw.style.background=c;sw.dataset.color=c;
    sw.onclick=()=>{el.querySelectorAll('.ss-swatch').forEach(s=>s.classList.remove('sel'));sw.classList.add('sel');};
    el.appendChild(sw);
  });
}

// ── Tool UI helpers ────────────────────────────────────────────
function _setToolUI(tool){
  ['tp-trend','tp-rect','tp-sel'].forEach(id=>{
    const el=document.getElementById(id);
    if(el)el.classList.toggle('active',
      (tool==='trend'&&id==='tp-trend')||(tool==='rect'&&id==='tp-rect')||(tool==='select'&&id==='tp-sel'));
  });
  const pb=document.getElementById('pen-btn');
  if(pb)pb.classList.toggle('active-mode',!!tool);
  const canvas=document.getElementById('draw-canvas');
  if(!canvas)return;
  // Canvas always intercepts; cursor changes to show intent
  if(tool==='rect'||tool==='trend'){
...
window.toggleToolsPopup=function(e){
  e.stopPropagation();
  const p=document.getElementById('tools-popup');
...
window.activateTool=function(tool){
  drawTool=tool;selectedIdx=-1;previewShape=null;_mode='idle';
  document.getElementById('tools-popup').style.display='none';
  _setToolUI(tool);redrawCanvas();
};

window.clearAllShapes=function(){
  shapes=[];selectedIdx=-1;previewShape=null;drawTool=null;_mode='idle';
  document.getElementById('tools-popup').style.display='none';
  _setToolUI(null);redrawCanvas();
};

window.selectThickness=function(w){
  document.querySelectorAll('.ss-thick-btn').forEach(b=>b.classList.toggle('sel',parseInt(b.dataset.w)===w));
};
window.selectLineStyle=function(s){
  document.querySelectorAll('.ss-style-btn').forEach(b=>b.classList.toggle('sel',b.dataset.style===s));
};
...
window.applyShapeSettings=function(){
  ...
  const bc=document.querySelector('#ss-border-palette .ss-swatch.sel');
  const thk=document.querySelector('.ss-thick-btn.sel');
  if(thk)sh.lineWidth=parseFloat(thk.dataset.w);
  if(sh.type==='rect'){
    if(bc)sh.borderColor=bc.dataset.color;
    const fc=document.querySelector('#ss-fill-palette .ss-swatch.sel');
    if(fc)sh.fillColor=fc.dataset.color;
    sh.midLine=document.getElementById('ss-midline').checked;
    sh.fillOpacity=parseFloat(document.getElementById('ss-opacity').value)||12;
  }else{
    if(bc)sh.color=bc.dataset.color;
    const stl=document.querySelector('.ss-style-btn.sel');
    if(stl)sh.lineStyle=stl.dataset.style;
    sh.extendRight=document.getElementById('ss-extend-right').checked;
    sh.extendLeft=document.getElementById('ss-extend-left').checked;
  }
  document.getElementById('ss-popup').style.display='none';
  redrawCanvas();
};

window.deleteSelectedShape=function(){
  if(selectedIdx>=0){shapes.splice(selectedIdx,1);selectedIdx=-1;}
  document.getElementById('ss-popup').style.display='none';
  redrawCanvas();
};

window.closeShapeSettings=function(){document.getElementById('ss-popup').style.display='none';};

function _openSettings(idx){
  if(idx<0||idx>=shapes.length)return;
  const sh=shapes[idx];
  const isRect=sh.type==='rect';
  document.getElementById('ss-popup-title').textContent=isRect?'Rectangle':'Trend Line';
  buildPalette('ss-border-palette',sh.borderColor||sh.color||'#4f6ef7');

  // show/hide rows
  document.getElementById('ss-fill-row').style.display    =isRect?'block':'none';
  document.getElementById('ss-midline-row').style.display =isRect?'flex':'none';
  document.getElementById('ss-opacity-row').style.display =isRect?'block':'none';
  document.getElementById('ss-style-row').style.display   =isRect?'none':'flex';
  document.getElementById('ss-extend-row').style.display  =isRect?'none':'flex';
  document.getElementById('ss-extleft-row').style.display =isRect?'none':'flex';

  if(isRect){
    buildPalette('ss-fill-palette',sh.fillColor||'#4f6ef7');
    document.getElementById('ss-midline').checked=!!sh.midLine;
    const op=document.getElementById('ss-opacity');
    const ov=document.getElementById('ss-opacity-val');
    op.value=sh.fillOpacity||12;ov.textContent=op.value+'%';
    op.oninput=()=>ov.textContent=op.value+'%';
    // Info
    const hi=Math.max(sh.p1,sh.p2),lo=Math.min(sh.p1,sh.p2);
    const pct=(Math.abs((hi-lo)/lo)*100).toFixed(2);
    const ir=document.getElementById('ss-info-row');
    ir.innerHTML=`Top: ${_fp(hi)}&nbsp;&nbsp;Bot: ${_fp(lo)}&nbsp;&nbsp;Δ ${pct}%`;
    ir.className='ss-info-row show';
  }else{
    document.getElementById('ss-extend-right').checked=!!sh.extendRight;
    document.getElementById('ss-extend-left').checked=!!sh.extendLeft;
    selectLineStyle(sh.lineStyle||'solid');
    const x1=_t2x(sh.t1),y1=_p2y(sh.p1),x2=_t2x(sh.t2),y2=_p2y(sh.p2);
    const ang=(x1!=null&&x2!=null)?Math.round(Math.atan2(-(y2-y1),(x2-x1))*180/Math.PI):null;
    const ir=document.getElementById('ss-info-row');
    ir.innerHTML=`P1: ${_fp(sh.p1)}&nbsp;&nbsp;P2: ${_fp(sh.p2)}`+(ang!=null?`&nbsp;&nbsp;Angle: ${ang}°`:'');
    ir.className='ss-info-row show';
  }

  // Thickness buttons
  const cw=sh.lineWidth||1.5;
  document.querySelectorAll('.ss-thick-btn').forEach(b=>b.classList.remove('sel'));
  const match=[...document.querySelectorAll('.ss-thick-btn')]
    .sort((a,b)=>Math.abs(parseFloat(a.dataset.w)-cw)-Math.abs(parseFloat(b.dataset.w)-cw));
  if(match[0])match[0].classList.add('sel');
...
document.addEventListener('click',function(e){
  const tp=document.getElementById('tools-popup');
  if(tp&&tp.style.display==='flex'&&!tp.contains(e.target)&&e.target.id!=='pen-btn')
...
// Guard: track whether canvas listeners have been attached.
// initDrawCanvas() is called on every loadChart(); without this guard
// each call stacks a new set of event handlers. Stacked handlers
// all fire simultaneously and the redundant ones call preventDefault
// on every pointer event — breaking LightweightCharts pan/zoom.
...
function initDrawCanvas(){
  ...
  // Only attach listeners ONCE for the lifetime of the page.
  // Subsequent loadChart() calls just resize the canvas and return.
  if(_canvasListenersAttached)return;
  _canvasListenersAttached=true;

  let pDown=false,startCX=0,startCY=0;
  let lastTapMs=0,lastTapIdx=-1;

  // Unified pointer extraction for mouse and touch
  function _cp(e){
  ...
  // Uses a named function so it is only registered once (guarded above).
  canvas.addEventListener('mousemove',function(e){
    if(pDown)return;
    const{cx,cy}=_cp(e);
    const{x,y}=_canvasXY(canvas,cx,cy);
    _cursor(canvas,x,y);
  });

  // Track live Shift state for parallel-clone shortcut
  document.addEventListener('keydown',ev=>{if(ev.key==='Shift')_shiftDown=true;});
  document.addEventListener('keyup',  ev=>{if(ev.key==='Shift')_shiftDown=false;});

  // ── POINTER DOWN ───────────────────────────────────────────
  function onDown(e){
    const{cx,cy}=_cp(e);
    startCX=cx;startCY=cy;pDown=true;_mode='idle';
    const{x,y}=_canvasXY(canvas,cx,cy);
    const cp=_xyToChart(canvas,cx,cy); // chart-domain coords

    // ── DRAW MODE: start a new shape drag ──────────────────
    if(drawTool==='rect'||drawTool==='trend'){
      if(cp){
        _dragStart={cx,cy,cp};
        _mode='drawing';
      }
    ...
    // ── SMART / SELECT MODE ────────────────────────────────
    const hi=_findShape(x,y);

    if(hi<0){
      // No shape under pointer — deselect and pass event through
      // to LightweightCharts so the user can still pan/zoom.
      selectedIdx=-1;
      redrawCanvas();
      // DO NOT preventDefault → chart gets the event
      return;
    }

    // A shape was hit — claim the event
    e.preventDefault();
    selectedIdx=hi;
    const hitType=_hit(shapes[hi],x,y);
    // cp can be null in edge cases (chart not fully initialised).
    // Without a valid drag-start chart coord we cannot compute move deltas,
    // so fall through to selection-only and skip starting a drag.
    _dragStart={cx,cy,cp};

    if(cp&&hitType==='body'){
      _mode='moving';
      // Snapshot the shape at drag-start so we can compute deltas
      _snapShape=JSON.parse(JSON.stringify(shapes[hi]));

      // ── Shift+drag on trendline body → parallel clone ──
      // Keep the original shape frozen as a ghost (_parallelClone)
      // and move the real shape to its new position on drag.
      if(shapes[hi].type==='trend'&&_shiftDown){
        _parallelClone=JSON.parse(JSON.stringify(shapes[hi]));
      }
    }else if(cp&&hitType&&(hitType.startsWith('corner')||hitType.startsWith('mid')||hitType.startsWith('ep'))){
      _mode='resizing';
      _resizeHandle=hitType;
      // Snapshot so resize uses the original coords as anchor reference
      _snapShape=JSON.parse(JSON.stringify(shapes[hi]));
    }
    redrawCanvas();
  }
  ...
  function onMove(e){
    if(!pDown)return;
    const{cx,cy}=_cp(e);
    const d=Math.hypot(cx-startCX,cy-startCY);

    // Only prevent chart from getting pointer events when we are
    // actively drawing or manipulating a shape. In all other cases
    // (idle with no shape hit) we let the event propagate so
    // LightweightCharts can handle pan/zoom normally.
    // NOTE: preventDefault must happen BEFORE the dead-zone return so that
    // even tiny jitter moves (<3px) don't leak through to the chart when
    // a drawing or shape-drag operation is in progress.
    if(_mode==='drawing'||_mode==='moving'||_mode==='resizing'){
      e.preventDefault();
    }

    if(d<3)return; // dead zone — ignore micro jitter (after preventing scroll above)

    const cp=_xyToChart(canvas,cx,cy);

    // ── Drawing: update live preview shape ────────────────
    if(_mode==='drawing'&&_dragStart?.cp&&cp){
      const ds=_dragStart.cp;
      if(drawTool==='rect'){
        // Rectangle preview: two opposite corner coordinates
        previewShape={
          type:'rect',
          t1:ds.time,p1:ds.price,
          t2:cp.time, p2:cp.price,
          borderColor:'#4f6ef7',fillColor:'#4f6ef7',
          lineWidth:1.5,fillOpacity:12,midLine:false
        };
      }else{
        // Trendline preview: P1 → P2
        previewShape={
          type:'trend',
          t1:ds.time,p1:ds.price,
          t2:cp.time, p2:cp.price,
          color:'#ffc93c',lineWidth:1.8,
          lineStyle:'solid',extendRight:false,extendLeft:false
        };
      }
      redrawCanvas();
      return;
    ...
    // For move/resize we need cp; if _xyToChart returns null (pointer
    // briefly outside chart area) we try clamping to the canvas boundary
    // so drags that start inside still work near edges.
    if((_mode==='moving'||_mode==='resizing')&&selectedIdx>=0){
      const sh=shapes[selectedIdx];
      const ds=_dragStart?.cp;
      if(!ds)return;

      // Use cp if valid; otherwise clamp to last known valid position
      const useCp=cp||_clampedChartCoord(canvas,cx,cy);
      if(!useCp)return;

    // ── Move: translate both endpoints by the pointer delta ──
    if(_mode==='moving'&&_snapShape){
      const dt=useCp.time-ds.time;   // time delta (seconds)
      const dp=useCp.price-ds.price; // price delta
      sh.t1=_snapShape.t1+dt; sh.p1=_snapShape.p1+dp;
      sh.t2=_snapShape.t2+dt; sh.p2=_snapShape.p2+dp;
      // _parallelClone stays at the original position (the ghost)
      redrawCanvas();
      return;
    }

    // ── Resize ────────────────────────────────────────────
    if(_mode==='resizing'&&_snapShape&&_resizeHandle){
      const h=_resizeHandle;

      if(sh.type==='rect'){
        // Corner-pin resize: the dragged corner moves to the pointer;
        // the OPPOSITE corner is locked in place.
        //
        // Corner mapping (same order as _hit):
        //   corner-0 = TL → opposite = BR (corner-3)
        //   corner-1 = TR → opposite = BL (corner-2)
        //   corner-2 = BL → opposite = TR (corner-1)
        //   corner-3 = BR → opposite = TL (corner-0)
        //
        // Mid-edge handles: mid-0=top, mid-1=right, mid-2=bottom, mid-3=left
        //   mid-0 (top edge)    → lock bottom price, move top price
        //   mid-1 (right edge)  → lock left time, move right time
        //   mid-2 (bottom edge) → lock top price, move bottom price
        //   mid-3 (left edge)   → lock right time, move left time
        //
        // The shape stores (t1,p1)→(t2,p2) not necessarily TL→BR,
        // so we work in pixel space to find the locked pixel, then
        // convert back to chart domain.
        const snap=_snapShape;
        const ox1=_t2x(snap.t1)||0, oy1=_p2y(snap.p1)||0;
        const ox2=_t2x(snap.t2)||0, oy2=_p2y(snap.p2)||0;

        // Normalise snap to pixel bounding box corners
        const plx=Math.min(ox1,ox2),prx=Math.max(ox1,ox2);
        const pty=Math.min(oy1,oy2),pby=Math.max(oy1,oy2);
        const bb=canvas.getBoundingClientRect();

        if(h.startsWith('corner')){
          // Locked pixel = corner diagonally opposite the dragged handle
          // TL(0)↔BR(3)  TR(1)↔BL(2)
          let lockPx,lockPy;
          if(h==='corner-0'){lockPx=prx;lockPy=pby;} // TL dragged → lock BR
          else if(h==='corner-1'){lockPx=plx;lockPy=pby;} // TR dragged → lock BL
          else if(h==='corner-2'){lockPx=prx;lockPy=pty;} // BL dragged → lock TR
          else               {lockPx=plx;lockPy=pty;} // BR dragged → lock TL

          // We need client coords for _xyToChart; locked pixel is canvas-local
          const lockChart=_xyToChart(canvas,lockPx+bb.left,lockPy+bb.top);
          if(lockChart){
            // Locked corner becomes (t1,p1); dragged pointer becomes (t2,p2)
            sh.t1=lockChart.time; sh.p1=lockChart.price;
            sh.t2=useCp.time;     sh.p2=useCp.price;
          }
        }else if(h.startsWith('mid')){
          // Mid-edge handle: move only one edge, lock the opposite
          // For each axis we compute the locked coord from the snap bbox
          // and the free coord from the pointer.
          if(h==='mid-0'){
            // Top edge dragged → only price changes (top moves, bottom locked)
            // Lock bottom (higher y = lower price)
            const lockBot=_xyToChart(canvas,plx+bb.left,pby+bb.top);
            if(lockBot){
              // Reconstruct: left-time = snap.t1 or t2 (whichever is smaller)
              const lt=Math.min(snap.t1,snap.t2),rt=Math.max(snap.t1,snap.t2);
              sh.t1=lt;sh.t2=rt;
              sh.p1=useCp.price;  // top (new)
              sh.p2=lockBot.price; // bottom (locked)
            }
          }else if(h==='mid-2'){
            // Bottom edge dragged → only price changes (bottom moves, top locked)
            const lockTop=_xyToChart(canvas,plx+bb.left,pty+bb.top);
            if(lockTop){
              const lt=Math.min(snap.t1,snap.t2),rt=Math.max(snap.t1,snap.t2);
              sh.t1=lt;sh.t2=rt;
              sh.p1=lockTop.price; // top (locked)
              sh.p2=useCp.price;   // bottom (new)
            }
          }else if(h==='mid-1'){
            // Right edge dragged → only time changes (right moves, left locked)
            const lockLeft=_xyToChart(canvas,plx+bb.left,pty+bb.top);
            if(lockLeft){
              const bp=Math.max(snap.p1,snap.p2),tp2=Math.min(snap.p1,snap.p2);
              sh.t1=lockLeft.time; sh.t2=useCp.time;
              sh.p1=bp;sh.p2=tp2;
            }
          }else if(h==='mid-3'){
            // Left edge dragged → only time changes (left moves, right locked)
            const lockRight=_xyToChart(canvas,prx+bb.left,pty+bb.top);
            if(lockRight){
              const bp=Math.max(snap.p1,snap.p2),tp2=Math.min(snap.p1,snap.p2);
              sh.t1=useCp.time;   sh.t2=lockRight.time;
              sh.p1=bp;sh.p2=tp2;
            }
          }
        }
        ...
        // Move exactly one endpoint; the other stays frozen
        if(h==='ep-0'){sh.t1=useCp.time;sh.p1=useCp.price;}
        else          {sh.t2=useCp.time;sh.p2=useCp.price;}
      }
      redrawCanvas();
    } // end if moving||resizing
  }
  ...
  function onUp(e){
    ...
    pDown=false;
    const{cx,cy}=_cp(e);
    const moved=Math.hypot(cx-startCX,cy-startCY)>6;

    // ── Commit new drawing ──────────────────────────────
    if(_mode==='drawing'&&previewShape&&moved){
      shapes.push({...previewShape});
      selectedIdx=shapes.length-1;
      previewShape=null;_dragStart=null;_mode='idle';
      // Keep the active tool so the user can draw multiple shapes
      // back-to-back without re-selecting from the menu.
      redrawCanvas();
      return;
    }

    // ── Commit parallel clone (Shift+drag trendline) ────
    // The moved shape is already stored at its new position;
    // push the original snapshot as a second shape.
    if(_parallelClone&&moved){
      shapes.push({..._parallelClone});
      _parallelClone=null;
    }

    previewShape=null;_dragStart=null;

    if(_mode==='moving'||_mode==='resizing'){
      // Always clear parallel clone on drag end — if user released without
      // enough movement the clone was never committed, so discard it.
      _parallelClone=null;
      _mode='idle';_snapShape=null;_resizeHandle=null;
      redrawCanvas();
      return;
    }
    _mode='idle';
    ...
    if(!moved){
      ...
      if(hi>=0){
        // ── Shape tapped ────────────────────────────────
        if(hi===selectedIdx){
          // Already selected: open settings immediately on any tap
          // (single-tap on selected = open settings, TradingView style)
          lastTapMs=0;lastTapIdx=-1;
          _openSettings(hi);
        }else{
          // Different shape: just select it; a second tap opens settings
          selectedIdx=hi;
          lastTapMs=now;lastTapIdx=hi;
          redrawCanvas();
        }
      }else{
  ...
  document.addEventListener('keydown',function(e){
    ...
    if(e.key==='Delete'||e.key==='Backspace'){
      if(selectedIdx>=0){shapes.splice(selectedIdx,1);selectedIdx=-1;redrawCanvas();}
    }
    if(e.key==='Escape'){
      previewShape=null;drawTool=null;selectedIdx=-1;_parallelClone=null;
      _setToolUI(null);redrawCanvas();
    }
  });
}
...
function init(){
  ...
  fetchPrices();
  fetchSignals();
  // Prices update every 1 second — server cache refreshes at same rate
  priceTimer=setInterval(fetchPrices,1000);
  // Signal refresh every 15s
...
// e.g. /market#AVAX_USDT
const hash=window.location.hash.replace("#","").trim().toUpperCase();
if(hash&&PAIRS.includes(hash)){
  setTimeout(()=>goToChart(hash),120);
}
