...
def detect_crt(candles, direction, ob=None):
    direction = _d(direction)
    found = []
    # ACCURACY FIX: candles[-1] is the CURRENTLY FORMING candle — its OHLC values
    # change every tick and will produce false CRT readings. Only use closed candles.
    closed = candles[:-1] if len(candles) > 1 else candles
    if len(closed)<5: return found
    limit = min(20, len(closed)-2)
    for offset in range(1, limit):
        i3=len(closed)-1-offset; i2=i3-1; i1=i2-1
        if i1<0: break
        c1=closed[i1]; c2=closed[i2]; c3=closed[i3]
...
def check_tbs(symbol, direction, crl, crh, crt_tf="Hour4"):
    ...
    ACCURACY FIX: We only look at the last MAX_TBS_LOOKBACK *closed* candles
    (excluding the currently-forming live candle at index -1 which is not yet
    closed). A TBS older than this is stale and no longer actionable — the
    price has already moved away and the entry/SL levels are wrong.

    TBS_STALENESS_LIMIT maps CRT timeframe → max LTF candles back to accept.
    For a 4H CRT using 5m LTF: 4H = 48 five-min candles; we allow up to 30
    candles back so the TBS formed within the last ~2.5H = still fresh.
    """
    direction = _d(direction)
    ...
    # How many closed LTF candles back to accept as "still fresh/actionable"
    # Keyed by CRT TF. Tighter window = higher accuracy (avoids stale signals).
    TBS_STALENESS_LIMIT = {
        "Day1":  40,   # 1D CRT: TBS on 30/45m LTF — up to 40 candles (~20H on 30m)
        "Hour4": 30,   # 4H CRT: TBS on 4/5m LTF   — up to 30 candles (~2.5H on 5m)
        "Hour3": 25,   # 3H CRT: TBS on 3/4m LTF   — up to 25 candles (~1.5H on 3m)
        "Hour2": 20,   # 2H CRT: TBS on 2/3m LTF   — up to 20 candles (~1H on 3m)
        "Min60": 15,   # 1H CRT: TBS on 1/2m LTF   — up to 15 candles (~30m on 2m)
    }
    max_lookback = TBS_STALENESS_LIMIT.get(crt_tf, 20)

    for tf in tfs_to_check:
        candles = get_candles(symbol, tf, limit=max_lookback + 10)
        if not candles or len(candles) < 4:
            continue

        # CRITICAL: candles[-1] is the CURRENTLY FORMING (not yet closed) candle.
        # Its open/close/high/low will change every tick — NEVER use it for TBS detection.
        # We only scan confirmed CLOSED candles: candles[:-1]
        closed_candles = candles[:-1]

        # Restrict to the freshness window
        recent = closed_candles[-max_lookback:]

        # Walk newest → oldest (most recent closed candle first)
        for i in range(len(recent) - 1, -1, -1):
            ...
            # Body must be meaningful — at least 25% of the candle's total range
            # This strictly enforces "body soup not wick soup"
            if body_size < candle_rng * 0.25:
                continue
...
def mexc_request(method, path, params=None, signed=True, _retry=0):
    ...
    try:
        ...
        # CRITICAL FIX: Never follow redirects on ANY request.
        # MEXC redirects corrupt the URL (adds stray &, switches to http://)
        # which causes 403 Access Denied on the redirected request.
        # If we get a redirect, it means the URL is wrong — log and fail.
        if r.status_code in (301, 302, 307, 308):
            loc = r.headers.get("Location", "–")
            log(f"🚫 MEXC unexpected redirect {r.status_code} → {loc}")
            log(f"   This should never happen — base URL is always HTTPS.")
            log(f"   Original URL: {url}")
            return None, f"Unexpected redirect {r.status_code} from MEXC — check API endpoint"
...
def place_order(sig):
    ...
    order_data, err = mexc_request("POST", "/order/create", order_params)
    if err:
...
def close_trade(symbol, reason="Manual"):
    ...
    _, err = mexc_request("POST", "/order/create", params)
...
<div class="tab-wrap">
  ...
  <div id="tab-monitor" style="display:none">
    ...
    <!-- ═══════════════════════════════════════════════════════════
         LIVE CHARTS GRID — 30 watchlist pairs
    ════════════════════════════════════════════════════════════════ -->
    <div class="panel" id="charts-panel" style="margin-bottom:14px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;flex-wrap:wrap;gap:8px">
        <div class="panel-ttl" style="margin-bottom:0">📊 Live Charts <span style="font-family:'JetBrains Mono',monospace;font-size:.7rem;color:var(--dim)">(30 pairs · updates every 2s)</span></div>
        <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
          <!-- Timeframe selector -->
          <div style="display:flex;gap:4px">
            <button class="chart-tf-btn active" data-tf="Min1"  onclick="setChartTF('Min1',this)">1m</button>
            <button class="chart-tf-btn"        data-tf="Min5"  onclick="setChartTF('Min5',this)">5m</button>
            <button class="chart-tf-btn"        data-tf="Min15" onclick="setChartTF('Min15',this)">15m</button>
            <button class="chart-tf-btn"        data-tf="Hour4" onclick="setChartTF('Hour4',this)">4H</button>
          </div>
          <!-- Filter -->
          <select id="chart-filter" onchange="applyChartFilter()" style="background:var(--card);border:1px solid var(--border);color:var(--text);padding:5px 10px;border-radius:8px;font-family:'JetBrains Mono',monospace;font-size:.68rem;cursor:pointer">
      ...
      <div id="charts-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:8px"></div>
    </div>

    <!-- Chart detail modal -->
    <div id="chart-modal" style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.85);backdrop-filter:blur(6px);padding:12px;box-sizing:border-box;overflow-y:auto">
      <div style="max-width:900px;margin:0 auto;background:var(--card);border:1px solid var(--border);border-radius:16px;overflow:hidden">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 18px;border-bottom:1px solid var(--border)">
          <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
            <div id="modal-title" style="font-family:'JetBrains Mono',monospace;font-weight:700;font-size:1rem;color:var(--text)">—</div>
            <div id="modal-price" style="font-family:'JetBrains Mono',monospace;font-size:.9rem;color:var(--green)">—</div>
            <div id="modal-change" style="font-family:'JetBrains Mono',monospace;font-size:.78rem">—</div>
            <div id="modal-signal-badge" style="display:none"></div>
          </div>
          <div style="display:flex;gap:8px;align-items:center">
            <div style="display:flex;gap:4px">
              <button class="modal-tf-btn active" data-tf="Min1"  onclick="setModalTF('Min1',this)">1m</button>
              <button class="modal-tf-btn"        data-tf="Min5"  onclick="setModalTF('Min5',this)">5m</button>
              <button class="modal-tf-btn"        data-tf="Min15" onclick="setModalTF('Min15',this)">15m</button>
              <button class="modal-tf-btn"        data-tf="Min30" onclick="setModalTF('Min30',this)">30m</button>
              <button class="modal-tf-btn"        data-tf="Hour4" onclick="setModalTF('Hour4',this)">4H</button>
            </div>
            <button onclick="closeChartModal()" style="background:rgba(239,68,68,.15);border:1px solid rgba(239,68,68,.4);color:var(--red);border-radius:8px;padding:6px 14px;font-family:'JetBrains Mono',monospace;font-size:.75rem;cursor:pointer;font-weight:700">✕ Close</button>
          </div>
        </div>
        <div id="modal-chart-container" style="height:420px;padding:0"></div>
        <div id="modal-info" style="padding:12px 18px;display:grid;grid-template-columns:repeat(4,1fr);gap:8px;border-top:1px solid var(--border)"></div>
      </div>
    </div>

    <style>
      .chart-tf-btn,.modal-tf-btn{
        ...
        font-size:.65rem;cursor:pointer;font-weight:600;transition:all .15s;
      }
      .chart-tf-btn.active,.modal-tf-btn.active{
        background:rgba(16,185,129,.15);border-color:rgba(16,185,129,.5);color:var(--green);
      }
      .chart-card{
      ...
      .chart-card:hover{border-color:rgba(16,185,129,.5);transform:translateY(-1px);}
      .chart-card.has-signal{border-color:rgba(16,185,129,.35);}
      .chart-card.has-signal.sell{border-color:rgba(239,68,68,.35);}
      .chart-card.monitored{border-color:rgba(251,191,36,.4);}
      .chart-card-header{padding:6px 8px;display:flex;justify-content:space-between;align-items:center}
      .chart-sym{font-family:'JetBrains Mono',monospace;font-size:.68rem;font-weight:700;color:var(--text);line-height:1}
      .chart-price{font-family:'JetBrains Mono',monospace;font-size:.62rem;color:var(--dim);text-align:right}
      .chart-change{font-size:.58rem;font-family:'JetBrains Mono',monospace;font-weight:600}
      .chart-sig-tag{
        position:absolute;top:6px;right:8px;font-size:.55rem;font-family:'JetBrains Mono',monospace;
        padding:2px 5px;border-radius:4px;font-weight:700;line-height:1.2;
      }
      .chart-sig-tag.buy{background:rgba(16,185,129,.2);color:var(--green);border:1px solid rgba(16,185,129,.4)}
      .chart-sig-tag.sell{background:rgba(239,68,68,.2);color:var(--red);border:1px solid rgba(239,68,68,.4)}
      .chart-sig-tag.mon{background:rgba(251,191,36,.2);color:var(--yellow);border:1px solid rgba(251,191,36,.4)}
...
// ══════════════════════════════════════════════════════════════════════
// LIVE CHARTS ENGINE — 30 watchlist pairs with lightweight-charts v4
// ══════════════════════════════════════════════════════════════════════
(function(){
  // ── State ──────────────────────────────────────────────────────────
  const CHART_TF_DEFAULT = "Min1";
  let chartTF       = CHART_TF_DEFAULT;
  let chartFilter   = "all";
  let watchlistData = [];           // latest /api/watchlist response
  let miniCharts    = {};           // sym → {chart, series, container}
  let modalChart    = null;         // the expanded modal chart instance
  let modalSeries   = null;
  let modalSym      = null;
  let modalTF       = "Min5";
  let chartsInited  = false;
  let chartPollId   = null;
  let modalPollId   = null;
  ...
  const MODAL_OPTS = {
    layout:     { background:{color:"#0c0b18"}, textColor:"#e2e8f0" },
    grid:       { vertLines:{color:"rgba(255,255,255,.06)"}, horzLines:{color:"rgba(255,255,255,.06)"} },
    crosshair:  { mode:1 },
    rightPriceScale:{ visible:true, borderColor:"rgba(124,58,237,.3)", scaleMargins:{top:.1,bottom:.1} },
    timeScale:  { visible:true, borderColor:"rgba(124,58,237,.3)", timeVisible:true, secondsVisible:false },
    handleScroll:true, handleScale:true,
  };

  function upColor(o,c){ return c>=o ? "#10b981" : "#ef4444"; }
  function candleSeries(chart){
    return chart.addCandlestickSeries({
      upColor:"#10b981", downColor:"#ef4444",
      borderUpColor:"#10b981", borderDownColor:"#ef4444",
      wickUpColor:"rgba(16,185,129,.7)", wickDownColor:"rgba(239,68,68,.7)",
    });
  }

  // ── Fetch candles ──────────────────────────────────────────────────
  async function fetchCandles(sym, tf, limit){
    try{
      const r = await fetch(`/api/candles?symbol=${sym}&interval=${tf}&limit=${limit||80}`);
      return await r.json();
    }catch{ return []; }
  }
  ...
  // ── Build / update the watchlist grid ─────────────────────────────
  function filteredPairs(){
    if(chartFilter==="all")       return watchlistData;
    if(chartFilter==="signal")    return watchlistData.filter(p=>p.signal);
    if(chartFilter==="monitored") return watchlistData.filter(p=>p.monitored);
    if(chartFilter==="buy")       return watchlistData.filter(p=>p.signal&&p.signal.direction==="BUY");
    if(chartFilter==="sell")      return watchlistData.filter(p=>p.signal&&p.signal.direction==="SELL");
  ...
  function buildGrid(){
    ...
    // Remove cards for pairs that are no longer visible
    const activeSym = new Set(pairs.map(p=>p.symbol));
    Object.keys(miniCharts).forEach(sym=>{
      if(!activeSym.has(sym)){
        if(miniCharts[sym].chart) miniCharts[sym].chart.remove();
        delete miniCharts[sym];
      }
    });
    ...
    pairs.forEach(pair=>{
      ...
      let card  = document.getElementById("cc-"+sym);
      ...
      if(!card){
        ...
        card.innerHTML = `
          <div class="chart-card-header">
            <div>
              <div class="chart-sym">${sym.replace("_USDT","")}</div>
              <div class="chart-change" id="chg-${sym}">—</div>
            </div>
            <div class="chart-price" id="cpx-${sym}">—</div>
          </div>
          <div class="chart-container" id="cct-${sym}"></div>`;
        card.addEventListener("click",()=>openModal(sym));
        grid.appendChild(card);
      }

      // Update price / change
      const priceEl  = document.getElementById("cpx-"+sym);
      const changeEl = document.getElementById("chg-"+sym);
      if(priceEl && pair.price!==null){
        priceEl.textContent = fmtP(pair.price);
      }
      if(changeEl && pair.change!==null){
        const chg = parseFloat(pair.change||0);
        changeEl.textContent  = (chg>=0?"+":"")+chg.toFixed(2)+"%";
        changeEl.style.color  = chg>=0?"#10b981":"#ef4444";
      }

      // Signal / monitor badge
      card.className = "chart-card";
      let badge = card.querySelector(".chart-sig-tag");
      if(!badge){ badge=document.createElement("div"); badge.className="chart-sig-tag"; card.appendChild(badge); }
      if(pair.monitored){
        card.classList.add("monitored");
        badge.className="chart-sig-tag mon"; badge.textContent="👁 MON"; badge.style.display="";
      } else if(pair.signal){
        const isBuy = pair.signal.direction==="BUY";
        card.classList.add(isBuy?"has-signal":"has-signal sell");
        badge.className="chart-sig-tag "+(isBuy?"buy":"sell");
        badge.textContent=(isBuy?"▲":"▼")+" "+pair.signal.grade;
        badge.style.display="";
      } else {
        badge.style.display="none";
      }

      // Init chart only once per card
      const container = document.getElementById("cct-"+sym);
      if(container && !miniCharts[sym] && typeof LightweightCharts!=="undefined"){
        const chart  = LightweightCharts.createChart(container, {
          ...CHART_OPTS, width:container.offsetWidth||160, height:90,
        });
        const series = candleSeries(chart);
        miniCharts[sym] = { chart, series };
        // Load initial candles async
        fetchCandles(sym, chartTF, 60).then(data=>{
    ...
    // Reorder DOM to match filtered order
    pairs.forEach(p=>{ const el=document.getElementById("cc-"+p.symbol); if(el)grid.appendChild(el); });
  }

  // ── Refresh all mini-chart candles (last candle append) ───────────
  async function refreshMiniCharts(){
    const pairs = filteredPairs();
    // Batch: fetch in groups of 5 to avoid hammering the server
    for(let i=0;i<pairs.length;i+=5){
      const batch = pairs.slice(i,i+5);
      await Promise.all(batch.map(async p=>{
        const sym = p.symbol;
        if(!miniCharts[sym]) return;
        const data = await fetchCandles(sym, chartTF, 60);
        if(data && data.length){
          try{
            miniCharts[sym].series.setData(data);
            miniCharts[sym].chart.timeScale().fitContent();
          }catch(e){}
        }
      }));
    }
  ...
  // ── Public: fetch watchlist prices & update cards ─────────────────
  window.fetchWatchlist = async function(){
  ...
  window.setChartTF = function(tf, btn){
    ...
    // Reload all mini charts with new TF
    Object.keys(miniCharts).forEach(sym=>{
      fetchCandles(sym, chartTF, 60).then(data=>{
        if(data && data.length){
          try{
  ...
  window.applyChartFilter = function(){
    ...
    chartFilter = sel ? sel.value : "all";
    buildGrid();
  };

  // ── Modal ─────────────────────────────────────────────────────────
  window.openModal = async function(sym){
    modalSym = sym;
    ...
    // Set title
    document.getElementById("modal-title").textContent = sym.replace("_USDT","/USDT");

    // Update price from watchlist
    const p = watchlistData.find(x=>x.symbol===sym);
    if(p){
      document.getElementById("modal-price").textContent  = fmtP(p.price);
      const chg = parseFloat(p.change||0);
      const cel = document.getElementById("modal-change");
      cel.textContent  = (chg>=0?"+":"")+chg.toFixed(2)+"%";
      cel.style.color  = chg>=0?"#10b981":"#ef4444";

      // Signal badge
      const sbadge = document.getElementById("modal-signal-badge");
      if(p.signal){
        const isBuy = p.signal.direction==="BUY";
        sbadge.style.display="inline-block";
        sbadge.style.cssText=`display:inline-block;padding:3px 10px;border-radius:6px;
          font-family:'JetBrains Mono',monospace;font-size:.7rem;font-weight:700;
          background:${isBuy?"rgba(16,185,129,.15)":"rgba(239,68,68,.15)"};
          border:1px solid ${isBuy?"rgba(16,185,129,.4)":"rgba(239,68,68,.4)"};
          color:${isBuy?"#10b981":"#ef4444"}`;
        sbadge.textContent = (isBuy?"▲ BUY":"▼ SELL")+" · "+p.signal.grade+" · "+p.signal.rr+"R";
      } else {
        document.getElementById("modal-signal-badge").style.display="none";
      }
      ...
      if(info){
        info.innerHTML = [
          ["24H High", fmtP(p.high)],
          ["24H Low",  fmtP(p.low)],
          ["Change",   (parseFloat(p.change||0)>=0?"+":"")+parseFloat(p.change||0).toFixed(2)+"%"],
          ["Monitor",  p.monitored||"–"],
        ].map(([k,v])=>`<div style="background:rgba(255,255,255,.03);border:1px solid var(--border);border-radius:8px;padding:8px 10px">
          <div style="font-family:'JetBrains Mono',monospace;font-size:.58rem;color:var(--dim);margin-bottom:3px">${k}</div>
          <div style="font-family:'JetBrains Mono',monospace;font-size:.8rem;font-weight:700;color:var(--text)">${v}</div>
        </div>`).join("");
      }
    }

    // Build or rebuild modal chart
    ...
    modalChart  = LightweightCharts.createChart(container, {
      ...MODAL_OPTS, width:container.offsetWidth||800, height:420,
    });
    modalSeries = candleSeries(modalChart);

    // Load data
    const data = await fetchCandles(sym, modalTF, 200);
    if(data && data.length){
      modalSeries.setData(data);
      modalChart.timeScale().fitContent();
    }

    // Start live refresh for modal
    if(modalPollId) clearInterval(modalPollId);
    modalPollId = setInterval(async()=>{
      if(!modalSym || document.getElementById("chart-modal").style.display==="none"){
        clearInterval(modalPollId); return;
      }
      const wd = watchlistData.find(x=>x.symbol===modalSym);
      if(wd){
        document.getElementById("modal-price").textContent = fmtP(wd.price);
        const chg = parseFloat(wd.change||0);
        const cel = document.getElementById("modal-change");
        cel.textContent = (chg>=0?"+":"")+chg.toFixed(2)+"%";
        cel.style.color = chg>=0?"#10b981":"#ef4444";
      }
      // Update last candle
      const latest = await fetchCandles(modalSym, modalTF, 200);
      if(latest && latest.length){
        try{ modalSeries.setData(latest); }catch(e){}
      }
    }, 2000);
  };
  ...
  window.setModalTF = function(tf, btn){
    modalTF = tf;
    document.querySelectorAll(".modal-tf-btn").forEach(b=>b.classList.remove("active"));
    if(btn) btn.classList.add("active");
    if(modalSym) openModal(modalSym);
  };
  ...
  if(typeof ResizeObserver!=="undefined"){
    const ro = new ResizeObserver(entries=>{
      entries.forEach(entry=>{
        ...
        if(sym && miniCharts[sym]){
          const {width,height} = entry.contentRect;
          try{ miniCharts[sym].chart.resize(width, height); }catch(e){}
        }
      });
    });
    // Observe containers after they are added
    const origBuildGrid = buildGrid;
    // Inline patch: after buildGrid we observe new containers
    const patchedObserve = ()=>{
      document.querySelectorAll(".chart-container").forEach(el=>{
        if(!el.dataset.observed){
          el.dataset.sym      = el.id.replace("cct-","");
          el.dataset.observed = "1";
          ro.observe(el);
        }
      });
    };
    // Hook into fetchWatchlist
    const origFetch = window.fetchWatchlist;
    window.fetchWatchlist = async function(){
      await origFetch();
      patchedObserve();
    };
  ...
  // ── Boot: when monitor tab opens, build immediately ───────────────
  const origSW = window.sw;
  window.sw = function(tab, btn){
    origSW(tab, btn);
    if(tab==="monitor"){
      window.fetchWatchlist();
      // Stagger chart candle loads after grid is built
      setTimeout(()=> refreshMiniCharts(), 500);
      // Start background candle refresh every 3s
      if(chartPollId) clearInterval(chartPollId);
      chartPollId = setInterval(()=>{
        if(activeTab==="monitor"){
          window.fetchWatchlist();
          refreshMiniCharts();
        } else {
          clearInterval(chartPollId);
          chartPollId=null;
        }
      }, 3000);
    }
...
window.sw=function(tab,btn){activeTab=tab;document.querySelectorAll(".tab").forEach(b=>b.classList.remove("active"));btn.classList.add("active");["signals","trades","monitor","history","trade-cfg","paper","log","settings"].forEach(t=>{const el=$("tab-"+t);if(el)el.style.display=t===tab?"block":"none";});if(tab==="log"){fetchLog();fetchDiag();}if(tab==="trades")window.fetchPnl();if(tab==="monitor")fetchMonitor();if(tab==="history")fetchHistory();if(tab==="trade-cfg"){loadTradeConfig();syncModelToggles();window.fetchPnl();}if(tab==="paper"){loadPaperConfig();fetchPaperData();}if(tab==="settings")loadSettings();};
...
def api_candles():
    """
    Returns OHLC candle data for a symbol + interval for lightweight-charts.
    Query params: symbol=PEPE_USDT&interval=Min5&limit=80
    Returns lightweight-charts format: [{time, open, high, low, close}]
    time is UNIX seconds (lightweight-charts expects seconds not ms).
    """
    ...
    # Convert ms timestamps → seconds for lightweight-charts
    out = []
    for c in candles:
        t = c["time"]
...
def api_watchlist():
    """Returns the 30 watchlist pairs with live price + 24h change + signal status."""
    sigs_by_sym = {}
    for s in list(signals)[:200]:
        sym = s.get("symbol","")
        if sym and sym not in sigs_by_sym:
            sigs_by_sym[sym] = s

    out = []
    for sym in TOP_PAIRS:
        ticker = get_ticker(sym)
        sig    = sigs_by_sym.get(sym)
        # Check if in manip monitor
        with manip_lock:
            mon = manip_monitor.get(sym)
        out.append({
            "symbol":    sym,
            "price":     ticker["price"]  if ticker else None,
            "change":    ticker["change"] if ticker else None,
            "high":      ticker["high"]   if ticker else None,
            "low":       ticker["low"]    if ticker else None,
            "signal":    {"direction": sig["direction"], "grade": sig["grade"],
                          "rr": sig["rr"], "score": sig["score"]} if sig else None,
            "monitored": mon["phase"] if mon else None,
        })
    return jsonify(out)
...
@app.route("/api/test-mexc")
def api_test_mexc():
    """
    Deep MEXC API diagnostics endpoint.
    Tests: outbound IP, key validity, futures permission, balance fetch, order permission.
    Call /api/test-mexc from the browser to debug 403 errors.
    """
    import json as _json
    ...
    results["ip_warning"]  = (
        f"⚠️ WHITELIST THIS IP on MEXC: {outbound_ip}" if outbound_ip != "unknown"
        else "Could not determine outbound IP"
    )

    # 2. Public ping
    try:
        rp = requests.get("https://contract.mexc.com/api/v1/contract/ping", timeout=8)
        results["mexc_public_ping"] = f"HTTP {rp.status_code} ({'OK' if rp.status_code==200 else 'FAIL'})"
    except Exception as e:
        results["mexc_public_ping"] = f"ERROR: {e}"

    if not api_key or not api_secret or "PASTE" in api_key:
        results["keys_status"] = "❌ API keys not configured — set them in Trade Config"
        return jsonify(results)

    results["key_last4"] = "…" + api_key[-4:]

    # 3. Server timestamp test
    ts = _get_mexc_ts()
    results["server_ts"] = ts
    local_ts = str(int(time.time() * 1000))
    drift_ms = abs(int(ts) - int(local_ts))
    results["clock_drift_ms"] = drift_ms
    results["clock_ok"] = "✅ OK" if drift_ms < 2000 else f"⚠️ HIGH DRIFT {drift_ms}ms — may cause 403"

    # 4. Test authenticated call: GET /account/assets
    data, err = mexc_request("GET", "/account/assets")
    if err:
        results["auth_test"] = f"❌ FAILED: {err}"
        results["auth_detail"] = (
            "Common causes of 403:\n"
            f"  1. IP {outbound_ip} not whitelisted on this MEXC API key\n"
            "  2. Futures trading permission NOT enabled on the key\n"
            "  3. API key/secret copy-paste error (extra space, missing char)\n"
            "  4. Key may be for SPOT only — create a new key with FUTURES permission"
        )
    else:
        assets = data if isinstance(data, list) else ([data] if data else [])
        usdt_bal = 0.0
        for a in assets:
            if (a.get("currency") or a.get("coin") or "").upper() == "USDT":
                usdt_bal = float(a.get("availableBalance") or a.get("equity") or 0)
                break
        results["auth_test"]    = f"✅ SUCCESS — USDT balance: ${usdt_bal:.4f}"
        results["usdt_balance"] = usdt_bal

    # 5. Test order permission by calling /order/list (read-only, no actual order)
    order_data, order_err = mexc_request("GET", "/order/list", {"symbol": "BTC_USDT", "states": "2", "limit": "1"})
    if order_err:
        results["order_permission"] = f"❌ {order_err}"
        results["order_hint"] = "Enable 'Contract Trading' permission on your MEXC API key"
    else:
        results["order_permission"] = "✅ Futures order access confirmed"
