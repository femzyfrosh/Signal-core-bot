...
def _parse_mexc_ticker(d):
    ...
    """
    price = float(d.get("lastPrice", d.get("last", 0)) or 0)
    high  = float(d.get("high24h",   d.get("high", 0)) or 0)
    low   = float(d.get("low24h",    d.get("low",  0)) or 0)
...
.wl-title{font-family:var(--display);font-size:.9rem;font-weight:800;letter-spacing:.1em;background:linear-gradient(135deg,var(--accent),var(--accent2),var(--green));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;text-transform:uppercase;margin-bottom:10px}
...
function renderWatchlist(){
  ...
  list.innerHTML=filtered.map(sym=>{
    ...
    return`<div class="wl-row${sigClasses}" onclick="openPair('${sym}')">
      <div class="sym-icon" style="background:${bg};border-color:${clr}60;color:${clr}">
        <span>${pairEmoji(sym)}</span>
        ${sig?`<span class="signal-badge" style="background:${badgeColor}"></span>`:""}
      </div>
      <div class="wl-info">
        <div class="wl-sym">${base}<span>/USDT</span></div>
