...
function initDrawCanvas(){
  ...
  function onDown(e){
    ...
    }else if(hitType&&(hitType.startsWith('corner')||hitType.startsWith('ep'))){
      _mode='resizing';
  ...
  function onMove(e){
    ...
    if(d<3)return; // dead zone — ignore micro jitter
    e.preventDefault();
    ...
    if(_mode==='moving'&&_snapShape){
      const dt=cp.time-ds.time;   // time delta (seconds)
      const dp=cp.price-ds.price; // price delta
    ...
    if(_mode==='resizing'&&_snapShape&&_resizeHandle){
      ...
      if(sh.type==='rect'){
        ...
        // Convert locked pixel back to chart domain
        const bb=canvas.getBoundingClientRect();
        ...
        if(lockChart){
          ...
          sh.t2=cp.time;        sh.p2=cp.price;
        }

      }else if(sh.type==='trend'){
        // Move exactly one endpoint; the other stays frozen
        if(h==='ep-0'){sh.t1=cp.time;sh.p1=cp.price;}
        else          {sh.t2=cp.time;sh.p2=cp.price;}
      }
      redrawCanvas();
    }
  }
  ...
  function onUp(e){
    ...
    if(!moved){
      ...
      const hi=_findShape(x,y);
      const now=Date.now();
      if(hi>=0&&hi===lastTapIdx&&now-lastTapMs<400){
        // Double-tap → open settings popup
        lastTapMs=0;lastTapIdx=-1;
        _openSettings(hi);
      }else{
        selectedIdx=hi>=0?hi:-1;
