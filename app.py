...
# HTF: used for bias & CRT formation (per spec: 1D,4H,3H,2H,1H,45m,30m,15m)
# CRT 3-candle formations ONLY on: 1D, 4H, 3H, 2H, 1H
CRT_TFS   = ["Day1","Hour4","Hour3","Hour2","Min60"]
# OB / key-level scan timeframes (HTF)
OB_TFS    = ["Day1","Hour4","Hour3","Hour2","Min60","Min45"]
# LTF: 15,5,4,3,2,1 min (per spec)
TBS_TFS   = ["Min15","Min5","Min4","Min3","Min2","Min1"]
# LTF to check for TBS keyed by the CRT timeframe (per spec)
TBS_TF_MAP = {
...
def scan_pair(symbol):
    ...
    for zone in active_zones:
        ...
        # A CRT is valid if price RECENTLY REACTED from the HTF key level
        # in alignment with the trend — CRT does not need to overlap the zone.
        # Two acceptance paths:
        #   1. CRT range is still near/inside the zone (price still reacting)
        #   2. CRT formed AFTER the zone tap — price reacted and moved away,
        #      then formed a CRT in the trend direction (post-reaction continuation)
        #
        # For SELL: zone is in premium — price tapped supply and fell. Any CRT
        #           with CRH at or below zone_top is valid (price below supply).
        # For BUY:  zone is in discount — price tapped demand and rose. Any CRT
        #           with CRL at or above zone_bot is valid (price above demand).
        #
        # We also accept CRTs that OVERLAP the zone (price still tapping it).
        reacted = price_reacted_from_zone(crt_candles, zone_top, zone_bot, direction, lookback=30)
        ...
        for crt in all_crts:
            ...
            # Path 1: CRT range overlaps or is at the zone (still tapping)
            overlaps_zone = (
                (direction == "SELL" and crl_c <= zone_top and crh_c >= zone_bot) or
                (direction == "BUY"  and crh_c >= zone_bot and crl_c <= zone_top)
            )

            # Path 2: Price reacted from zone and CRT is in the correct direction
            # (price moved away from zone in trend direction — post-reaction CRT)
            post_reaction = reacted and (
                (direction == "SELL" and crh_c <= zone_top * 1.02) or
                (direction == "BUY"  and crl_c >= zone_bot * 0.98)
            )

            if overlaps_zone or post_reaction:
        ...
        if not valid_crts:
            ...
            if not already_monitored and not monitor_full:
                ...
                manip_in_zone = [
                    ...
                    if (
                        (direction == "SELL" and (
                            (m["crl"] <= zone_top and m["crh"] >= zone_bot) or
                            (reacted and m["crh"] <= zone_top * 1.02)
                        )) or
                        (direction == "BUY"  and (
                            (m["crh"] >= zone_bot and m["crl"] <= zone_top) or
                            (reacted and m["crl"] >= zone_bot * 0.98)
                        ))
                    )
                ]
                if manip_in_zone:
        ...
        for crt in valid_crts:
            ...
            # ── IMMEDIATE SIGNAL on TBS close — no AWAIT_PRICE wait ────
            # Per spec: "send the signal immediately the TBS close is detected"
            ...
            sig["entry_type"]   = "Model #1 (TBS close — enter now @ open {})".format(tbs_entry)
            sig["market_order"] = True
            diag["passed"] += 1
            results.append(sig)

            # Also register in manip_monitor so live trade tracking works
            with manip_lock:
                already = symbol in manip_monitor
                full    = len(manip_monitor) >= MAX_MONITORED
            if not already and not full:
                has_pd = "DISCOUNT" in zone_name or "PREMIUM" in zone_name
                score_q = 90 if has_pd and rr_chk >= 3.0 else 80 if rr_chk >= 3.0 else 70
                grade_q = "A+" if score_q >= 85 else "A"
                with manip_lock:
                    manip_monitor[symbol] = {
                        "phase":      "SIGNAL_FIRED",
                        "direction":  direction,
...
def detect_manip_phase(candles, direction, crt_tf="Hour4"):
    ...
    # Valid window: 1 second → 59 minutes before candle close (per spec)
...
def detect_manip_phase_live(candles, direction, tf_name, min_mins=0.016, max_mins=59):
    ...
    with 1 second → 59 minutes remaining before it closes (per spec).
    ...
    if mins_left < min_mins or mins_left > max_mins:
        return pending  # Outside the valid window
...
def manip_monitor_loop():
    ...
    AWAIT_TBS  — CRT confirmed (C2 closed back inside range).
                 ...
                 When TBS closes → signal fires IMMEDIATELY (entry=TBS open,
                 SL=TBS close, TP=opposite CRT level) → RUNNING.
        ...
        try:
            ...
            for symbol in symbols:
                ...
                if phase == "WATCHING":
                    ...
                    all_crts  = detect_crt(crt_candles, direction, ob=None)
                    # CRT is valid if:
                    # 1. Its range overlaps the HTF zone (still tapping), OR
                    # 2. Price recently reacted from the zone in trend direction
                    #    and the CRT is on the correct side (post-reaction continuation)
                    reacted = price_reacted_from_zone(crt_candles, zone_top, zone_bot, direction, lookback=30)
                    zone_crts = []
                    for c in all_crts:
                        overlaps = (
                            (direction == "SELL" and c["crl"] <= zone_top and c["crh"] >= zone_bot) or
                            (direction == "BUY"  and c["crh"] >= zone_bot and c["crl"] <= zone_top)
                        )
                        post_react = reacted and (
                            (direction == "SELL" and c["crh"] <= zone_top * 1.02) or
                            (direction == "BUY"  and c["crl"] >= zone_bot * 0.98)
                        )
                        if overlaps or post_react:
                            zone_crts.append(c)
                    ...
                    if tbs_found:
                        ...
                        log(f"🐢 TBS already confirmed — FIRING NOW: {symbol} on {tbs_tf} | "
                            f"Entry:{tbs_entry} SL:{tbs_sl} TP:{tp_lvl} RR:{rr}R")
                        ...
                        sig = {
                            ...
                            "sl":          round(tbs_sl, 8),
                            "tp":          round(tp_lvl, 8),
                            "tp1":         round((tbs_entry + tp_lvl) / 2, 8),
                            "tp2":         round(tp_lvl, 8),
                            "rr":          rr,
                            "crh":         crt_crh,
                            "crl":         crt_crl,
                            ...
                            "details": [
                                ...
                                f"✅ CRT on {crt_tf}: CRH={crt_crh} CRL={crt_crl}",
                                f"🐢 TBS on {tbs_tf}: Entry={tbs_entry} SL={tbs_sl}",
                                f"TP: {tp_lvl} ({'CRL' if direction == 'SELL' else 'CRH'}) | RR:{rr}R",
                                f"⚡ Signal fired at TBS close (enter at TBS open {tbs_entry})",
                            ],
                            "tbs_found":   True,
                            "tbs_tf":      tbs_tf,
                            "tbs_entry":   tbs_entry,
                ...
                if phase == "AWAIT_TBS":
                    ...
                    log(f"🐢 M1 TBS CONFIRMED — FIRING SIGNAL NOW: {symbol} on {tbs_tf} | "
                        f"Entry(TBS open):{tbs_entry} SL(TBS close):{tbs_sl} TP:{tp} RR:{rr}R")
...
<div class="tab-wrap">
  ...
  <div id="page-signals" class="page">
    ...
    <div style="margin-top:20px">
      <div class="panel-ttl" style="font-family:'Fredoka One',sans-serif;font-size:1rem;letter-spacing:.06em;color:rgba(167,139,250,.8);margin-bottom:10px">👁 Monitor</div></div>
      </div></div>
      </div></div>
      </div>
    </div>
...
async function syncModelToggles(){try{const r=await fetch("/api/settings");const d=await r.json();[1].forEach(n=>{const key=`model${n}_enabled`;const on=d[key]!==false;const btn=$(`m${n}-toggle-btn`);if(btn){const labels={1:"🎯 Model #1"};btn.textContent=`${labels[n]||"Model #"+n}: ${on?"ON":"OFF"}`;btn.style.borderColor=on?"rgba(16,185,129,.4)":"rgba(239,68,68,.3)";btn.style.background=on?"rgba(16,185,129,.1)":"rgba(239,68,68,.08)";btn.style.color=on?"var(--green)":"var(--red)";}});}catch{}}
...
def api_settings():
    if request.method == "POST":
        ...
        with settings_lock:
            ...
            for k in ["tg_signals","tg_trades","model1_enabled"]:
