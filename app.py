...
def fmt_tg_m4(sig):
    ...
    return (
        f"{e} <b>MAD MAN MODEL #4 — {sig['direction']}</b>\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"<b>Pair:</b>      {sig['symbol']}\n"
        f"<b>HTF Zone:</b>  {hl} · {sig.get('ob_zone','–')}\n"
    ...
    for htf in M4_HTF:
        ...
        for kl in kls[:6]:
            ...
            if not already and not full:
                ...
                log(f"👁 M4 QUEUED: {symbol} {direction} | {htf} {pd_name} OB={zone_bot}–{zone_top} | fresh ✅ choch ✅ tapping ✅")
            break
            ...
            for symbol in symbols:
                ...
                if ticker:
                    ...
                    if zone_mid > 0 and abs(price - zone_mid) / zone_mid > 0.08:
                        ...
                        log(f"❌ M4 EXPIRED: {symbol} — price 8%+ from zone")
                        continue

                # ════ PHASE: AWAIT_SWEEP ════════════════════════════════
                if phase == "AWAIT_SWEEP":
                    # Look for a swing high/low INSIDE the HTF OB zone on LTF
                    # then a sweep of that level
                    swing_target = None
                    for c in reversed(ltf_c[-60:]):
                        if direction == "SELL" and zone_bot <= c["high"] <= zone_top:
                            swing_target = c["high"]; break
                        if direction == "BUY"  and zone_bot <= c["low"]  <= zone_top:
                    ...
                    log(f"✅ M4 SWEEP: {symbol} {direction} sweep={round(sweep_extreme,8)}")
                    with m4_lock:
                ...
                if phase == "AWAIT_CHOCH":
                        ...
                        if st:
                            try:
                                ...
                                if (datetime.now(LOCAL_TZ) - t0).total_seconds() > 7200:
                                    with m4_lock: m4_monitor.pop(symbol, None)
                                    log(f"❌ M4 TIMEOUT: {symbol} no CHoCH in 2hr")
                            except: pass
                        continue
                    ...
                    if rr < 1.0:
                        with m4_lock: m4_monitor.pop(symbol, None)
                        log(f"⚠️ M4 {symbol} RR={rr}R < 1.0 — skip")
                        continue

                    log(f"✅ M4 CHoCH: {symbol} {direction} choch={choch_lvl} ob={ob_bot}–{ob_top} RR={rr}R")
                    with m4_lock:
                        if symbol in m4_monitor:
                            m4_monitor[symbol].update({
                                "phase":      "AWAIT_TAP",
                ...
                if phase == "AWAIT_TAP":
                    ...
                    if direction == "SELL" and price > ob_top * 1.003:
                        with m4_lock: m4_monitor.pop(symbol, None)
                        log(f"❌ M4 INVALID: {symbol} blew through OB top")
                        continue
                    if direction == "BUY" and price < ob_bot * 0.997:
                        with m4_lock: m4_monitor.pop(symbol, None)
                        log(f"❌ M4 INVALID: {symbol} blew through OB bot")
                        continue
                    ...
                    if not tapped and ct:
                        try:
                            ...
                            if (datetime.now(LOCAL_TZ) - t0).total_seconds() > 14400:
                                with m4_lock: m4_monitor.pop(symbol, None)
                                log(f"❌ M4 EXPIRED: {symbol} OB not tapped in 4hr")
                        except: pass
                        continue

                    if not tapped: continue

                    log(f"🚀 M4 OB TAPPED: {symbol} {direction} price={price} ob={ob_bot}–{ob_top} RR={rr}R — SIGNAL!")
                    ...
                    sig = {
                        ...
                        "entry_type":    "Model #4 (30m OB + LTF Sweep→CHoCH→OB)",
                        "sl":            sl,
                        "tp":            round(liq_tgt, 8),
                    ...
                    send_telegram(fmt_tg_m4(sig), kind="signal")
                    ...
                    if paper_config["enabled"] and paper_config["auto_trade"]:
                        ...
                        if ok2: log(f"📝 M4 paper: {msg2}")

                    with m4_lock:
                        m4_monitor.pop(symbol, None)

        except Exception as e:
            log(f"❌ M4 monitor error: {e}")
        time.sleep(5)
        ...
        try:
            ...
            for symbol in symbols:
                ...
                if phase == "RUNNING":
                    ...
                    entry  = monitor.get("signal_entry", 0)
                    # TP hit
                    if direction == "BUY"  and price >= tp_p and tp_p > 0:
                        log(f"🏆 M1 TP HIT: {symbol} price={price} TP={tp_p}")
                        with manip_lock: manip_monitor.pop(symbol, None)
                        continue
                    if direction == "SELL" and price <= tp_p and tp_p > 0:
                        log(f"🏆 M1 TP HIT: {symbol} price={price} TP={tp_p}")
                        with manip_lock: manip_monitor.pop(symbol, None)
                        continue
                    # SL hit
                    if direction == "BUY"  and price <= sl_p and sl_p > 0:
                        log(f"💀 M1 SL HIT: {symbol} price={price} SL={sl_p}")
                        with manip_lock: manip_monitor.pop(symbol, None)
                        continue
                    if direction == "SELL" and price >= sl_p and sl_p > 0:
                        log(f"💀 M1 SL HIT: {symbol} price={price} SL={sl_p}")
                        with manip_lock: manip_monitor.pop(symbol, None)
                        continue
                    # Still running — update badge time
                    with manip_lock:
                        if symbol in manip_monitor:
                            manip_monitor[symbol]["last_price"] = price
                    continue

                # ── PHASE: WATCHING — standard C2 invalidation + TBS check ─
                ...
                log(f"✅ MANIP COMPLETE: {symbol} {direction} — searching TBS...")
                ...
                sl_final = (round(c2_mon.get("low",  tbs_sl), 8) if direction=="BUY"
                            else round(c2_mon.get("high", tbs_sl), 8))
                ...
                if rr < 2.0:
                    log(f"⚠️ {symbol} RR too low ({rr}R) — skipping")
                    with manip_lock: manip_monitor.pop(symbol, None)
                    continue
                ...
                sig = {
                    ...
                    "entry_type":  "Model #1 (TBS Open — Limit)",
                    "sl":          sl_final,
                    "tp":          round(tp, 8),
                    ...
                    "grade":       grade,
                    "details":     [
                        "✅ Manip monitor confirmed",
                        f"✅ TBS on {tbs_tf}",
                        f"Entry: TBS Open (LIMIT) | SL: C2 sweep extreme | TP: {'CRH' if direction=='BUY' else 'CRL'}",
                        f"RR: {rr}R",
                    ],
                    "tbs_found":   True,
                    "tbs_tf":      tbs_tf,
                    ...
                    "from_monitor":True,
                    "limit_order": True,   # place limit at TBS open price
                    "timestamp":   datetime.now(LOCAL_TZ).strftime("%Y-%m-%d %H:%M UTC+1"),
                }

                signals.appendleft(sig)
                send_telegram(fmt_tg(sig))
                log(f"🎯 M1 SIGNAL: {direction} {symbol} | {grade} | {rr}R | TBS:{tbs_tf} | LIMIT @ {tbs_entry}")

                # ── Place LIMIT order at TBS open price ──────────────────
                ready, reason = live_trade_ready()
                if ready:
                    ok, msg = place_order(sig)
                    log(f"{'✅' if ok else '❌'} M1 limit order: {msg}")
                else:
    ...
    while True:
        try:
            ...
            for i,symbol in enumerate(pairs):
                ...
                # Skip pairs already queued in ANY monitor — re-scanning them generates duplicates
                with manip_lock:
                ...
                if already_in_monitor or already_in_m2 or already_in_m4:
                    log(f"⏩ {symbol} — in monitor queue, skipping scan")
                    time.sleep(1)
                    continue
                ...
                try:
                    ...
                    m4 = scan_pair_model4(symbol) if scan_settings.get("model4_enabled", True) else []
                    all_res = m1 + m2 + m4
                    for sig in all_res:
                        m = sig.get("model","1")
...
<div class="tab-wrap">
  ...
  <div id="tab-monitor" style="display:none">
    ...
    <div class="panel" style="margin-top:14px">
      <div class="panel-ttl" style="color:#38bdf8">👁 Model #2 &amp; #3 — Pattern Monitor <span id="m2-mon-count" style="font-family:'JetBrains Mono',monospace;font-size:.72rem;color:var(--dim)">(0/6)</span></div>
      <div style="font-family:'JetBrains Mono',monospace;font-size:.62rem;color:var(--dim);margin-bottom:12px;line-height:1.6">Pairs queued after first-touching an HTF key level — monitor watches for M2 (Sweep→FVG) or M3 (Sweep→CHoCH→FVG) pattern to complete before firing a signal.</div>
      <div id="m2-monitor-wrap"><div class="empty" style="padding:40px"><div class="empty-ico">🔭</div><div class="empty-t">No M2/M3 pairs queued</div><div class="empty-s">Pairs enter here when price first touches an HTF OB/BB in a P/D zone. Signal fires when full pattern completes.</div></div></div>
    </div>
  </div>
  <!-- HISTORY -->
  ...
  <div id="tab-trade-cfg" style="display:none">
    <div class="panel">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:18px">
        ...
        <button id="m2-toggle-btn" onclick="toggleModelScan(2)" style="padding:10px 8px;border-radius:10px;border:2px solid rgba(16,185,129,.4);background:rgba(16,185,129,.1);color:var(--green);font-family:'JetBrains Mono',monospace;font-size:.68rem;font-weight:700;cursor:pointer">👁 Model #2: ON</button>
        ...
        <button id="m4-toggle-btn" onclick="toggleModelScan(4)" style="padding:10px 8px;border-radius:10px;border:2px solid rgba(16,185,129,.4);background:rgba(16,185,129,.1);color:var(--green);font-family:'JetBrains Mono',monospace;font-size:.68rem;font-weight:700;cursor:pointer">💎 Model #4: ON</button>
      </div>
      <div class="trade-form">
        ...
        <div class="tf-group"><div class="tf-lbl">Leverage (10–500x) <span style="font-size:.7rem;color:var(--dim)">0 = auto-calculate</span></div><input class="tf-inp" type="number" id="t-leverage" value="0" min="0" max="500" step="1" placeholder="0 = auto"/><div style="font-size:.68rem;color:var(--dim);margin-top:4px;font-family:'JetBrains Mono',monospace">Auto: bot calculates leverage to risk exactly your % per trade · Manual: overrides auto</div></div>
        <div class="tf-group"><div class="tf-lbl">Max Simultaneous Open Trades</div><input class="tf-inp" type="number" id="t-max" value="3" min="1" max="10" step="1"/></div>
...
async function fetchMonitor(){if(activeTab!=="monitor")return;try{
  ...
  $("m2-mon-count").textContent=`(${d2.length}/6)`;
  const w2=$("m2-monitor-wrap");
  if(w2){
    if(!d2.length){
      w2.innerHTML='<div class="empty" style="padding:40px"><div class="empty-ico">🔭</div><div class="empty-t">No M2/M3 pairs queued</div><div class="empty-s">Pairs enter here when price first touches an HTF OB/BB in a P/D zone.</div></div>';
    }else{
      const phaseLabel={"AWAIT_PATTERN":"👀 Awaiting Pattern","AWAIT_FVG":"🔍 Awaiting FVG","AWAIT_TAP":"🎯 Awaiting FVG Tap"};
      const phaseColor={"AWAIT_PATTERN":"var(--yellow)","AWAIT_FVG":"var(--orange)","AWAIT_TAP":"var(--green)"};
      w2.innerHTML=`<div class="monitor-grid">${d2.map(m=>{
        const dir=(m.direction||"BUY").toUpperCase();
        const phase=m.phase||"AWAIT_PATTERN";
        const model=m.model?`Model #${m.model}`:"M2/M3";
        const htf=TFM[m.htf]||m.htf||"–";
        const ltf=TFM[m.ltf]||m.ltf||"–";
        return`<div class="mon-card ${dir.toLowerCase()}" style="border-color:rgba(56,189,248,.25)">
        <div class="mon-sym">${dir==="BUY"?"🟢":"🔴"} ${m.symbol||"–"} <span style="font-size:.6rem;color:#38bdf8;font-family:'JetBrains Mono',monospace">${model}</span></div>
        ...
        <div class="mon-row"><span>Zone</span><span>${(m.zone_name||"–").slice(0,20)}</span></div>
        ...
        <div class="mon-row"><span>Added</span><span>${m.added_at||"–"}</span></div>
        <div class="mon-status" style="background:rgba(56,189,248,.1);border-color:rgba(56,189,248,.3);color:${phaseColor[phase]||"var(--yellow)"}">
...
async function loadTradeConfig(){try{const r=await fetch("/api/trade-config");const d=await r.json();if(d.api_key)$("t-apikey").value=d.api_key;$("t-risk").value=d.risk_pct||1;if($("t-leverage"))$("t-leverage").value=d.leverage||0;if($("t-max"))$("t-max").value=d.max_trades||3;updateTradeBadge(d.enabled);}catch{}}
async function syncModelToggles(){try{const r=await fetch("/api/settings");const d=await r.json();[1,2,3,4].forEach(n=>{const key=`model${n}_enabled`;const on=d[key]!==false;const btn=$(`m${n}-toggle-btn`);if(btn){const labels={1:"🎯 Model #1",2:"👁 Model #2",3:"🔍 Model #3",4:"💎 Model #4"};btn.textContent=`${labels[n]}: ${on?"ON":"OFF"}`;btn.style.borderColor=on?"rgba(16,185,129,.4)":"rgba(239,68,68,.3)";btn.style.background=on?"rgba(16,185,129,.1)":"rgba(239,68,68,.08)";btn.style.color=on?"var(--green)":"var(--red)";}});}catch{}}
...
async function loadSettingsBal(){
  ...
  // Only save keys if the user actually typed something in the fields
  // Never overwrite env-loaded keys with blank strings
  const apiKey=((document.getElementById("s-apikey")||{}).value||"").trim();
  const apiSec=((document.getElementById("s-secret")||{}).value||"").trim();
  if(apiKey||apiSec){
    const patch={};
    if(apiKey)patch.api_key=apiKey;
    if(apiSec)patch.api_secret=apiSec;
    try{await fetch("/api/settings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(patch)});}catch{}
  }
  try{
...
async function saveAllSettings(){
  ...
  // Only include API key/secret if the user actually typed something — never overwrite env vars with blanks
  const apiKey=((g("s-apikey")||{}).value||"").trim();
  const apiSec=((g("s-secret")||{}).value||"").trim();
  if(apiKey) payload.api_key=apiKey;
  if(apiSec) payload.api_secret=apiSec;
...
async function poll(){tick++;const ps=[fetchSigs(),fetchStats(),fetchState()];if(tick%(priceRefreshSecs)===0)ps.push(fetchPrices());if(activeTab==="log")ps.push(fetchLog());if(activeTab==="log"&&tick%3===0)ps.push(fetchDiag());if(activeTab==="monitor"&&tick%2===0)ps.push(fetchMonitor());if(activeTab==="trades"&&tick%3===0)ps.push(window.fetchPnl());if(activeTab==="trade-cfg"&&tick%3===0)ps.push(window.fetchPnl());if(activeTab==="history"&&tick%5===0)ps.push(fetchHistory());if(activeTab==="paper"&&tick%2===0)ps.push(fetchPaperData());await Promise.all(ps);setTimeout(poll,1000);}
...
def api_settings():
    if request.method == "POST":
        ...
        with settings_lock:
            ...
            for k in ["tg_signals","tg_trades","model1_enabled","model2_enabled","model3_enabled","model4_enabled"]:
    ...
    out["model4_enabled"] = scan_settings.get("model4_enabled", True)
            ...
            for symbol in symbols:
                ...
                if phase == "AWAIT_FVG":
                    ...
                    if model_tag == "3" and choch_idx is not None:
                        ...
                        pre_fvgs = _find_fvg_in_range(ltf_c, max(0, sweep_idx - 20), choch_idx, direction)
                        if pre_fvgs:
                        ...
                        # M2: FVG forms after sweep — search broadly
                        post_fvgs = _find_fvg_in_range(ltf_c, max(0, sweep_idx - 10), len(ltf_c), direction)
                        if post_fvgs:
                            fvg_found_data = post_fvgs[0]
...
def start_scanner():
    ...
    log("🚀 Scanner + M1 + M2/M3 + M4 + paper monitor threads launched.")
