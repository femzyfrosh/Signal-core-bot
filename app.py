...
trade_config = {
    ...
    "risk_pct":      25.0,    # 25% risk per trade
    "max_trades":    5,       # max 5 simultaneous open trades
...
def _run_startup_diagnostics():
    """Run on bot startup: log and Telegram-notify key presence, trading mode."""
...
def mexc_request(method, path, params=None, signed=True, _retry=0):
    ...
    try:
        ...
        if r.status_code == 403:
            ...
            log(f"     1. Futures trading permission not enabled on API key")
            log(f"     2. API key or secret mismatch / copy-paste error")
            log(f"     3. Timestamp drift > 5000ms — check server clock")
            tg_alert = (
                "🚫 <b>MEXC 403 ACCESS DENIED</b>\n\n"
                "<b>How to fix:</b>\n"
                "1. Go to MEXC → API Management\n"
                "2. Make sure <b>Futures trading</b> permission is enabled\n"
                "3. Check API key/secret are correct\n\n"
...
.tabs{display:flex;gap:6px;background:var(--s1);border:2px solid var(--border);border-radius:16px;padding:5px;margin-bottom:18px;overflow-x:auto;scrollbar-width:none}
.tabs::-webkit-scrollbar{display:none}
.tab{flex:1;min-width:72px;padding:9px 10px;border:none;border-radius:12px;font-family:'Nunito',sans-serif;font-size:.73rem;font-weight:800;cursor:pointer;transition:all .2s;color:var(--dim);background:transparent;white-space:nowrap;text-align:center}
  ...
  <div id="tab-trade-cfg" style="display:none">
    <div class="panel">
      ...
      <div class="trade-form">
        <div class="tf-group"><div class="tf-lbl">Risk per Trade (%)</div><input class="tf-inp" type="number" id="t-risk" value="25" min="0.1" max="100" step="0.1"/></div>
        ...
        <div class="tf-group"><div class="tf-lbl">Max Simultaneous Open Trades</div><input class="tf-inp" type="number" id="t-max" value="5" min="1" max="10" step="1"/></div>
    ...
    <div class="panel">
      ...
      <div class="trade-form">
        ...
        <div class="tf-group">
          ...
          <input class="tf-inp" type="number" id="p-risk" value="25" min="0.1" max="100" step="0.1"/>
        </div>
        <div class="tf-group">
          <div class="tf-lbl">Max Simultaneous Trades</div>
          <input class="tf-inp" type="number" id="p-max" value="5" min="1" max="10" step="1"/>
        </div>
        <div class="tf-group">
...
async function fetchPrices(){try{const r=await fetch("/api/prices");const data=await r.json();$("pupd").textContent="Updated "+new Date().toLocaleTimeString();const top5=TOP.slice(0,5).map(sym=>({sym,d:data[sym]||{}}));$("pgrid").innerHTML=top5.map(({sym,d})=>{const name=sym.replace("_USDT","");const up=(d.change||0)>=0;return`<div class="pc ${up?"up":"dn"}"><div class="pc-sym">${name}/USDT</div><div class="pc-price ${up?"up":"dn"}">${d.price?fmtP(d.price):"–"}</div><span class="pc-chg ${up?"up":"dn"}">${d.price?(up?"▲":"▼")+" "+Math.abs(d.change||0).toFixed(2)+"%":"–"}</span></div>`;}).join("");}catch{}}
...
<div class="app">
  ...
  <div class="main">
    ...
    <div class="page visible" id="page-watchlist">
      <div class="wl-header">
        <div class="wl-title" style="font-weight:900;letter-spacing:.12em">WATCHLIST · 30</div>
        <div class="wl-search">
    ...
    <div class="page hidden-right" id="page-chart">
      ...
      <div class="tf-bar" id="tf-bar">
        ...
        <button class="tf-btn active" data-tf="Min15" onclick="switchTF(this,'Min15')">15m</button>
        ...
        <button class="tf-btn" id="draw-trend-btn" onclick="toggleDrawMode('trendline')" title="Draw Trendline" style="min-width:36px;font-size:.72rem;letter-spacing:0">╱ Line</button>
        <button class="tf-btn" id="draw-rect-btn"  onclick="toggleDrawMode('rectangle')"  title="Draw Rectangle"  style="min-width:36px;font-size:.72rem;letter-spacing:0">▭ Rect</button>
        <button class="tf-btn" id="draw-clear-btn" onclick="clearDrawings()"               title="Clear Drawings"  style="min-width:36px;font-size:.72rem;letter-spacing:0;color:var(--red)">✕ Clear</button>
      </div>
      ...
      <div class="chart-wrap">
        ...
        <!-- Candle-close countdown overlay (bottom-left of chart) -->
        <div id="candle-countdown-overlay" style="display:none;position:absolute;bottom:44px;left:10px;z-index:10;background:rgba(7,8,16,.88);border:1px solid rgba(79,110,247,.28);border-radius:8px;padding:5px 12px;backdrop-filter:blur(10px);pointer-events:none;user-select:none">
          <span style="font-family:var(--mono);font-size:.52rem;color:var(--dim);text-transform:uppercase;letter-spacing:.07em;display:block;margin-bottom:1px">Candle closes</span>
          <span id="candle-countdown" style="font-family:var(--mono);font-size:.92rem;font-weight:800;color:#ffc93c;letter-spacing:.06em">00:00</span>
        </div>
...
let drawMode=null;   // 'trendline' | 'rectangle' | null
let drawStart=null;  // {x, price}
const drawings=[];   // [{type, series/shapes}]

function toggleDrawMode(mode){
  if(drawMode===mode){
    drawMode=null;
    document.getElementById("draw-trend-btn").classList.remove("active");
    ...
    drawMode=mode;
    drawStart=null;
    document.getElementById("draw-trend-btn").classList.toggle("active",mode==="trendline");
    document.getElementById("draw-rect-btn").classList.toggle("active",mode==="rectangle");
...
function clearDrawings(){
  drawings.forEach(d=>{
    try{if(d.series)chart.removeSeries(d.series);}catch{}
    try{if(d.series2)chart.removeSeries(d.series2);}catch{}
    try{if(d.div)d.div.remove();}catch{}
  });
  drawings.length=0;
}
...
function getChartCoords(e){
  ...
  const price=chart.priceScale("right").coordinateToPrice(y,candleSeries);
...
document.getElementById("chart-container").addEventListener("click",function(e){
  if(!drawMode||!chart||!candleSeries)return;
  ...
  const p1=drawStart,p2=coords;
  drawStart=null;
  if(drawMode==="trendline"){
    // Black 2px trendline
    const lineSeries=chart.addLineSeries({color:"#000000",lineWidth:2,crosshairMarkerVisible:false,lastValueVisible:false,priceLineVisible:false});
    const pts=[{time:p1.time,value:p1.price},{time:p2.time,value:p2.price}].sort((a,b)=>a.time-b.time);
    lineSeries.setData(pts);
    drawings.push({type:"trendline",series:lineSeries});
  }else if(drawMode==="rectangle"){
    // Gray filled rectangle using HTML div overlay (no border lines)
    ...
    if(x1!==null&&x2!==null&&y1!==null&&y2!==null){
      const lf=Math.min(x1,x2),tp=Math.min(y1,y2);
      const wd=Math.abs(x2-x1),ht=Math.abs(y2-y1);
      const div=document.createElement("div");
      div.style.cssText=`position:absolute;left:${lf}px;top:${tp}px;width:${wd}px;height:${ht}px;background:rgba(180,180,180,0.22);pointer-events:none;z-index:5;box-sizing:border-box;`;
      cont.appendChild(div);
      drawings.push({type:"rectangle",div:div});
    }
  }
