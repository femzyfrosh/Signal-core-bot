...
def api_diag():
    ...
    # Test auth by hitting /account/assets
    auth_test = "not tested"
    if api_key and api_secret and "PASTE" not in api_key:
        _, auth_err = get_account_balance()
        auth_test = "✅ OK" if not auth_err else f"❌ {auth_err}"
    ...
    return jsonify({
        ...
        "trading": {
            ...
            "trading_mode":   "Cross Margin (openType=2)",
            "auto_trade":     trade_config.get("enabled", False),
            "mexc_ping":      mexc_ping,
            "auth_test":      auth_test,
        }
    })
