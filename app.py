...
function _screenToChart(canvas,cx,cy){
  ...
  // px/py = position within the canvas element (CSS pixels)
  const px=cx-bb.left, py=cy-bb.top;
  // Guard: reject coords outside canvas bounds
...
function resizeDrawCanvas(){
  ...
  if(!parent)return;
  // Match parent's actual rendered size (not offsetWidth which may lag)
  const pr=parent.getBoundingClientRect();
  const w=Math.round(pr.width),h=Math.round(pr.height);
  if(w>0&&h>0&&(c.width!==w||c.height!==h)){
    c.width=w;c.height=h;
    // Force canvas CSS size to match exactly
